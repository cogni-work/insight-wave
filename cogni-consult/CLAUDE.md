...
- **Publish seam**: `consult-publish` is consultant-elected and never auto-fired. Every format appends `{format, brief_path, route_steps, source_deliverable, published_at}` to `publish[]`; slides/web-poster may add `artifact_path` after an elected public cogni-publishing validate → compose → PPTX/HTML render. Report and infographic remain Claude Design handoffs. Brief and artifact fields are path references, never copied content; no route re-narrates the Pyramid/MECE/SCQA source. Optional polish and local rendering degrade to the native brief when publishing is absent. Canonical routing: `references/publish-routing.md`; schema: `references/data-model.md`
