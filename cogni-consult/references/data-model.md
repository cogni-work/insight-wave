...
discipline. (The convention applies to numbers a consultant registers; the Test-stage
promote-check now wires the authoring stages to register them by default —
`references/orchestration/test-promote-check.md`.)
...
| `citation` | No | Source-lineage quad (`source_url`, `entity_ref`, `propagated_at`, `claim_id?`) per the monorepo convention, so `cogni-workspace:claims` corrections can cascade to the assumption; an optional `source_title` is added when a claim resolves via `alternative_source`. `claim_id` is the claim-verification back-reference the verify round-trip writes (`scripts/submit-assumption-claim.py propagate`); it is what the render-time verified-evidence gate checks, so a claim-type assumption can carry `status: "verified"` only when this id resolves to a ClaimRecord that is itself verified. `resolve-propagate` mutates the quad when a claim is disputed and resolved: `corrected` rewrites `value` (`claim_id` current), `alternative_source` rewrites `source_url`/`source_title` (value untouched), and `discarded` clears `claim_id` (value retained as a last-known figure) — all three stamp `propagated_at` and demote `status` `verified`→`reviewed`. Note the naming collision: this `entity_ref` is cogni-consult's flat self-identity **string** coordinate, while a ClaimRecord's `entity_ref` is the cross-plugin **object** locator `{type, file, field_path}` — the submit-time adapter maps the former onto the latter; the two shapes never share a store |
...
| `status` | No | Where the value sits on the verification ladder `stated → reviewed → verified`, **capped by `provenance_type`**: `given` may only be `stated`; `estimate` may be hand-set up to `reviewed`. Only a `claim` may reach `verified`, and only through the **live `cogni-workspace:claims` verify round-trip**: `scripts/submit-assumption-claim.py submit` appends the ClaimRecord, `cogni-workspace:claims` verifies it, and `propagate` writes `verified` + `citation.claim_id` back onto the record. The resolver's verified-evidence gate then requires that back-reference to resolve to a ClaimRecord that is itself verified — a hand-set `verified` without genuine claims evidence is still rejected fail-loud, so a guess can never carry a verified confidence marker. When that ClaimRecord is later disputed and resolved (`corrected`/`alternative_source`/`discarded`), `resolve-propagate` demotes the assumption `verified`→`reviewed`, since a `resolved` (no longer `verified`) record must stop satisfying the gate |
...
`start_date`, `due_date`, `duration`, `owner`, `milestone` (all optional, all
absent by default) record the deliverable's schedule: `start_date` / `due_date`
are ISO 8601 dates (`YYYY-MM-DD`), `duration` is estimated effort in whole days
(an integer), `owner` is a free-text name or role, and `milestone` is a boolean
marking a plan-level checkpoint. They are single-sourced on the deliverable entry
— never on the engagement-root `consult-project.json` — so schedule lives beside
`state` and `depends_on[]`, the same place a read surface already reads them. Like
the other optional deliverable fields they need no script change to reach read
surfaces — `engagement-status.sh` passes every deliverable field through verbatim
(its rollup reads only `state`). The phased-timeline read-model derived from these
fields plus `depends_on[]` is the canonical contract in
`references/project-plan-model.md`.
...
knowledge-base page/claim ref) so `cogni-workspace:claims` corrections can cascade to
deliverables:
...
`cogni-workspace:claims` corrections cascade. This is the deliverable-`sources[]` analogue
...
- **decision-log.json** — key decisions with rationale and evidence refs: `{"decisions": [{"id": "d-001", "action_field": "...", "deliverable": "...", "decision": "...", "rationale": "...", "evidence_refs": [...], "timestamp": "..."}]}`. Gap-check entries share the same array, discriminated by `"kind": "gap-check"`: `{"id": "d-002", "kind": "gap-check", "action_field": "...", "deliverable": "...", "question": "<verbatim --question>", "theme_label": null, "verdict": "covered", "top_hit": "<page-slug>", "top_score": 0.36, "timestamp": "..."}` (recording contract: `references/research-routing.md`, Gap-Check Recording). Framework-selection entries likewise share the array, discriminated by `"kind": "framework-selection"`: `{"id": "d-003", "kind": "framework-selection", "action_field": "...", "deliverable": "...", "candidates_presented": ["pyramid-principle", "scqa", "..."], "chosen": "pyramid-principle", "is_combination": false, "rationale": "...", "timestamp": "..."}` — `candidates_presented[]` is the top-5 framework slugs shown at deliverable creation, `chosen` is the value stored in the deliverable's `chosen_framework` (a registry slug, a `"combo:<slugA>+<slugB>"` string, or `null`), and `is_combination` is `true` when `chosen` is a combo. Evidence-provenance-waiver entries likewise share the array, discriminated by `"kind": "evidence-provenance-waiver"`: `{"id": "d-004", "kind": "evidence-provenance-waiver", "action_field": "...", "deliverable": "...", "evidence_class": "<provenance class>", "rationale": "...", "timestamp": "..."}` — appended at deliverable completion when no `gap-check` verdict covers the deliverable, naming the `evidence_class` (mirrored to the deliverable entry) and the consultant's rationale for completing without a recorded gap-check. A `gap-check` OR an `evidence-provenance-waiver` for the deliverable's `(action_field, deliverable)` coordinates is the provenance record `engagement-status.sh` checks for. Diagnostic-field-0-waiver entries also share the array, discriminated by `"kind": "diagnostic-field-0-waiver"`: `{"id": "d-005", "kind": "diagnostic-field-0-waiver", "rationale": "...", "timestamp": "..."}` — appended by `consult-scope` at WBS-close when the consultant opts the engagement out of the default diagnostic field-0 (whose canonical default slug is `diagnostic-as-is`). Unlike the deliverable-stage kinds above it carries **no** `action_field`/`deliverable` coordinates, because it is recorded at scope close before any field or deliverable exists; the `rationale` is the consultant's stated reason for scoping without a diagnostic field-0. Key-question-grounding entries also share the array, discriminated by `"kind": "key-question-grounding"`: `{"id": "d-006", "kind": "key-question-grounding", "verdict": "grounded", "rationale": "...", "timestamp": "..."}` — appended by `consult-scope` at the Touch-2 re-validation step (after the five scoping dimensions are gathered) recording whether the key question was re-confirmed grounded in the diagnosed as-is problem; `verdict` is `"grounded"` when the original question held or `"reframed"` when it was rewritten against the as-is material. Like the diagnostic-field-0-waiver it carries **no** `action_field`/`deliverable` coordinates — it is recorded at scope time before any field exists. Kb-deposit-waiver entries also share the array, discriminated by `"kind": "kb-deposit-waiver"`: `{"id": "d-007", "kind": "kb-deposit-waiver", "action_field": "...", "deliverable": "...", "evidence_class": "<provenance class>", "rationale": "...", "timestamp": "..."}` — appended by `consult-design-thinking` at the Step 8 close-the-session gate when the consultant declines the elective deposit of a completed deliverable into the engagement's bound knowledge base (the default-on `cogni-knowledge:knowledge-ingest-source` deposit). Like the deliverable-stage kinds above it carries the `(action_field, deliverable)` coordinates and the deliverable's `evidence_class`; it is append-once per deliverable (check `decisions[]` for an existing `kb-deposit-waiver` with the same coordinates before appending, mirroring the evidence-provenance-waiver "if none exists, append" discipline), so a Step-8 re-run on resume neither double-deposits nor double-logs. Adherence-review entries likewise share the array, discriminated by `"kind": "adherence-review"`: `{"id": "d-008", "kind": "adherence-review", "action_field": "...", "deliverable": "...", "chosen_framework": "<slug|combo>", "adherence": "strong"|"partial"|"drifted", "findings_count": 0, "summary": "...", "timestamp": "..."}` — appended by `consult-design-thinking` in the Test-stage survive branch (before the `state` → `complete` write) when the deliverable's `chosen_framework` is non-`null`, recording the read-only `consult-framework-adherence-reviewer` agent's structural-conformance verdict (the framework-adherence rung of the Three-Layer Quality Gate). Discrete filterable keys (`adherence`/`findings_count`), no prose `decision` string, mirroring the gap-check shape; a `null`-framework deliverable writes no such entry. It carries the `(action_field, deliverable)` coordinates and is append-once per completion — a genuine rework re-review appends a fresh entry (contract: `references/orchestration/test-adherence-review.md`). Plan-schedule-edit entries likewise share the array, discriminated by `"kind": "plan-schedule-edit"`: `{"id": "d-009", "kind": "plan-schedule-edit", "action_field": "...", "deliverable": "...", "field": "<start_date|due_date|duration|owner|milestone>", "from": "<old value or null>", "to": "<new value>", "rationale": "...", "timestamp": "..."}` — appended by the plan-editing surface whenever a consultant edits one of the five optional scheduling fields on a deliverable entry, naming the `field` and its `from`/`to` values so the roadmap read-model's audit trail matches the discipline of the deliverable-state kinds above. Discrete filterable keys (`field`/`from`/`to`), no prose `decision` string, mirroring the gap-check shape; it carries the `(action_field, deliverable)` coordinates and appends one entry per edit (the schema and read-model it serves: `references/project-plan-model.md`). Assumption-promotion entries likewise share the array, discriminated by `"kind": "assumption-promotion"`: `{"id": "d-010", "kind": "assumption-promotion", "action_field": "...", "deliverable": "...", "outcome": "promoted" | "declined" | "none-found", "promoted_ids": ["asm-<slug>", ...], "promoted_count": 0, "declined_count": 0, "timestamp": "..."}` — appended by `consult-design-thinking` in the Test-stage survive branch (before the `state` → `complete` write) when the act-by-default promote-check scans the finished artifact for bare quantified planning literals and registers the accepted ones into `assumptions.json` as `{{asm:<slug>}}` placeholders. Discrete filterable keys (`outcome`/`promoted_count`/`declined_count`), no prose `decision` string, mirroring the adherence-review shape; `outcome` is `declined` when promotable literals were found but the consultant opted out (an opt-out is a recorded decision) and `none-found` when the scan surfaced nothing. It carries the `(action_field, deliverable)` coordinates and is append-once per completion — a genuine rework re-run appends a fresh entry (contract: `references/orchestration/test-promote-check.md`). Rework-intent entries likewise share the array, discriminated by `"kind": "rework-intent"`: `{"id": "d-011", "kind": "rework-intent", "action_field": "...", "deliverable": "...", "intent": "<consultant's verbatim answer>", "target_stage": "define", "timestamp": "..."}` — appended by `consult-design-thinking` in the Step 2 Open-the-Loop rework branch (`complete` → `in-progress`), before the `dt-stage-advance.sh` call, recording what the consultant said the reopened deliverable should do better and which stage the rework re-enters (`target_stage` is the same value passed to the helper). Discrete filterable keys (`intent`/`target_stage`), no prose `decision` string, mirroring the gap-check shape. It carries the `(action_field, deliverable)` coordinates and, unlike the append-once waiver kinds, appends a fresh entry on **every** reopen — successive reworks have successive intents. Within a single reopen the append is idempotent: the most recent `rework-intent` entry for these coordinates, when it has no `"to": "complete"` transition later than its `timestamp` in `execution-log.json`'s `transitions[]` belongs to the same still-open episode and is reused rather than appended a second time, so a re-run of the same reopen does not double-append. The match is on `"to"`, not on the word *complete* — the reopen's own `complete` → `in-progress` transition is a `"from": "complete"` appended moments after the intent entry, and reading it as an episode close would defeat the guard on every re-run. A `"to": "complete"` transition does close the episode, and the next reopen keys a fresh entry. The Define stage reads the most recent entry for the deliverable as its first-ranked framing input, alongside (not in place of) the empathize outputs and the field's `framing`.
...
| cogni-workspace | Consumes | Deliverable `sources[]` carries the lineage triple (`source_url`, `entity_ref`, `propagated_at`) so claim corrections cascade to deliverables. Opt-in local-render fallback for publish briefs (`cogni-workspace:enrich-report` / `/render-infographic`, or `document-skills`) — the standard `consult-publish` path builds every format as a consult-native brief and hands it to Claude Design to render, so no local renderer sits on the standard export route |
