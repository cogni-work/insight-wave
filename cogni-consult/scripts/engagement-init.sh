...
if [ -f "$BASE_DIR/consult-project.json" ]; then
  ...
  # Backfill the browsable register stub too — register-generator.py overwrites
  # it (the generated-marker footer authorizes that), so an empty stub is a safe
  # regeneratable starting point rather than a hand-authored file it would refuse.
  if [ ! -f "$BASE_DIR/assumptions.md" ]; then
    printf '# Assumptions\n\nNo assumptions registered yet.\n\n---\n\n_Auto-generated assumption register — regenerated by register-generator.py; edits here are overwritten._\n' > "$BASE_DIR/assumptions.md"
  fi
...
# Browsable register stub: the human-facing view of assumptions.json (see
# references/data-model.md), seeded empty alongside the registry so the
# directory always carries the front-door register. Carries the generated-marker
# footer so register-generator.py regenerates it in place (an empty stub is a
# regeneratable starting point, not a hand-authored file the generator refuses).
with open(os.path.join(base, "assumptions.md"), "w") as f:
    f.write(
        "# Assumptions\n\nNo assumptions registered yet.\n\n---\n\n"
        "_Auto-generated assumption register — regenerated by "
        "register-generator.py; edits here are overwritten._\n"
    )
