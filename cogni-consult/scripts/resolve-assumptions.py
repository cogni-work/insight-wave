...
def _render_value(entry, link_render=False):
    ...
    With ``link_render`` (the ``--mode link`` capability) the value is wrapped in
    an Obsidian wikilink into the browsable register generated by
    register-generator.py — ``[[assumptions#<slug>|<value>]]``, where ``<slug>``
    is the entry id minus the ``asm-`` prefix (the register's ``## <slug>``
    heading anchor). The ``(prov: type/status)`` parenthetical still trails the
    link exactly as it trails a bare value. Double square brackets can never
    match the brace-only LOOSE_ASM_RE, so the post-substitution leftover check
    is untouched and cannot be tripped. The default (link_render=False) path is
    byte-for-byte the pre-existing literal-value behaviour.
    """
    ...
    return "%s (prov: %s/%s)" % (rendered, ptype, entry["status"])
...
def cmd_resolve(args):
    ...
    link_render = getattr(args, "mode", "value") == "link"
    resolved, count = PLACEHOLDER_RE.subn(
        lambda m: _render_value(registry[ID_PREFIX + m.group(1)], link_render), text)
...
def main():
    ...
    p_resolve.add_argument("--mode", choices=("value", "link"), default="value",
                           help="value (default): substitute the literal value (plus any "
                                "(prov: ...) marker) — the pre-existing publish behaviour. "
