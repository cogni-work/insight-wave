...
**Scaffold the diagnostic field-0 first.** Per the method reference, field-0 is always a diagnostic of the current state, not one of the solution fields — it precedes them and every solution field gates on it. Scaffold it by default ahead of the 3-6 solution fields: slug `diagnostic-as-is`, title `Diagnostic: Current State`, and a one-line CMO/as-is framing derived from the key question (e.g. "Where does the client stand today on the central problem?"). Then name the 3-6 solution fields per the method reference — each with a kebab-case slug, a title, and a one-line framing. Confirm the ordered, diagnostic-first set with the consultant, then persist it in two writes:
...
**Opt-out with a recorded reason.** The diagnostic field-0 is scaffolded by default; an engagement may decline it, but only on the record. When the consultant opts out, do not write the `diagnostic-as-is` field or its slug — instead append a waiver to `.metadata/decision-log.json` `decisions[]`, discriminated by `"kind": "diagnostic-field-0-waiver"`, carrying the consultant's `rationale` and a `timestamp`. Because the waiver is recorded at scope close — before any field or deliverable exists — it carries no `action_field`/`deliverable` coordinates, unlike the deliverable-stage decision kinds (schema owner: `$CLAUDE_PLUGIN_ROOT/references/data-model.md`). Opting out leaves the engagement diagnostic-free without taking the choice off-book.
