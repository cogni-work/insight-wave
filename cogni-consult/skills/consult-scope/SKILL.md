...
### 3.5. Touch 2 — Grounded Re-validation

The key question was first stressed against the SMART criteria plus Grounded while it was drafted in Step 3 (Touch 1). But Grounded — whether the question is anchored in the diagnosed as-is problem — can only be fully judged once the as-is material exists. Now that the five dimensions are gathered (including the two `[diagnostic-seed]` dimensions, Strategic Context and Constraints / Barriers), re-validate it per the convergence checklist in the method reference: confirm the converged `key_question` traces to concrete signal in that as-is material. If it has drifted free of the diagnosed problem, reframe it with the consultant and `Edit` the new wording into `consult-project.json` `key_question` (re-running the relevant SMART checks on the new wording).

Either way, record the verdict so the second touch is auditable: append an entry to `.metadata/decision-log.json` `decisions[]` discriminated by `"kind": "key-question-grounding"`, carrying an `id`, the `verdict` (`"grounded"` when the original question held, `"reframed"` when it was rewritten), the consultant's `rationale`, and a `timestamp`. Like the diagnostic-field-0 waiver, this entry carries **no** `action_field`/`deliverable` coordinates — it is recorded at scope time before any field exists. The schema is owned by `$CLAUDE_PLUGIN_ROOT/references/data-model.md`.
