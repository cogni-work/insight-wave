...
- **Validation (Step 5)**: existing checks plus translation-specific bullets — target charset matches (umlauts present for `de`, absent for `en`); citation count exactly preserved (not just `>=`); URLs byte-identical; frontmatter technical IDs unchanged; readability relative to source (score source and output on the target-language Flesch scale via `calculate_readability.py --lang $TARGET_LANG`, require `output_score >= source_score - 5`; absolute band remains an aspirational note). The absolute Flesch band (EN 50–60 / DE 30–50) applies only when `TARGET_LANG` is unset.
