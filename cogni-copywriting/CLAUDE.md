...
- **Pre-checks (Step 1)**: resolve source language via the existing detector; abort if `source_lang == TARGET_LANG` (no-op); apply the **arc-mode gate** (#3) — *allow* arc translation when both ends are EN/DE and `arc_id ∈ {corporate-visions, jtbd-portfolio}` (headings are substituted in Step 2.5, see below); *abort* for other arcs on an EN↔DE pair (coverage, #318) or when the pair involves FR/IT/PL/NL/ES (#318); abort if `TARGET_LANG` is outside the accept-set `{de,en,fr,it,pl,nl,es}` (#4); abort via the **pivot guard** (#5) if neither `source_lang` nor `TARGET_LANG` is in `{en,de}` (direct non-EN/DE pairs like fr→it are rejected — pivot via EN or DE).
...
Languages: any direction pivoting on EN or DE — `{de,en,fr,it,pl,nl,es}` with EN or DE on one end. Slice 1 of #255 landed (non-arc FR/IT/PL/NL/ES); Slice 2 landed (arc-mode **EN↔DE** for `corporate-visions` and `jtbd-portfolio`, headings substituted from the downstream mirror). Slice 3 (#318) remains: arc-mode FR/IT/PL/NL/ES (needs upstream `language-templates.md` to gain those languages) and broader EN↔DE arc coverage (mirror more arcs downstream). **Cross-plugin invocation is explicit**: translation runs only via `/copywrite --translate=`; downstream adapters (`narrative-adapt`, `channel-adapter`, `customer-narrative-writer`) do not silently chain it, avoiding hidden language-switches in pipelines.
