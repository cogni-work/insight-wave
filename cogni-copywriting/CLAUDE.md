...
- **Pre-checks (Step 1)**: resolve source language via the existing detector; abort if `source_lang == TARGET_LANG` (no-op); apply the **arc-mode gate** (#3) — *allow* arc translation when `arc_id ∈ {corporate-visions, jtbd-portfolio}` and the pair pivots on EN/DE (at least one end ∈ `{en,de}`), covering en↔de and en/de↔fr/it/pl/nl/es (headings are substituted in Step 2.5, see below); *abort (coverage)* for any other arc regardless of language; for an in-scope arc whose pair has neither end in `{en,de}`, fall through to the pivot guard. Then abort if `TARGET_LANG` is outside the accept-set `{de,en,fr,it,pl,nl,es}` (#4); abort via the **pivot guard** (#5) if neither `source_lang` nor `TARGET_LANG` is in `{en,de}` (direct non-EN/DE pairs like fr→it are rejected — pivot via EN or DE).
- **Arc-heading substitution (Step 2.5, `arc_mode` only)**: arc-element + bridge headings are **substituted** (not freely translated) with the canonical target-language headings read from the **downstream mirror in `09-preservation-modes/arc-preservation.md`** — the runtime skill does **not** read cogni-narrative files (no stable cross-plugin path). Elements are identified **positionally** (Nth arc-element H2 → element index N); the `audit-copywriter` C2/C3 checks keep the mirror in sync with upstream `language-templates.md`.
- **Pass A — Translate (new Step 2.5)**: loads `01-core-principles/translation-principles.md` plus the direction-specific guide, whose filename is constructed deterministically as `translation-{source_lang}-to-{TARGET_LANG}.md` (22 supported directions; see the validity matrix in `translation-principles.md`). Translates prose; preserves byte-identical: citations (markers + URLs), URLs in inline links, code blocks, frontmatter technical IDs (`arc_id`/`source_url`/`entity_ref`), protected content (`<diagram-placeholder>`, `Figure N`/`Abbildung N`, `![[assets/*.svg]]`, kanban tables), Power Position markers (`**IS**:`, `**DOES**:`, `**MEANS**:`), and acronyms (acronym expansion is Step 3's job, running on the translated text).
- **Pass B — Polish (existing Step 3)**: applies target-language style discipline. For DE output: Wolf-Schneider rules (12-word clauses, Satzklammer breaking, Mittelfeld shortening, Floskel elimination). For EN/FR/IT/PL/NL/ES output: 15–20 word sentences, 80%+ active voice, target-language clarity discipline. Audience-tuned acronym expansion runs here, on the translated text.
- **Validation (Step 5)**: existing checks plus translation-specific bullets — target charset matches the per-language diacritic set (single source of truth: `translation-principles.md` § "Per-Language Charset Rules"); citation count exactly preserved (not just `>=`); URLs byte-identical; frontmatter technical IDs unchanged; readability relative to source (score source and output on the target-language Flesch-family scale via `calculate_readability.py --lang $TARGET_LANG`, require `output_score >= source_score - 5`; absolute band remains an aspirational note). The absolute Flesch band applies only when `TARGET_LANG` is unset.

Languages: any direction pivoting on EN or DE — `{de,en,fr,it,pl,nl,es}` with EN or DE on one end. #255 is complete: Slice 1 landed non-arc FR/IT/PL/NL/ES; Slice 2 landed arc-mode EN↔DE for `corporate-visions` and `jtbd-portfolio`; Slice 3 (#318) extended arc-mode to FR/IT/PL/NL/ES for those two arcs (upstream `language-templates.md` now carries their headings; the downstream `arc-preservation.md` mirror holds all 7 languages). The only remaining arc-mode expansion is the other 9 arcs (any non-EN/DE language and their downstream mirror) — future work. **Cross-plugin invocation is explicit**: translation runs only via `/copywrite --translate=`; downstream adapters (`narrative-adapt`, `channel-adapter`, `customer-narrative-writer`) do not silently chain it, avoiding hidden language-switches in pipelines.
...
- Multilingual readability support: English uses Flesch Reading Ease (target 50-60), German uses Amstad (target 30-50) with Wolf Schneider style rules; FR/IT/PL/NL/ES use Flesch-family formulas (Kandel-Moles, Flesch-Vacca, generic fallback, Flesch-Douma, Szigriszt-Pazos) with aspirational bands — translation mode scores these on a relative-to-source rule. EN/DE coefficients and counters are unchanged; the `count_syllables` dispatcher keeps them byte-identical
...
- Readability script: `python3 scripts/calculate_readability.py <file> [--lang de|en|fr|it|pl|nl|es|auto]`
