...
Languages: any direction pivoting on EN or DE — `{de,en,fr,it,pl,nl,es}` with EN or DE on one end. Slice 1 of #255 landed (non-arc FR/IT/PL/NL/ES); arc-mode translation is still blocked and tracked in #255 (Slice 2 EN↔DE, Slice 3 FR/IT/PL/NL/ES, the latter requiring heading-set substitution via `cogni-narrative/skills/narrative/references/language-templates.md`, currently EN/DE only). **Cross-plugin invocation is explicit**: translation runs only via `/copywrite --translate=`; downstream adapters (`narrative-adapt`, `channel-adapter`, `customer-narrative-writer`) do not silently chain it, avoiding hidden language-switches in pipelines.
