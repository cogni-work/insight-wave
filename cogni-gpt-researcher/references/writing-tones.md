...
Available writing tones for report generation. The tone shapes vocabulary, sentence structure, and rhetorical approach throughout the report.
...
## Tone Interaction with Researcher Role

The `researcher_role` persona and `tone` setting work together:
- **Role** controls domain expertise, terminology, and analytical lens (e.g., "Financial Analyst" uses financial metrics)
- **Tone** controls rhetorical style and register (e.g., "persuasive" builds a case with strong conclusions)

Example: A "Cybersecurity Analyst" role with "critical" tone produces a security-focused evaluation that weighs threats and identifies vulnerabilities. The same role with "informative" tone produces an accessible security explainer.
