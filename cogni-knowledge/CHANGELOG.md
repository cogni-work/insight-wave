...
Scope discipline (smallest shippable, this release): the honesty pass is pure prose/format/manifest — zero behaviour change, zero schema change to `verify-vN.json` / `citation-manifest.json` / `binding.json`, two additive synthesis-page frontmatter keys, no new agent. The `--resweep` skeleton is a single `Skill(...)` dispatch into an existing primitive — opt-in only, never auto-dispatched, no cadence scheduler. The synthesis-page extractor adapter (so the resweep can pull cited URLs out of `[N]`/`[[slug]]`-style synthesis pages, vs the upstream's inline-URL heuristic), the `--resweep-on-finalize` gate, a cadence, and resweep-aware verify replay are deferred to v0.1.17+/v0.1.18+.

### Added

- **Two additive frontmatter keys on every new `wiki/syntheses/<slug>.md`** (`skills/knowledge-finalize/SKILL.md` Step 5):
  - `verification: citation_consistent_zero_network` — the fixed-enum string declaring what the per-run verifier actually checked.
  - `verification_ratio: "verbatim=<V> paraphrase=<P> synthesis=<S> unsupported=<U>"` — the same counts already in `verify-vN.json::counts`, surfaced on the durable artefact (double-quoted string scalar; additive — cogni-wiki `wiki-health` tolerates unknown frontmatter keys, same posture as `draft_revision_round`). The four counts captured at Step 2 are threaded into Step 5's compose subprocess as `VERIFY_VERBATIM` / `VERIFY_PARAPHRASE` / `VERIFY_SYNTHESIS` / `VERIFY_UNSUPPORTED`.
- **`knowledge-refresh --resweep` opt-in orthogonal flag** (`skills/knowledge-refresh/SKILL.md` §3). Dispatches `cogni-wiki:wiki-claims-resweep` against `binding.wiki_path`. Composable with `--mode push|pull` (runs push/pull then resweep) or standalone (resweep alone, no `--mode` required — the missing-mode prompt is suppressed). Forwards `--resweep-page <slug>` / `--resweep-stale-only` / `--resweep-days <N>` / `--resweep-dry-run` to the upstream skill. **Never auto-runs** — the zero-network invariant on the per-run path is preserved. Pre-flight probes `cogni-wiki:wiki-claims-resweep` and aborts cleanly when absent.
- **Dashboard "Claim verification scope" block** (`skills/knowledge-dashboard/SKILL.md` §2). Renames the existing "Claim verification heatmap" block, prepends a paragraph naming citation-consistency / zero-network / the two unchecked-link corollaries, reads `<wiki_path>/.cogni-wiki/last-resweep.json` to surface `sweep_date` + age-in-days (or `never`), and suggests `/cogni-knowledge:knowledge-refresh --resweep` as the refresh path. `last_resweep` is surfaced wiki-global (scope block + short summary), not as a redundant per-project column — the deposited-projects table already carries `verbatim` / `paraphrase` / `unsupported` columns.

### Changed

- **`skills/knowledge-finalize/SKILL.md` Step 11 final summary.** New lines: `Verification: citation-consistent (zero-network, no live-source re-check; #337)` + `Verbatim/paraphrase ratio: <V>/<P> = <pct>% verbatim` (printed only when `V+P>0`; suffix `(high copy-paste — consider revising for synthesis density)` when `V/(V+P) > 0.5`; `(no scored verdicts)` when `V+P==0`). `## Out of scope` + `## Output` updated.
- **`skills/knowledge-verify/SKILL.md` Step 6 final summary.** New preamble: `Verification scope: citation-consistent vs ingest-time pre_extracted_claims (zero-network; #337). Live-source re-check opt-in via knowledge-refresh --resweep.` Plus the same verbatim/paraphrase ratio line. The existing "Does NOT re-fetch any URL" out-of-scope bullet is strengthened with the explicit `--resweep` cross-reference.
- **`agents/wiki-verifier.md`.** One new §"Writing guidelines" bullet surfaces the verbatim/paraphrase ratio as the operator's confidence signal, naming the downstream **citation-consistent** qualifier. The §"What this agent does NOT do" `WebFetch` bullet is strengthened to cross-reference `knowledge-refresh --resweep` as the structurally-separate, never-auto-run live-source surface. Behaviour-level contract (tools, inputs, output shape) is untouched.
- **`references/inverted-pipeline.md` Phase 6.** New paragraph naming the citation-consistent semantics + the opt-in `wiki-claims-resweep` delegation. New bullet on the "What is still delegated upstream" list for `cogni-wiki:wiki-claims-resweep`.
- **Plugin description** (`plugin.json` + `marketplace.json`). Extended to surface "(citation-consistent — opt-in live-source resweep via knowledge-refresh --resweep)" so the slash-command + marketplace discovery surfaces carry the qualifier. `CLAUDE.md` skill-table rows for `knowledge-refresh` / `knowledge-verify` / `knowledge-dashboard` / `knowledge-finalize` updated.

### Tests

- Extended: `tests/test_finalize_contract.sh` — new SKILL.md content-invariant assertions (frontmatter keys, Step 11 lines, `citation-consistent` / `zero-network` literals, `#337` references, `## Output` / `## Out of scope` updates).
- Extended: `tests/test_verify_contract.sh` — new assertions (Step 6 preamble, `citation-consistent` literal, verbatim/paraphrase ratio, `--resweep` cross-reference, `#337`).
- New: `tests/test_refresh_resweep_contract.sh` — SKILL.md content-invariant assertions (`--resweep` + four `--resweep-*` parameter rows, `### 3. Resweep dispatch` heading, `Skill("cogni-wiki:wiki-claims-resweep"` dispatch, synthesis-extractor underyield deferral, `## When` / `## Never` / `## References` updates, `#337`).
- New: `tests/test_dashboard_contract.sh` — SKILL.md content-invariant assertions (renamed `Claim verification scope` heading, citation_consistent / zero-network literals, `last-resweep.json`, `--resweep` cross-reference, missing-file edge case, `#337`).
- Extended: `tests/test_skill_contracts.sh` — new assertions on `agents/wiki-verifier.md` (`#337`, `knowledge-refresh --resweep`, regression-guard of the "Does NOT WebFetch or WebSearch" invariant).
- Extended: `tests/test_pipeline_summary.sh` — new asserts confirming `verify_counts.verbatim` / `.paraphrase` field names (regression guard against renaming).
- Full `cogni-knowledge/tests/` sweep green; zero behavioural regression. Script-level scripts (`verify-store.py`, `pipeline-summary.py`, `_knowledge_lib.py`, …) are untouched.

### Out of scope (explicit defers, per the #337 issue + ultraplan)

- **Synthesis-page extractor adapter for `wiki-claims-resweep`.** The upstream `extract_page_claims.py` matches sentences containing inline `http(s)://` URLs or `[text](url)` links — `wiki/sources/<slug>.md` pages yield correctly (verbatim source body with inline URLs), but `wiki/syntheses/<slug>.md` pages use `[N]` markers + a `## References` block + bare `[[<slug>]]` backlinks and will underyield. Adapter is v0.1.17+. The minimal `--resweep` skeleton ships in v0.1.16 so the operator-facing surface is wired; the adapter is the next slice.
- **Auto-dispatch of `wiki-claims-resweep` from `knowledge-finalize`** (would re-introduce the WebFetch cost the inverted pipeline structurally fixed), **cadence scheduler**, and **`--resweep-on-finalize` gate** — all opt-in/deferred to v0.1.17+.
- **`excerpt_position` semantic check at ingest.** Stays positional; the semantic gap is structurally addressed by the v0.1.17+ live-source resweep adapter, not a stronger ingest-time check (a separate per-source LLM dispatch — expensive, and the same "validate-the-validator" regress the verifier itself acknowledges).
- **`verify-vN.json` / `citation-manifest.json` / `binding.json` schema changes.** All counts surfaced here are pre-existing fields (`pipeline-summary.py` already extracts `verbatim`/`paraphrase`); new presentation surfaces only. No `last_resweep_at` field added to `binding.json` — `last-resweep.json` (cogni-wiki, locked) is the canonical cadence-pointer; cogni-knowledge reads it, does not shadow it.
- **Resweep-aware verify replay** (warn future `knowledge-compose` runs when a resweep flags a deviation on a cited source page) — v0.1.18+.
- **Thesis claim downgrade.** `references/differentiation-thesis.md` Pillar 2 is #335's scope — byte-for-byte unchanged here.

### Notes

- **No new dependency.** `cogni-wiki:wiki-claims-resweep` is a soft dependency only invoked when the operator passes `--resweep`; the existing `probe_plugin` check covers the absent-plugin case cleanly.
- **No `binding.json` write on resweep** and **no `wiki/log.md` extension** — the upstream `wiki-claims-resweep` writes its own `last-resweep.json` (lock-wrapped) + report under `<wiki_root>/raw/claims-resweep-<date>/`; cogni-knowledge does not duplicate that state.
- **`agents/wiki-verifier.md` behaviour is unchanged** — two prose bullets only, no tool/input/output-shape change.

## 0.1.15 — 2026-05-28

Resolves **#335** Phase 1 — ships approach **(a) observable contradiction tripwire at finalize** from the issue's own smallest → fullest path. `references/differentiation-thesis.md` Pillar 2 (*"Contradictions surface at ingest. When `wiki-ingest` writes page B and page A already says something incompatible, the conflict is visible at file-write time."*) had **zero structural defense** in the v0.1.0 inverted pipeline: `source-ingester` (Phase 4) never compares against existing pages, `wiki-verifier` (Phase 6) only scores cited-page fidelity, and `knowledge-finalize` Step 10.5 ran `lint_wiki.py --fix=all` (5 **deterministic** classes only) + `health.py` without ever invoking the cogni-wiki `wiki-lint` SKILL.md LLM semantic checks (Step 4a–4d, where the contradiction pass lives). A knowledge run could deposit a synthesis that flatly contradicted a cited source — and nothing flagged it. The word *contradiction* appears nowhere in `references/inverted-pipeline.md`; this was thesis-drift, not a documented deferral.

This release closes the observability gap by dispatching a new `wiki-contradictor` agent at `knowledge-finalize` Step 10.6 — pure surface, **no auto-resolution, no rollback, no behaviour change downstream**. **Partially defends Pillar 2 at synthesis-write time**; the thesis's literal "wiki-ingest writes page B" framing is approach **(b)** (per-source ingest-time check), explicitly deferred per the issue's own guidance until Phase 1 produces ≥ 3 confirmed real-fork examples. Mirrors the #340/PR-#347 observability-first tripwire posture. Maturity stays **Preview**.

Phase 1 scope discipline (smallest shippable, this release): `kind ∈ {contradiction, unknown}` only; source-page comparison only (synthesis-vs-prior-syntheses defers — synthesis pages carry no `pre_extracted_claims:` for a cheap structural surface, so body-vs-body scoring is expensive); summary surfaces only `high`-severity findings by name (`medium`/`low` are count-only); `--no-contradictor` CLI opt-out flag as cheap insurance against false-positive flooding. Approaches **(b)** ingest-time check and **(c)** thesis downgrade remain deferred per the issue's own path (*"Ship (a) immediately; (b) after Phase 1 produces real fork examples; (c) only if (a) ships and produces zero usable signal across ≥ 3 real runs"*).

### Added

- **`agents/wiki-contradictor.md` — single-pass, zero-network LLM contradiction scorer.** Reads the just-deposited `<wiki>/syntheses/<slug>.md` body + each cited `<wiki>/sources/<slug>.md` page's `pre_extracted_claims:` frontmatter (via the same stdlib line-by-line YAML idiom `wiki-verifier` uses — no `import yaml`); walks the synthesis body sentence-by-sentence, pre-filtering to assertive sentences (≥ 30 chars + at least one digit / uppercase mid-sentence / date keyword) so non-assertive prose can't structurally contradict a structured claim; emits `<project>/.metadata/contradictor-v<N>.json` (schema `0.1.0`) with `findings[]` carrying `kind ∈ {contradiction, unknown}` and `severity ∈ {high, medium, low}`. Conservative scoring discipline (default `low`/`unknown` on doubt; `high` reserved for outright numeric/named-entity flips with overlapping scope); `unknown` capped at 3 per run with excess collapsed into one rolled-up entry. Tools: `["Read", "Write", "Glob", "Grep"]` — no `Task`, no `Bash`, no `WebFetch` (matches `wiki-verifier`'s posture exactly).
- **`skills/knowledge-finalize/SKILL.md` Step 10.6 (#335).** Dispatches `wiki-contradictor` via `Task` after the Step 10.5 conformance gate's four sub-steps (`lint_wiki.py --fix=all` → `health.py` + orphan re-lint → `overview.md` refresh → `rebuild_context_brief.py`), before Step 11. The Step 5/6 subprocess additionally emits `cited_source_slugs` (filtered list where `page_kind_by_slug[slug] == "source"`) so Step 10.6 reuses the page-kind resolution rather than re-resolving cited pages. Three documented skip paths — `--dry-run` (silent), `--no-contradictor` (logs `Contradiction tripwire skipped: --no-contradictor`), empty citation manifest (logs `Contradiction tripwire skipped: empty citation manifest`). Hard input cap of 30 source slugs (truncated entries surfaced in Step 11 as `⚠ contradiction tripwire truncated at 30/<N>`). **Fail-soft posture (explicit):** a Task failure, schema mismatch, or malformed envelope never rolls back the synthesis — surfaces in Step 11 as `⚠ contradiction tripwire FAILED — synthesis on disk; re-run interactive cogni-wiki:wiki-lint for forensic detail`. Step 11 final summary prints the tripwire line **only when `counts.high > 0 OR counts.unknown > 0`** — clean runs are silent.
- **`skills/knowledge-finalize/SKILL.md` `--no-contradictor` CLI flag.** Opt-out for the Step 10.6 tripwire. Default OFF (tripwire runs). Documented in the `## Parameters` table.
- **`references/inverted-pipeline.md` Phase 7 contract.** Documents the new artifact + agent + schema in the artifacts table and adds a "#335 contradiction tripwire (v0.1.15)" prose paragraph naming the partial Pillar-2 defense at synthesis-write time, the v0.1.16 deferrals (`type_drift`, `undercited_synthesis`, synthesis-vs-synthesis), and the v0.1.17+ approach (b) gating on "≥ 3 confirmed real-fork examples".

### Changed

- **`skills/knowledge-finalize/SKILL.md` `allowed-tools`.** Adds `Task` (required for the new Step 10.6 agent dispatch). The pre-v0.1.15 "no-Task" posture was tied to M9's no-agents contract, which is no longer the shape. Existing `Read`/`Write`/`Bash` retained.
- **`skills/knowledge-finalize/SKILL.md` Step 5/6 subprocess output.** Now additionally emits `cited_source_slugs` and `output_language` on the trailing JSON line — Step 10.6 reads both from the captured subprocess output instead of re-parsing the citation manifest or re-reading `plan.json`.
- **`skills/knowledge-finalize/SKILL.md` Step 11 final summary.** Now carries a conditional contradiction tripwire line (only when `high > 0 OR unknown > 0`), surfacing the top-3 `high`-severity findings by name plus a `Cost: $<usd>` audit so the operator sees per-finalize observability cost.

### Tests

- **New: `tests/test_contradictor_contract.sh`** — 28 content-invariant assertions on the new agent file (frontmatter shape, tools list, Phase-1 kind vocabulary, three severity levels, ≥ 8 "What this agent does NOT do" invariants, schema `0.1.0` literal, `#335` reference, zero-network invariant verbatim, type_drift / undercited_synthesis named as deferred, synthesis-vs-synthesis named as out of scope, conservative-bias + unknown-cap discipline documented, `synthesis_unreadable` + `write_failed` failure envelopes documented, clean-break defence-in-depth).
- **Extended: `tests/test_finalize_contract.sh`** — 12 new SKILL.md assertions for Step 10.6 (heading present, `wiki-contradictor` dispatch, `contradictor-v<N>.json` output artifact, `--no-contradictor` flag in Parameters, `#335` reference, fail-soft framing, three skip conditions, Step 11 tripwire line, Step 5/6 subprocess emits `cited_source_slugs`, partial Pillar-2 framing, References block links the agent) + 3 new `inverted-pipeline.md` assertions (Phase 7 names the agent, the artifact, and `#335`) + the existing `allowed-tools` block updated to assert `Task` is now **present** (replacing the now-stale no-Task posture).
- **Regression sweep:** the full `cogni-knowledge/tests/` directory (26 test files) all green; zero behavioural regression in any of the 25 unchanged files (`_knowledge_lib.py`, `concept-store.py`, `verify-store.py`, etc. untouched).

### Out of scope (explicit defers, per the #335 issue)

- **(b) Ingest-time contradiction check** — Phase 4 `source-ingester` per-source fan-out against the existing wiki. Defer until (a) produces ≥ 3 confirmed real-fork examples to validate the agent prompt against. Same deferral logic as #340 (b) and the parallel #347/0.1.14 release.
- **(c) Thesis downgrade** — only justified if (a) ships and produces zero usable signal across ≥ 3 real runs. Until then, leaving Pillar 2 in place is honest because Phase 1 fulfils it (non-fatally, at synthesis-write time).
- **Synthesis-vs-prior-syntheses comparison** — synthesis pages carry no `pre_extracted_claims:`, so the cheap structural-comparison surface this Phase 1 exploits doesn't exist for synthesis pairs. Body-vs-body LLM scoring against the top-K adjacent syntheses is expensive (token cost ~ N×body-size). Defer to v0.1.16 once Phase 1 produces real-run noise data — there is no point optimizing a layer above a layer we haven't observed yet.
- **`type_drift` / `undercited_synthesis` / `missing_concept` checks** — the other three `wiki-lint` SKILL.md §"4a–4d" check kinds. Defer to v0.1.16 once Phase 1's `contradiction` + `unknown` produce real false-positive volume data so the prompt can be tuned against measured noise.
- **Auto-resolution / rollback.** `wiki-update` remains the human/LLM-judgement reconciliation surface; this release does not extend it. The synthesis deposit at Step 6 is final — Step 10.6 surfaces, never rolls back.
- **`citation-manifest.json` / `verify-vN.json` / `binding.json` schema bumps.** The new `contradictor-vN.json` is a peer file, not a field on existing manifests — same lifecycle posture as `verify-vN.json`. No downstream consumer pins this schema yet.
- **`wiki/log.md` enum extension.** Adding a `contradictor` log prefix would require cogni-wiki log-format enum coordination; deferred to keep this PR cogni-knowledge-only.
- **Re-fetching sources.** Zero-network invariant preserved (`agents/wiki-verifier.md` Phase 6 contract).
- **`differentiation-thesis.md` edits.** Approach (c) deferred; Pillar 2 stays load-bearing.

### Notes

- **Cost (estimated, conservative back-of-envelope).** ~$0.045/finalize on a 25-citation synthesis with Sonnet (~12K input tokens / ~300 output). Surfaced in the Step 11 summary as `Cost: $<usd> (<input_words>w in / <output_words>w out)` so the operator sees per-run cost. If sustained > $0.05/run across real bases, the v0.1.16 follow-up will gate the tripwire behind `--contradictor` opt-in (the inverse of v0.1.15's `--no-contradictor` opt-out). The 30-source input cap bounds the worst case at ~30K input tokens.
- **Bilingual mismatch (#345)** acknowledged. The contradictor operates monolingually in `OUTPUT_LANGUAGE`; a German synthesis cited against German sources is scored in German, and cross-language source pairs (DE↔EN) are correctly emitted as `unknown` or skipped. This is approach (c) territory and explicitly out of scope here — same posture as the #340/PR-#347 tripwire's R9 bilingual acknowledgement.
- **Idempotency.** Re-finalize on the same `draft_version` (e.g. via `--overwrite`) overwrites `contradictor-v<N>.json` (matches `verify-v<N>.json` convention). Same findings re-surface on each re-run — **accepted Phase-1 behavior**, documented in `## Edge cases`. De-dup keyed on `(synthesis_excerpt, conflicting_page, conflicting_claim_id)` defers to v0.1.16.
- **No new cogni-wiki dependency.** The agent reads `wiki/sources/<slug>.md` frontmatter via the existing stdlib idiom; `_wiki_lock` is not needed (read-only against the wiki tree). No cross-plugin coordination required.
- **Step 5/6 `page_kind_by_slug` reuse.** The orchestrator reuses the page-kind resolution the existing Step 5/6 subprocess already computes — Step 10.6 does not re-walk the citation manifest or re-stat `wiki/sources/` vs `wiki/syntheses/`. Marginal change to a 200-line subprocess is two lines.
...
- **`skills/knowledge-distill/SKILL.md` (Phase 4.5, optional + fail-soft).** Builds a claim bundle from `ingest-manifest.json` + each `wiki/sources/<slug>.md`'s `pre_extracted_claims:`, dispatches `concept-distiller`, runs `concept-store.py merge`, then per new/updated slug runs `backlink_audit.py --apply-plan` + `wiki_index_update.py --category Concepts|Entities --max-summary 240` + `config_bump.py --delta <n_created>` (the #302 inserted-only lockstep), and appends a `distill` `wiki/log.md` line. A distill failure never blocks `knowledge-compose`.
- **`agents/concept-distiller.md` (`model: sonnet`, tools `Read`/`Write`).** Pure proposal: clusters the run's source claims into `concept`/`entity` proposals and writes a raw-text concept-records file. Never writes wiki pages, never builds JSON/YAML, never computes slugs, never decides dedup (the #325 + claim-dedup discipline).
- **`scripts/concept-store.py` (`init`/`merge`/`read`).** The locked create-or-merge engine. Derives slugs via `slugify(title)`; under cogni-wiki's `_wiki_lock` (imported from `_wikilib` via `--wiki-scripts-dir` — cogni-knowledge is now a locked writer into the wiki tree) creates-or-merges `wiki/{concepts,entities}/<slug>.md` with **claim-level dedup** keyed on `<source_slug>#<claim_id>` (exact `norm_key`, then symmetric `claim_similarity` ≥ 0.85, **fail-safe keep-both**). Owns all YAML serialization + the on-disk created-vs-updated decision + a pre-write round-trip self-check; preserves a human `## Notes` region byte-for-byte across runs; refuses `foundation: true` and no-sentinel hand-authored pages.

### Changed

- **`scripts/_knowledge_lib.py` — tokenization primitives lifted here as the shared source of truth.** `tokenize` / `token_weight` / `compound_match` / `_fold` / `_stem` / `GENERIC_DENYLIST` / `STOPWORDS` moved from `wiki-coverage.py`; added `norm_key`, the symmetric `claim_similarity`, and `parse_concept_records`. `wiki-coverage.py` imports them back — behavior-preserving (`tests/test_wiki_coverage_bilingual.sh` is the regression guard); `coverage_score` (directional recall) stays in `wiki-coverage.py`, distinct from `claim_similarity` (symmetric).
- **`agents/wiki-composer.md` + `skills/knowledge-compose/SKILL.md` — concept/entity framing-read.** The composer now reads `wiki/concepts/*.md` + `wiki/entities/*.md` as **framing context only** (topic-matched, lazily). They are **never cited** — absent from the citation manifest, not scored by the verifier — so Phases 5/6/7 stay byte-stable whether or not distill ran. The composer cites the underlying source pages a concept backlinks, never the concept page.
- **`scripts/pipeline-summary.py`** — `distill` added to `_PHASE_ORDER`; `project` now loads `distill-manifest.json` and exposes `concepts_created` / `concepts_updated` / `concepts_total` / `claims_attached` / `claims_deduped`. `knowledge-resume` + `knowledge-dashboard` surface the concept counts + the claim-dedup ratio; `knowledge-refresh` push-mode adds the fail-soft distill step to the per-topic chain.
- **Docs.** `references/inverted-pipeline.md` gains the Phase 4.5 contract + the new phase line; `references/differentiation-thesis.md` is corrected to option (a) (the compounding loop + claims-dedup metric now describe the **built** distillation layer — dedup points at `knowledge-distill` via the `claims_deduped_total/claims_attached_total` ratio; the Pillar-2 contradiction line is untouched, #335's scope). `CLAUDE.md` + `references/absorption-roadmap.md` updated. `plugin.json` + `marketplace.json` description + version → **0.1.13**.

### Tests

- `tests/test_concept_store.sh` (new) — init idempotency; create path (frontmatter + sentinels + bare `[[backlinks]]` + round-trip); cross-run update (exact-`norm_key` dedup → backlink union, new claim appended, `updated:` bumped / `created:` unchanged, human `## Notes` preserved, `distilled_from_research` union); **claim-id namespacing** (two sources both `clm-001`, different facts → 2 distinct claims); byte-stable re-run (`unchanged`); foundation + no-sentinel skips.
- `tests/test_distill_contract.sh` (new) — knowledge-distill SKILL + concept-distiller agent contract.
- `tests/test_knowledge_lib.sh` — units for the lifted primitives + `norm_key` + `claim_similarity` + `parse_concept_records`. `tests/test_pipeline_summary.sh` — distill counts. `tests/test_skill_contracts.sh` — clean-break audit extended to the new files. `tests/test_wiki_coverage_bilingual.sh` — green after the primitive lift.

### Notes

- No `binding.json` schema bump — distill writes only `<project>/.metadata/distill-manifest.json` + the concept/entity wiki pages. The `distill` `wiki/log.md` prefix is additive-safe (cogni-wiki readers bucket unknown prefixes; same posture `compose`/`verify`/`finalize` had before cogni-wiki v0.0.45 formalized them — an optional cogni-wiki enum formalization is a non-blocking follow-up). No cogni-wiki code change is required: `type: concept`/`entity` were already in `PAGE_TYPE_DIRS`. The live end-to-end compounding proof (run 2 enriches a run-1 concept page) folds into the next German re-run. Deferred under #264: full LLM re-narration of concept bodies (pairs with #335), entity disambiguation, `summary`/`learning` types, feeding concept pages into `wiki-coverage.py`, composer-cites-concepts, cross-lingual claim merge.
...
- **`_fold` uses NFD, not NFKD.** NFKD *compatibility* decomposition fabricated ASCII digits from typographic glyphs (`½` → `1`/`2`, superscript `²`, fullwidth digits), which `tokenize` then kept as pure-numeric tokens and weighted ×3.0 as article-number anchors — a fraction or footnote marker in a page's claim text would inject a spurious cross-lingual anchor (weight 1.2, clearing the floor) and falsely cover an unrelated `Artikel N` sub-question. NFD's *canonical* decomposition still de-accents every supported-market letter (é→e, ñ→n, Polish ł via the manual map) but leaves `½`/`²`/`№` for the `[a-z0-9]` split to discard.
- **Bilingual test hardened.** The floor and boilerplate-head-guard cases were near-misses that passed on zero overlap (they would not have caught a regression of the mechanism they named). Reworked so each drives its guard branch: a floor case (`Daten erfassen` → `daten`~`datenbank`, weight ~0.625) that flips to `partial` if the floor is dropped, and a head-guard case (`Systemverwaltung`~`systeme`, shared denylisted prefix `system`) that flips to `covered` if the `t_sq[:cpl] not in denylist` guard is removed. Added an NFD regression case (a `½` in claim text must not fabricate an anchor).
