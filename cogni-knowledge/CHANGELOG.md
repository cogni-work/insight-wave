...
- **`source-fetcher` shrank to cobrowse-only.** `WebFetch` + `Read` dropped from its `tools:` (the PDF `Read`-loop moved to the curator); keeps `Bash` + the `claude-in-chrome` MCP tools. Step 1 cache-lookup now short-circuits only on a positive `ok` hit (a prior cobrowse rescue) — a negative entry does not short-circuit, since cobrowse is the explicit retry. Files: `agents/source-fetcher.md`.
