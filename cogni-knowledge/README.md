...
inverted pipeline (knowledge-plan → … → knowledge-finalize) --knowledge-slug X --topic T
  ...
  → knowledge-curate    source-curator per sub-question (WebSearch + score) → candidates.json
  → knowledge-fetch     source-fetcher per batch (WebFetch + cobrowse fallback, shared fetch-cache) → fetch-manifest.json
