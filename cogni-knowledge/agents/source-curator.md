...
| `NORMALIZE_PDF_BODY` | No | Opt-in toggle (truthy = on; default off/absent). When set, the Phase-4 PDF text-layer fallback appends `--normalize-pdf-body` to its `pdf-extract.py` call so the stored body is cleaned (NFKC-fold ligatures, map smart quotes/dashes to ASCII, rejoin hyphenated column-wrap breaks). **Absent / empty ⇒ behave exactly as today** — the flag is not appended, so the stored body / `content_hash` stay byte-identical. Threaded by the orchestrator (`knowledge-curate --normalize-pdf-body`). |
...
1. **Cache-lookup pass first (cheap, no network).** Run Step 1 (the `fetch-cache.py fetch` call below) for *every* survivor up front and partition them: **cache-resolved** (`ok` or `unavailable` hits — fully dispositioned, no fetch needed) vs **fetch-needed** (`miss`/`stale`). Apply the Step-1.5 EUR-Lex ELI/`oj` recover-or-skip to the fetch-needed set here too (a constructable ELI/`oj` URL is rewritten to its `legal-content/TXT` form and WebFetched at Step 2 under the rewritten URL; a non-constructable one goes straight to Step 4 `webfetch_empty_body`, never to WebFetch).
...
**Step 1.5 — EUR-Lex ELI/OJ recover-or-skip (runs only after the Step-1 cache lookup).** Before spending a WebFetch, check whether the URL matches the known-dead EUR-Lex ELI/`oj` landing pattern `eur-lex.europa.eu/eli/.../oj/...` (the JS-rendered HTML form that reliably returns an empty body). If it does, **do not WebFetch the ELI/`oj` URL** — instead attempt a deterministic **CELEX rewrite** to the body-returning `legal-content/<LANG>/TXT/?uri=CELEX:<id>` form, which serves the full normative text where the ELI/`oj` HTML does not:

- **Construct the CELEX id (no network, no LLM)** from the canonical ELI shape `eur-lex.europa.eu/eli/<type>/<year>/<number>/oj[/<lang>]`: `CELEX = 3` + `<year>` + `<type-letter>` + `<number>`, where the type-letter map is `reg → R`, `dir → L`, `dec → D`, and `<number>` is copied **verbatim** (no zero-padding — `eli/reg/2024/1689/oj` → `CELEX:32024R1689`, `eli/reg/2016/679/oj` → `CELEX:32016R679`). Derive `<LANG>` from the in-scope `MARKET`: `dach`/`de` → `DE`, `fr` → `FR`, `it` → `IT`, `pl` → `PL`, `nl` → `NL`, `es` → `ES`, and any other (`us`/`uk`/`eu`/unknown) → `EN` (English TXT is always available). The rewritten URL is `https://eur-lex.europa.eu/legal-content/<LANG>/TXT/?uri=CELEX:<id>`.
- **Rewrite only when confidently constructable** — `<type>` ∈ {`reg`, `dir`, `dec`} **and** `<number>` is purely numeric. Any other shape (the OJ C-series `eli/C/...`, a non-numeric `<number>`, an unknown type token, consolidated/corrigendum variants) is **not** constructable → fall back to the **skip** unchanged: go straight to Step 4 with `reason: webfetch_empty_body`, recording the miss now (write the negative-cache entry via the Step 4 `fetch-cache.py store --status unavailable --reason webfetch_empty_body` invocation — write **now**, do not defer), which keeps the URL `cobrowse_eligible: true` and lets a re-curate within the freshness window short-circuit at Step 1. This conservative gate means the recover path never regresses behaviour — it only adds recovery for the high-confidence cases.
- **On a successful rewrite**, proceed to Step 2 and WebFetch the **rewritten** `legal-content/TXT` URL (never the ELI/`oj` URL). Cache and disposition under the **rewritten** URL, and record provenance `rewritten_from: <original-eli-url>` in the candidate's `fetch` sub-object so the lineage from the discovered ELI URL to the fetched body stays auditable. If the rewritten URL **also** returns an empty body (or any WebFetch miss), fall back to Step 4 for the **original** ELI/`oj` URL exactly as the skip would (`reason: webfetch_empty_body`, `cobrowse_eligible: true`) — a browser fetch can still recover the JS-rendered page on opt-in.

This guard is deliberately placed **after** Step 1: a prior Phase-3 cobrowse rescue (cached `status: ok`) or a prior negative-cache entry is honored by Step 1 first, so this recover-or-skip never clobbers a recovered body — it only fires on a genuine miss/stale. A non-ELI/`oj` EUR-Lex URL (e.g. the `legal-content/<LANG>/TXT/...` form) does **not** match and proceeds to Step 2 normally. Then continue:
...
1. Loop `Read` over the saved PDF in 20-page windows (`pages: "1-20"`, then `"21-40"`, then `"41-60"`, …) until either:
   ...
   **Read-render failure (distinct from end-of-PDF).** If the **first** window returns no usable text because the Read tool reports it **cannot render the PDF in this runtime** (its page→image rasterization is unavailable here) — as opposed to legitimately reaching the end of a short PDF after transcribing real pages — abandon the Read loop, but do **not** give up on the source yet: attempt a **pure-Python text-layer fallback** before recording a terminal outcome. Run:
   ...
   ```
   **When `NORMALIZE_PDF_BODY` is set/truthy**, append `--normalize-pdf-body` to that call so the stored text layer is cleaned (ligatures / smart quotes / column-wrap). Default off/absent ⇒ the flag is **not** appended and the stored body / `content_hash` stay byte-identical to today.
...
Because the stored body is an LLM-mediated WebFetch extract rather than a raw byte fetch, the curator's own dispatch framing (its `sq-NN` sub-question label, "this session"/"this curator") can occasionally bleed into what should be verbatim source text. `fetch-cache.py store` scans every persisted body for these pipeline-internal tokens at the store choke point and flags a match on the cache entry (`contamination_suspected` + `contamination_match`, surfaced on both the store envelope and every later `fetch`) — **fail-soft: the body is still stored** (flag-and-store, never rejected), so a leaked-framing "verbatim" body is surfaced before it feeds claims rather than silently poisoning downstream consumers. No curator action is required; the guard is deterministic and covers every `store` caller.
