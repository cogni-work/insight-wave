...
5. **Capture the dispatch start time** for the Phase-4 `duration_ms` field. Record a monotonic millisecond timestamp at the very start of work, before the Phase-1 cache read and the Phase-2 `claim-extractor` dispatch, so the measured wall clock spans this agent's full lifetime (including the sub-agent):
   ...
   Hold `START_MS` for Phase 4. **Each Bash tool call runs in a fresh shell**, so this value does *not* survive as a shell variable into the Phase-4 call — record the literal integer and re-type it (or set it inline) in the Phase-4 duration compute, and NEVER persist it to a fixed-named file in the shared session scratchpad (a concurrent sibling would clobber it). (This runs even on the skip paths so a skip envelope can still report its `duration_ms`.)

6. **Create the per-dispatch scratch root.** The session scratchpad is **shared across every parallel sibling `source-ingester` dispatch in the same ingest wave** (many ingesters fan out at once). Writing any intermediate artifact to a fixed, generic name there lets a concurrent sibling clobber it mid-dispatch — so this dispatch could read *another* source's body and mis-attribute its claims/provenance. Two forces have to hold at once: the scratch dir must be **unique per dispatch** (sibling isolation) AND its path must be **recomputable in a later, fresh shell** — because **each Bash tool call runs in a fresh shell, so a `WORK_DIR` shell variable does not survive from this call into the Phase-1 / Phase-3 / cleanup calls**. A random `mktemp -d` value satisfies the first but not the second; bridging that random value through a fixed-named file in the shared scratchpad to "carry it across calls" reintroduces exactly the sibling-clobber hazard above. Resolve both by deriving the path **deterministically from the already-validated, per-dispatch-unique `SLUG`**:

   ```bash
   WORK_DIR="${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"
   rm -rf "$WORK_DIR"
   mkdir -p "$WORK_DIR"
   ```

   `SLUG` was validated at step 2 (`^[a-z0-9][a-z0-9-]{0,79}$` — lowercase, dashes, ≤80 chars, no `/` or shell metacharacters) and the orchestrator dedupes slugs pre-fan-out, so distinct sources get distinct `WORK_DIR`s and a sibling can never collide. `WORK_DIR` is the load-bearing isolation boundary for this dispatch: the Phase-1 body file and the Phase-3 composed page (the only two scratch artifacts this agent writes) both live under it. Because the path is a pure function of `SLUG`, **re-type this exact literal expression at the top of every later Bash call that needs it** (Phases 1, 3, and the cleanup) — never persist the value to a bridge file. The leading `rm -rf` also makes a same-`SLUG` retry (e.g. after an `integrity_mismatch` skip) idempotent: it clears any leftover `body.txt` / `page.md` from a prior crashed dispatch so this run starts clean, where a random `mktemp -d` would instead leak a fresh directory each retry. (Deriving from `SLUG` rather than `mktemp` is deliberate: `mktemp -d -t "…${SLUG}…"` is *not* portably deterministic — BSD `mktemp` appends its own random suffix, GNU `mktemp` requires a literal `XXXXXX` — so its path cannot be recomputed in a later call.)
...
Otherwise, take `data.entry.body` as the source body. Write it to the per-dispatch body file `$WORK_DIR/body.txt` (under the Phase-0 scratch root — re-type the deterministic `WORK_DIR="${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"` literal in this call; never a bare `mktemp` value you can't recompute here, and never a *generic* fixed name a sibling could clobber — the `SLUG`-keyed path is safe precisely because it is unique per dispatch) for the `claim-extractor` dispatch — pass a path, not a string, so the extractor stays Read-only.
...
First write the composed page markdown to the per-dispatch scratch file `$WORK_DIR/page.md` (the Phase-0 scratch root — re-type the deterministic `WORK_DIR="${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"` literal in this call; never a bare `mktemp` value you can't recompute, and never a *generic* fixed name — the `SLUG`-keyed path is unique per dispatch, so a sibling dispatch owns its own `$WORK_DIR` and this file cannot collide). Then write atomically via `_knowledge_lib.atomic_write_text` against `<WIKI_ROOT>/wiki/<page-type-dir>/<slug>.md` — with the default `PAGE_TYPE=source` this is `<WIKI_ROOT>/wiki/sources/<slug>.md`; for `PAGE_TYPE=interview` it is `<WIKI_ROOT>/wiki/interviews/<slug>.md` (the `<page-type-dir>` resolved in Phase 0). Pass paths via env vars so apostrophes / spaces in WIKI_ROOT or tmp paths cannot break the Python literal.
...
A skip path still reports `duration_ms` (computed the same way — set `START_MS` inline in that Bash call from the re-typed Phase-0 integer, since a fresh shell holds no prior variable) so a wave where every dispatch skipped still records a meaningful `max_agent_duration_ms` for the phase.
...
- The whole per-dispatch scratch directory (holding the body file and the composed page) is removed at end of dispatch. A `trap … EXIT` cannot span the multi-Bash-call + `claim-extractor` Task dispatch (each Bash call is a fresh shell), so make cleanup an explicit final step: in the last Bash call, recompute the deterministic path and sweep the whole directory — `rm -rf "${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"` — not individual files, so no intermediate artifact leaks. A leftover `$WORK_DIR` is tolerable but unsightly (and the next same-`SLUG` dispatch's step-6 `rm -rf` clears it anyway).
