...
| `CONTRADICTION_INGEST_PATH` | No | Absolute path to the project's ingest-time `contradiction-ingest.json` (the recency-survivor annotations). Set by the orchestrator **only** when that file carries ≥1 recency-resolved contradiction; omitted otherwise. When set, Phase 0 reads it and builds a recency-survivor map so Phase 2 prefers the more-recent survivor claim over a superseded loser claim for the same contested fact. Absent / unset → no surfacing, behaviour byte-identical to before. |
...
3. `Read` `<WIKI_ROOT>/wiki/index.md`. Focus on the `## Sources` category (lists every ingested source with its summary) and the `## Syntheses` category if present — those are the catalogs relevant to composition. Also note the `## Concepts` and `## Entities` categories if present: those are the distilled pages Phase 4.5 (`knowledge-distill`) deposited — **higher-order framing AND citable cross-source evidence** (see step 5). Other categories (`## Decisions`, `## Interviews`, …) can be skipped where they aren't part of this project's evidence.
...
7. **Read the ingest-time recency-survivor annotations (only when `CONTRADICTION_INGEST_PATH` is set).** Skip this step entirely when the parameter is unset — there is then no surfacing and the rest of Phase 0 / Phase 2 is unchanged. When it is set, `Read` `<CONTRADICTION_INGEST_PATH>` (the project's ingest-time `contradiction-ingest.json`) and build an in-memory **recency-survivor map** from its `findings[]`:

   - Consider **only** entries with `kind == "contradiction"` carrying a `resolution` object whose `survivor_claim_id` is non-null (a real recency pick). Ignore `unknown` findings and any `resolution` with `survivor_claim_id: null` (no recency basis to choose).
   - Each such finding names a claim pair: the NEW side `(new_page, new_claim_id)` and the conflicting side `(conflicting_page, conflicting_claim_id)`. The **survivor** is whichever side's `claim_id` equals `resolution.survivor_claim_id`; the **loser** is the other side of the same pair.
   - Add one map entry keyed by the **loser** `(page, claim_id)` → `{survivor_page, survivor_claim_id, rationale}`, where `rationale` is the finding's `resolution.rationale` (a one-line timestamp comparison).
   - This file is **fail-soft** input: if it is missing, unreadable, malformed, or has no qualifying findings, treat the map as empty and proceed exactly as if the parameter were unset. Never fetch — this is a zero-network read of an artifact already on disk.

   The map is consumed in Phase 2 (the survivor-preference rule). It is a *surfacing* aid only: every survivor and loser is an ordinary on-page claim you already cite via the existing `(wiki_slug, claim_id)` mechanism, so it changes only *which* claim of a contradicting pair you cite — never how a citation is recorded, never the citation-record shape, never a wiki page.
8. **Do NOT pre-load every `wiki/sources/<slug>.md`.** A populated knowledge base may have 30+ pages totalling >100K words — pre-loading blows the input budget. Read source pages lazily during Phase 2, scoped per-section.
...
2. Assign a word budget per section. **Branch on `PROSE_DENSITY`:**
   ...
   - **`executive`**: `sum(budgets) ≤ TARGET_WORDS` (no headroom — `TARGET_WORDS` is a ceiling). Prefer fewer, denser sections (4–6 total); every section earns its words. Do not pad to reach the ceiling — under-ceiling is the correct executive outcome. **Reserve an opening `index: "00"` Key Takeaways slot** (`heading: "Key Takeaways"`, budget ~150 words, `covers_sub_questions` = the union of all sub-question ids — it speaks for the whole document) as the first section, ahead of the topical sections; its ~150 words count toward the ceiling like any other section. **Write every topical section's `heading` as the section's conclusion, not its topic** — e.g. `"Adoption stalls on integration cost, not capability"`, never `"Adoption barriers"`. Structural headings (`Key Takeaways`, `References`) keep their plain names. (See `${CLAUDE_PLUGIN_ROOT}/references/concise-structure.md` for the Pyramid + takeaways rubric.)
...
- **Open with a Key Takeaways block.** Before the first topical H2, write a `## Key Takeaways` block of **3–5 BLUF bullets** — the document-level bottom line a busy reader absorbs in 20 seconds. Each bullet carries **exactly one** inline citation marker (the same `<sup>[N](url)</sup>` / `<sup>[N]</sup>` shape and the **one-citation-per-claim** rule above), and is recorded into `citations` exactly like any other cited sentence: append one entry whose `draft_sentence` is the bullet copied **verbatim** (including its `[N]` marker), so Phase 6 verifies it unchanged. Draw each takeaway from a claim already on a cited page — never assert a takeaway you cannot cite. The block counts toward the `TARGET_WORDS` ceiling.
- **Takeaway-style H2 headers.** Every **topical** H2 heading states the section's *conclusion*, not its *topic* — e.g. `## AI Act Mandates a Two-Stage Risk Classification`, not `## AI Act Risk Classification`. The header is the section's BLUF in title form. Structural headings (`## Key Takeaways`, `## References`) are exempt and keep their plain names.

**Prefer the recency survivor (only when a recency-survivor map was built in Phase 0 step 7).** When the map is empty (the common case — no `CONTRADICTION_INGEST_PATH`, or no resolved contradictions), ignore this rule entirely and cite as usual. When the map is non-empty: as you choose which claim to cite for a contested fact, check whether the claim you are about to cite is a **loser** key `(wiki_slug, claim_id)` in the map. If it is, **prefer citing the survivor instead** — cite `(survivor_page, survivor_claim_id)`, drawing the sentence wording from the survivor claim's `text` on its page (the survivor is an ordinary on-page claim, cited via the normal `(wiki_slug, claim_id)` mechanism — no new record field, no new marker shape). The two sides asserted contradicting values for the same fact and the ingest-time tripwire judged the survivor the more-recent source, so the synthesis should state the current value, not the superseded one. You **may** add a brief recency note (e.g. "— the more recent figure supersedes an earlier source") drawn from the map's `rationale`, but keep it to a clause; do not turn the draft into a contradiction log. This is **advisory and conservative**: when the loser is the only claim that grounds a sentence you must write, or when preferring the survivor would distort the section's argument, keep the loser citation — never drop grounding to honour the preference. The survivor preference never changes the citation-record shape, the verifier, or any wiki page; it only changes *which* existing claim you cite.
...
4. **Word-count self-check — branch on `PROSE_DENSITY`, but NEVER loop.** Tally per-section drafted words. Update each `sections[].drafted_words` in the outline file (re-`Write` the outline atomically — Phase 1's path) so the reviewer/verifier have a pre-written audit hook. This is best-effort shaping of ONE pass — there is no re-dispatch loop in either mode (the orchestrator dispatches you exactly once; the advisory `wiki-reviewer` surfaces any gap to the operator at finalize).
   ...
   - **`executive`**: if the total is over `TARGET_WORDS` (the ceiling), trim **redundancy** — restatements, qualifier stacks, "as discussed above" references — not citations or concrete numbers. If you are under the ceiling, stop; do not pad. Under-ceiling is the correct executive outcome. The opening Key Takeaways block (~150 words) is part of the draft body and counts toward the ceiling — include it in the tally.
