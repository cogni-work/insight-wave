---
name: wiki-reviewer
description: Phase-7 zero-network structural quality reviewer for the inverted pipeline. Reads the latest <project>/output/draft-vN.md + .metadata/plan.json (sub-questions, output_language, target_words, prose_density) + .metadata/ingest-manifest.json (source diversity), scores the draft on 5 weighted structural dimensions (Completeness 0.25, Coherence 0.20, Source-Diversity 0.20, Depth 0.20, Clarity 0.15), and emits <project>/.metadata/structural-review-vN.json (schema 0.1.1) with structural_scores, citation_density, word_count, source_diversity, issues[], strengths[], verdict, score. Pure advisory — non-blocking, fail-soft, no auto-fix loop.
model: sonnet
color: yellow
...
| `PLAN_PATH` | Yes | Absolute path to `<PROJECT_PATH>/.metadata/plan.json`. Source of `sub_questions[]` (Completeness audit), `output_language` (Clarity scoring language), and `citation_format` (the citation family the Inline Citation Density Gate counts markers for — no separate parameter carries it). |
...
2. `Read` `PLAN_PATH`. Parse `sub_questions[]` (each carries `id`, `query`, `theme_label`) — this is the completeness reference set: every sub-question's subject should be substantively addressed somewhere in the draft. Capture `output_language` (the `OUTPUT_LANGUAGE` parameter is authoritative; the plan field is the cross-check). Capture `topic` for context. Capture `target_words` (default `2000`) + `prose_density` (default `executive`) for the advisory Word Count Gate — the `TARGET_WORDS` / `PROSE_DENSITY` parameters are authoritative; the plan fields are the cross-check. Capture `citation_format` (default `ieee`) and resolve its **family**: `ieee`/`chicago` → numbered, `apa`/`mla`/`harvard` → author-date, anything else (including the deprecated `wikilink` alias and an absent field) → numbered. This is the plan's own field with no authoritative parameter beside it, and it selects which inline-marker shapes the Inline Citation Density Gate counts — read it here rather than inferring the family from the draft, so a genuinely uncited author-date section is still scored as one.
...
cogni-knowledge's composer (`wiki-composer`) emits inline citations as clickable markers whose shape follows the run's **citation family** — numbered superscripts `<sup>[N](url)</sup>` under `ieee`/`chicago`, parenthesized author-date links under `apa`/`mla`/`harvard` — with `[[sources/<slug>]]` wikilinks confined to the reference list in either family. Scan the draft for H2 section boundaries, **excluding** a trailing references section — match the heading language-aware against `## References` / `## Quellen` / `## Referenzen` / `## Bibliographie` / `## Literaturverzeichnis` / `## Bibliografia` / `## Bibliografía` / `## Bibliografie` (pick whichever matches `OUTPUT_LANGUAGE`). For each remaining H2 section, count:
...
- **Inline citations**: the superscript-wrapped numeric link the composer emits, `<sup>[N](url)</sup>` — regex approximately `<sup>\[\d+\]\([^)]+\)</sup>`. Also count the URL-less fallback `<sup>[N]</sup>` (regex `<sup>\[\d+\]</sup>`) for sources with an empty URL field.
  **Under the author-date citation family also count the author-date marker** — regex approximately `\(\[[^\[\]]+\]\((?:<(?:https?|file)://[^>]+>|(?:https?|file)://[^)]+)\)\)`, the `([Author, Year](url))` / `([Author](url))` / `([Author Year](url))` shapes the composer emits for `apa` / `mla` / `harvard`. The bracketed `(<url>)` alternative comes FIRST and is not optional: the composer is instructed to angle-bracket a paren-bearing destination, so without that arm a section citing such a URL scores as a density deficit purely because of the marker shape. (The numbered pattern above has the same gap; it is left as it is because it is the pre-existing shape `ieee`/`chicago` output is pinned against, so widening it belongs to its own change.) A fully-cited author-date section carries no `[N]` at all, so a numbered-only scan would score it as a citation-density deficit purely because of the marker shape. **Also count the DESTINATION-LESS author-date marker** under this family — regex approximately `\(\[[^\[\]]+\]\)`, the `([Author, Year])` / `([Author])` / `([Author Year])` shapes the composer emits for a source with no external URL (a synthesis, distilled `dcl-NNN` or question-node `acl-NNN` citation). That arm is what the URL-less `<sup>[N]</sup>` count at the previous bullet is for the **numbered** family, and the two are per-family alternatives rather than a shared fallback: an author-date draft carries no `<sup>[N]</sup>` at all, so counting only the linked author-date shape would score a fully-cited URL-less section as a density deficit. Its label class excludes `[` and `]` for the same reason the linked pattern's does, and its shape — `(` immediately followed by `[`, `]` immediately followed by `)`, no destination between — is what keeps ordinary parenthesized prose like `(see Smith, 2020)` and a non-scheme link like `(see [the annex](#section-3))` out of the count. Two properties of that pattern are load-bearing and must not be relaxed: it is **scheme-anchored**, so ordinary parenthesized prose links like `(see [the annex](#section-3))` are not counted as citations; and its label class **excludes `[` and `]`**, so it can never match the `[[N]]` anti-pattern the next paragraph flags.
