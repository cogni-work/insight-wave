...
**Status.** M1–M9 shipped (PR #269, #271, #274, plus v0.0.19 docs-only + v0.0.20 Slice 2 + v0.0.21 PDF Read-loop + v0.0.22 Slice 3 + v0.0.23 Slice 4 + v0.0.24 Slice 5); **M10 shipped — M10a at v0.0.25 (read-side adapters), M10b at v0.0.26 (refresh push-mode rewrite) + cogni-wiki v0.0.45 (log-enum cleanup)**; **M11 shipped at v0.0.27 (legacy `knowledge-research` / `knowledge-report` chain archived to `_archive/`)**. The first full **M12 alpha gate** ran 2026-05-23 and was **HELD** on C3 + C4; the three blockers (F21/F22/F23) were cleared in **v0.0.28 (Slice 10)**, and the fetch-parallelization optimization (#292, Option B) shipped in **v0.0.29 (Slice 11)**, but the gate has **not** been re-run and the maturity flip is deferred to the next landing. Plugin at v0.0.29 / Incubating. M12 pending the gate re-run — see the M-table for status and **Current sprint** below for the next concrete slice. Maturity callout stays at Incubating until the milestone 12 alpha re-run is clean.
...
| 12 | Alpha re-run on `eu-ai-act-v0.1`; bump `plugin.json` + `marketplace.json` to 0.1.0; flip maturity callout to Preview | gated — first gate run 2026-05-23 HELD on C3+C4; blockers F21/F22/F23 cleared in v0.0.28 (Slice 10); fetch-parallelization (#292) shipped in v0.0.29 (Slice 11); **gate re-run + flip is the next landing** |
