...
| 12 | Alpha re-run on `eu-ai-act-v0.1`; bump `plugin.json` + `marketplace.json` to 0.1.0; flip maturity callout to Preview | pending — first gate run 2026-05-23 HELD on C3+C4; blockers F21/F22/F23 cleared in v0.0.28 (Slice 10); **gate re-run + flip is the next landing** |
