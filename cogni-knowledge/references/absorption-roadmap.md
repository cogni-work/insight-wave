...
**Status.** M1–M7 shipped (PR #269, #271, #274, plus v0.0.19 docs-only + v0.0.20 Slice 2 + v0.0.21 PDF Read-loop + v0.0.22 Slice 3); plugin at v0.0.22 / Incubating. M8–M12 pending — see the M-table for status and **Current sprint** below for the next concrete slice. Maturity callout stays at Incubating until milestone 12 (alpha re-run) is clean.
...
*Slice 4 — Current sprint complete (next: M8 — `wiki-verifier` agent + `revisor` fork + `knowledge-verify` skill).* Awaiting its own planning pass. Natural next surface: Phase 6 of the inverted pipeline reads `<project>/.metadata/citation-manifest.json` + each cited `wiki/sources/<slug>.md`'s `pre_extracted_claims:` frontmatter and scores each draft sentence as `verbatim` / `paraphrase` / `unsupported` (zero network calls). `revisor` (forked from cogni-research) closes the loop on `unsupported` findings, capped at 2 iterations per `references/inverted-pipeline.md` Phase 6. This is the structural cost win versus cogni-claims (20–30 min verify → < 5 min).
