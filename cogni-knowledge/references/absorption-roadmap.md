...
| 10 | Rebuild `knowledge-query`, `knowledge-dashboard`, `knowledge-resume`, `knowledge-refresh` on new manifests | **M10a shipped at v0.0.25** (read-side: query/dashboard/resume read inverted-pipeline state via `pipeline-summary.py`; cogni-research probe dropped). M10b pending — `knowledge-refresh --mode push` rewritten on the seven-phase chain (clean-break commitment) + cogni-wiki log-enum cleanup |
...
*Slice 7 — next: M10b (refresh push-mode clean break + cogni-wiki log-enum cleanup).* Two PRs: (1) cogni-wiki v0.0.45 appends `compose`/`verify`/`finalize` to the `wiki/log.md` operation enum in `SCHEMA.md.template` + `CLAUDE.md` (additive, no `schema_version` bump — `queue` precedent; these three prefixes already ship since v0.0.22–24 but were never enumerated; `plan`/`curate`/`fetch` are excluded because they never write `wiki/log.md`); (2) cogni-knowledge v0.0.26 rewrites `knowledge-refresh --mode push` to dispatch the seven-phase chain (`knowledge-plan` → … → `knowledge-finalize`) per stale topic instead of `knowledge-research` + `wiki-refresh`, and drops its cogni-research probe. M10b is the last slice before M11's archive step + M12's alpha re-run.
