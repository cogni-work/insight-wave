...
*Slice 9 — next: M12 (alpha re-run + v0.1.0 bump + maturity flip).* Alpha re-run on a fresh v0.1.0 base (`.alpha/eu-ai-act-gpai/`), then bump `plugin.json` + `marketplace.json` to 0.1.0 and flip the maturity callout from Incubating to Preview. M11's audit-grep is the canary that fails the M12 PR if the legacy chain creeps back.
