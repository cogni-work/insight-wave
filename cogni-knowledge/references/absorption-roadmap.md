...
*Slice 8 — next: M11 (archive `knowledge-research` + `knowledge-report` → `skills/_archive/`).* With the read-side + refresh surfaces fully off cogni-research (M10), the legacy `knowledge-research` / `knowledge-report` skills are the last runtime reachers of cogni-research. M11 moves them to `skills/_archive/`, rewrites README / CLAUDE.md / references to drop the legacy chain, and runs an audit-grep at the boundary to confirm no remaining skill implies the legacy path drives anything. Then M12: alpha re-run on `.alpha/eu-ai-act-v0.1`, bump `plugin.json` + `marketplace.json` to 0.1.0, flip the maturity callout to Preview.
