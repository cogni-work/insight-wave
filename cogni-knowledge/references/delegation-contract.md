...
> **Note (M11+).** The rows describing `cogni-research:*` dispatch and `cogni-wiki:wiki-from-research` Mode A/B were the legacy `knowledge-research` / `knowledge-report` delegation targets, now archived under `_archive/`. The live v0.1.0 inverted pipeline does **not** use `wiki-from-research`; it writes `wiki/sources/*.md` and `wiki/syntheses/*.md` directly (see `references/inverted-pipeline.md`). Its live `Skill`-dispatch delegation surface is `cogni-wiki:wiki-setup` / `wiki-lint` / `wiki-claims-resweep`, plus cogni-wiki helper scripts called at script level (`backlink_audit.py`, `wiki_index_update.py`, `config_bump.py`, `rebuild_context_brief.py`). The read/render skills `knowledge-query`, `knowledge-dashboard`, and `knowledge-resume` no longer dispatch `cogni-wiki:wiki-query` / `wiki-dashboard` / `wiki-resume` — they resolve the wiki engine **vendored-first** under `scripts/vendor/cogni-wiki/` (`wiki-grounding.py`; `render_dashboard.py` + `build_graph.py`; `health.py`) and run with no `cogni-wiki` plugin installed (the install is a graceful-degradation fallback only). Each of these read/render skills (`knowledge-resume`, `knowledge-dashboard`, `knowledge-query`, and the standalone `knowledge-health` / `knowledge-lint`) carries a **verbatim copy** of the `resolve_wiki_scripts()` vendored-first resolver helper — the duplication is intentional (each skill is self-contained and `Bash`-runnable in isolation), and **this note is its canonical reference point**: when the resolver's probe precedence changes, update every copy in lockstep.
