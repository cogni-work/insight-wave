...
| `pdf_render_unavailable` | environmental | A saved PDF file **was** surfaced, but the **Read tool could not render it** in this runtime — its page→image rasterization has no PDF-rendering support here (e.g. the Claude Code build shells out to a local rasterizer that is absent). `fallback_attempted: false`. Operator-actionable, NOT URL-fatal — re-run where the Read tool can render PDFs and the URL resolves, exactly as `cobrowse_unavailable` says "install the MCP and re-run". The missing rasterizer is the **Read tool's own** environment dependency, not anything cogni-knowledge ships — the curator records the honest outcome and never attempts local text extraction (PDFs are read via the Read tool only, no homegrown parser). Cobrowse still cannot render PDF text, so `cobrowse_eligible: false`. Issue #458. |
