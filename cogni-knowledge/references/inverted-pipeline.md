...
Single `wiki-composer` agent. Reads `wiki/index.md` + selected `wiki/sources/*.md` + relevant prior `wiki/syntheses/*.md`, plus (since Phase 4.5) the distilled `wiki/{concepts,entities,summaries,learnings}/*.md` pages as **framing context only** — topic-matched, lazily read, and **never cited** (summary/learning added to the framing read at #342). Distilled pages carry `distilled_claims:` (not `pre_extracted_claims:`), are absent from the citation manifest, and the verifier does not score them, so the composer cites the underlying **source** pages a distilled page backlinks, never the distilled page itself — keeping Phases 5/6/7 byte-stable whether or not distill ran. Drafts the report with `[[wiki-slug]]` citations (not URLs — URLs are looked up via the page's frontmatter when rendering).
