...
{
  "schema_version": "0.1.1",
  "verified": [{"id": "cit-001", "draft_position": "...", "verdict": "verbatim", "wiki_slug": "...", "claim_id": "...", "grounded": true}],
  "deviations": [{"id": "cit-023", "draft_position": "...", "verdict": "unsupported", "reason": "...", "note": "...", "grounded": false}],
...
**Draft↔excerpt grounding rate (grounding L3, verify schema 0.1.0 → 0.1.1, additive).** Each verdict entry carries an additive per-citation `grounded` signal — a deterministic, zero-network draft↔excerpt grounding marker emitted by `wiki-verifier`: `true` when the cited `draft_sentence` grounds in the cited claim's `excerpt_quote` (a normalized contiguous substring or ≥90% token overlap; for a text-only distilled/question claim, derived from the verdict — `verbatim`/`paraphrase` → `true`, `unsupported` → `false`), `false` when it does not, and `null` for a `synthesis` verdict (not scorable). `verify-store.py merge` sums them into the headline `grounding_metrics` block (`grounding_rate = grounded / (grounded + ungrounded)`, or `null` when nothing is scorable — the same None-guard as the ingest-time `excerpt_presence_rate`). It is the verify-phase headline analog of L1's ingest-time excerpt-presence sweep, one phase later: pure observability, fail-soft, never a verify gate. A `0.1.0` reader ignores both the per-citation field and the aggregate block — the citation-manifest schema is unchanged (the grounding signal is derived from claims already on the wiki, not from the manifest).

**#337 verification semantics (v0.1.16).** Phase 6's verdicts are explicitly **citation-consistent**: the verifier scores each draft sentence against the cited page's `pre_extracted_claims:` extracted at *ingest time*, never against the live source URL. This is the structural cost win (< 5 min vs cogni-claims' 20–30 min) — and the explicit qualifier upstream (`knowledge-finalize` synthesis-page frontmatter `verification: citation_consistent_zero_network` + `verification_ratio:`; the dashboard's §"Claim verification scope" block; the finalize Step 11 / verify Step 6 summaries; the plugin description) makes the limitation explicit so a reader does not over-interpret a `verbatim` or `paraphrase` verdict as "the live source still says this." Two links stay structurally unchecked at verify time: extraction fidelity (the `source-ingester` recomputes `excerpt_position` authoritatively at write time via `body.find(excerpt_quote)` — so a present quote is located and never dropped on offset drift, and a genuinely-absent quote is dropped with a warning — but this is a *positional* locate, not a *semantic* check that the quote actually supports the claim text) and live ground-truth drift. For live-source re-verification on a cadence, `/cogni-knowledge:knowledge-refresh --resweep` (opt-in) dispatches `cogni-wiki:wiki-claims-resweep`, which WebFetches each cited URL once and LLM-compares the live source. **Two structurally separate surfaces — preserved by design; resweep is never auto-run from finalize.** The synthesis-page extractor adapter (so the resweep pulls cited URLs out of `[N]`/`[[slug]]`-style synthesis pages), the finalize-gate, and a cadence are deferred to v0.1.17+.
