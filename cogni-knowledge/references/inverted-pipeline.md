...
**#338 (v0.1.19) — open-questions backlog refresh.** Step 10.5 sub-step 5 calls `rebuild_open_questions.py --wiki-root "$WIKI_ROOT"` (via the already-resolved `$WIKI_LINT_SCRIPTS`) so `wiki/open_questions.md` tracks finalize-time state instead of going stale until the next interactive `cogni-wiki:wiki-lint`. Fail-soft (the synthesis is already on disk — a rebuild failure surfaces loudly in Step 11 but never rolls back). Research-time gap seeding from `wiki-coverage.json::uncovered`/`partial` is deferred to follow-up #354 until cogni-wiki accepts a new `class` enum value (`research_uncovered` / `research_partial`) and adds `finalize` to its `CLOSING_OPS`.
