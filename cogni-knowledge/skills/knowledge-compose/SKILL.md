...
The composer reads `wiki/index.md` + selected `wiki/sources/*.md` (lazily, claims under `pre_extracted_claims:`) + prior `wiki/syntheses/*.md`. It also reads two optional `knowledge-distill` surfaces, topic-matched and lazily: the distilled `wiki/{concepts,entities}/*.md` pages (claims under `distilled_claims:`, ids `dcl-NNN`) and the `type: question` nodes at `wiki/questions/*.md` — first-class pages each recording one research question the base has already explored, with `## Findings` `[[links]]` to the sources that answered it (answers under an optional `answer_claims:` block, ids `acl-NNN`).

Both surfaces serve **both** as narrative framing **and** as citable cross-source evidence, under one shared rule: when ≥2 distinct sources converge (≥2 sources agreeing on a fact the distilled page captures; ≥2 distinct sources in a question node's `backlinks[]`), the composer cites the distilled page or question node *directly* via its `dcl-NNN`/`acl-NNN` claim id, so the convergence carries epistemic weight rather than a row of stacked source markers. Below that bar — a single-source answer, or a page/node carrying no claim block yet — the surface stays framing-only: the composer reads it for orientation but cites the backing **source** page (an inline citation to a claim-less node would score `unsupported`). The verifier scores each such citation against that claim's `text`. Both surfaces are optional and fail-soft: with neither present the composer composes from sources + syntheses alone.
...
**No wiki engine required.** cogni-knowledge bundles its wiki engine under `scripts/vendor/cogni-wiki/`, resolved vendored-first by every wiki-touching step, and this skill resolves none of it directly — it reads and writes the wiki through cogni-knowledge's own `wiki-source-manifest.py` and `control-path.py`. There is no `cogni-wiki` plugin install to probe and no hard dependency to abort on (clean-break — no cogni-research, no cogni-claims). The binding read below is the gate, together with the `.cogni-wiki/config.json` and `wiki/` existence checks that follow it.
...
**Select `EXPAND_SECTIONS`** from the just-written outline `<project_path>/.metadata/writer-outline-v<N>.json` — the topical sections (excluding the References section, `covers_sub_questions: []`) that **cover ≥1 sq in `uncited_evidence_sq_ids`**, then density-filtered per the selector documented below. This is conservative by design — the brevity-first intent prefers under-firing (no padding) over over-firing. Quoted literal CLI args again, with `$COVERAGE` the JSON the snippet above printed and `PROSE_DENSITY` the resolved density:
...
`compose-coverage.py expand-sections` reads the outline + the `$COVERAGE` JSON and prints the bare comma-list of qualifying section indices. Its selector branches on `density == "executive"` — **only a section that covers a zero-cited sq qualifies** (one in `zero_cited_sq_ids`), because executive caps *length* and must never pad a section that already carries its citation — vs `standard`, where a section qualifies when it is **thin** (`drafted_words < budget × 0.9`) **or** covers a zero-cited sq. A section whose `covers_sub_questions` doesn't intersect `uncited_evidence_sq_ids` is left alone. **The gate fires iff `EXPAND_SECTIONS` is non-empty.** If it is empty, skip with `expansion skipped: no thin/zero-cited section maps to the uncited evidence` (under `executive`: `expansion skipped: no zero-cited section maps to the uncited evidence`).
...
- Does NOT run an unbounded expansion loop or story arcs — the composer is single-pass per dispatch, and this skill runs at most the ONE bounded, fail-soft, zero-network coverage-gated expansion of Step 5.5 (firing rules there). That expansion re-elaborates existing wiki claims only — it never fetches new evidence (that is `knowledge-curate`/`-fetch`'s job), and never pads toward a word count.
