...
`citation-manifest.json` shape (consumed by M8 `wiki-verifier`):
...
| `--target-words` | No | Soft target word count. Default reads `target_words` from `plan.json` if present, else `5000`. Advisory — no re-dispatch on shortfall in v0.0.22. |
...
This is a **soft target** in v0.0.22 — the composer does not re-dispatch on shortfall. Documented in `references/absorption-roadmap.md` Slice 3 notes; the expansion-loop port from upstream is deferred to a follow-up if M12 alpha demands it.
...
Note on the `compose` prefix: cogni-wiki's log-format enum (per `cogni-wiki/CLAUDE.md` §"Key Conventions") does not yet list `compose`, but the same paragraph notes that "pre-v0.0.35 readers count unknown prefixes in their catch-all bucket without crashing" — `compose` is additive and safe. Formalising the prefix lands in Slice 4 / M10 when query/dashboard get rebuilt on the new manifests anyway.
...
If the composer returned a word-count well below `TARGET_WORDS`, surface a `⚠ Below target (N/TARGET)` warning line — but do not auto-retry. The expansion-loop port from upstream is deferred (see Slice 3 deferral list in `references/absorption-roadmap.md`).

## Edge cases

- **F11 recovery in action.** The outline file exists from a prior crashed run. The composer skips Phase 1 (saves model time and avoids re-deriving the section plan), runs Phase 2 fresh, and writes the draft + citation manifest. The outline's `drafted_words` placeholders get filled by the resume pass. Surface "RESUME_FROM_OUTLINE=true (F11 recovery)" in the summary so the operator sees what happened.
...
- **Citation manifest empty.** If the composer returns `ok: true` but `citations[] == 0` (every page had zero `pre_extracted_claims:` — unusual but possible if M5's claim-extractor failed across the board), surface as `⚠ Zero citations — every cited statement will fail M8 verification`. Do not block — that's an upstream-data issue, not a composer bug.
...
- Does NOT verify citations — Phase 6 (`knowledge-verify`, M8).
- Does NOT deposit the draft into the wiki as `wiki/syntheses/<slug>.md` — Phase 7 (`knowledge-finalize`, M9).
- Does NOT modify `binding.json` — Phase 7 appends the project entry.
- Does NOT re-run any earlier phase.
- Does NOT support multilingual output, executive density, story arcs, or expansion loops in v0.0.22 — deferred per `references/absorption-roadmap.md` Slice 3 notes.
