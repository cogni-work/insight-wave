...
progressive disclosure — **run it exactly as written** in
[`references/finalize-compose-subprocess.md`](../../references/finalize-compose-subprocess.md)
§"Subprocess (run verbatim)". Set these env inputs on the invocation line:

- `KNOWLEDGE_SCRIPTS="${CLAUDE_PLUGIN_ROOT}/scripts"` — prepended to `sys.path`
- `WIKI_ROOT` / `PROJECT_PATH` / `PROJECT_SLUG` / `SYNTHESIS_SLUG` — resolved wiki root, project path, project + synthesis slugs
- `DRAFT_VERSION=<N>` / `REVISION_ROUND=<round>`
- `VERIFY_VERBATIM` / `VERIFY_PARAPHRASE` / `VERIFY_SYNTHESIS` / `VERIFY_UNSUPPORTED` — the Step 2 verify counts (written into the synthesis `verification_ratio:` frontmatter)
- `QUESTION_SLUGS_CSV="$QUESTION_SLUGS_CSV"` — the Step 4.7 forward-link slugs (empty when none)
...
**Gate-wide fail-soft posture.** Every sub-step below is a post-deposit refresh, so nothing here ever rolls back the synthesis: the synthesis page, index entry, `entries_count` bump, `binding.json` append, and `wiki/log.md` line are already on disk when the gate opens, as is every sub-step preceding the one that failed. **Atomicity is universal, zero exceptions** — every write is a tempfile + `os.replace`, so a forced failure never leaves a partial page. **Locking has one exception** — sub-step 4's `rebuild_context_brief.py` is atomic but unlocked, which is why it runs last and is non-fatal. **Byte-diff skipping is per-caller idempotence, not gate-wide** — the write primitives never compare, so a site rewrites unless its own caller diffs first; read the sub-step, not the gate. Neither guarantee above rests on it. These scripts return an error envelope rather than raising (missing wiki-scripts dir, `_wikilib` import failure, non-wiki `--wiki-root`), so treat a non-zero result as a warning surfaced loudly in Step 11 — never an abort, never a rollback. Each sub-step below restates only what is specific to it.
...
**Derive the synthesis-side consistency rate (fail-soft).** After the contradictor returns `ok: true` (and the `contradictor-v<N>.json` is on disk), run `contradiction-finalize-store.py record` to compute the **consistency rate** — the synthesis-side scoreboard the contradiction track named as its proof-of-value (the finalize-time analog of the ingest-time `resolution_coverage`):

```
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/contradiction-finalize-store.py" record \
    --project-path "$PROJECT_PATH" \
    --out "$PROJECT_PATH/.metadata/contradiction-finalize.json" \
    --output-language "$OUTPUT_LANGUAGE"
```

The store reads the project's latest `contradictor-v<N>.json`, computes `consistency_rate {syntheses_total, syntheses_clean, pct}` (a synthesis is **clean** when it carries zero *unresolved high-severity* contradiction — a `high`-severity `contradiction` finding whose `resolution.survivor_claim_id` is absent/null) plus the same `resolution_coverage` block the ingest store reports, and atomic-writes `contradiction-finalize.json`. Capture `data.consistency_rate` + `data.resolution_coverage` for Step 11. **Fail-soft, observability-only** — a missing / unreadable contradictor file degrades to a zeroed artifact with `data.skipped_reason` (never blocks finalize; the synthesis already landed). Skip the call entirely when the contradictor returned anything other than `ok: true` (no contradictor output to read) and when `--no-contradictor` was passed.

### 10.7 Structural-quality review

The structural-quality half of the cogni-research feature-parity gate. Step 10.6 (and Phase 6) check **citation-claim alignment** — does each cited sentence match the cited page's claims. This step checks **structural quality** — does the draft address every sub-question, flow coherently, draw on diverse publishers, go deep, and read cleanly in its output language. A synthesis can pass verify (every citation aligned) and still fail structural review (a sub-question treated in one shallow paragraph). Dispatches the `wiki-reviewer` agent to score the draft on the same 5 weighted dimensions cogni-research's reviewer scores (Completeness 0.25, Coherence 0.20, Source-Diversity 0.20, Depth 0.20, Clarity 0.15, with an inline citation-density gate that caps Depth and an advisory Word Count Gate that caps Completeness on a `standard`-density deficit / `executive`-density excess) and emit `<project_path>/.metadata/structural-review-v<N>.json` (schema `0.1.1`).
...
**Resolve `OUTPUT_LANGUAGE` + the Word-Count-gate inputs (self-contained, independent of Step 10.6).** Read them directly from `plan.json` so this step does not depend on whether the contradictor ran. `TARGET_WORDS` + `PROSE_DENSITY` feed the reviewer's advisory Word Count Gate:
...
- Contradiction tripwire (Step 10.6): print this block **only on `ok: true` AND (`counts.high > 0` OR `counts.unknown > 0`)** — clean successful runs are silent (no false-alarm noise). On `ok: false` use the FAILED branch below; on skip use the skip-message branch below. Each branch is its own independent surface — gating is per-branch, not joint:
  ...
  ```
- Consistency rate (Step 10.6 finalize-store): print this single line on `ok: true` whenever `contradiction-finalize.json` was written (i.e. the contradictor ran) — **including a clean 100% run**, since the rate is the proof-of-value signal the track exists to surface, not a warning. Skip it only when the contradictor was skipped/failed (no `consistency_rate` to read):
  ```
  Consistency rate: <pct>% — this synthesis has <"no" if syntheses_clean else N> unresolved high-severity contradiction(s); recency coverage <resolution_coverage.resolved>/<resolution_coverage.contradictions>. — observability-only
  ```
  `<pct>` is `consistency_rate.pct` (100.0 when the synthesis is clean, 0.0 when it carries ≥1 unresolved high-severity contradiction — one synthesis per finalize run, so the rate is binary this run; it aggregates across syntheses as the base grows). `resolution_coverage` is the same recency-survivor share the ingest-time store reports (`resolved`/`contradictions`). Both come from the `contradiction-finalize.json` written at Step 10.6; never block on a missing file (the line is simply omitted).
...
- Assembles the reference list per **citation family**, keyed off `plan.json::citation_format`. Numbered (`ieee` — `**[N]** Publisher, "Title". [URL](URL) — [[<slug>]]`; `chicago` — `**[N]** Publisher. "Title." …`, period-separated) lists in first-appearance order matching the composer's inline `[N]`, and runs the renumber pass over the body. The **author-date** family (`apa`/`mla`/`harvard`) assembles an alphabetical, un-numbered list — one entry per distinct source in that format's own bibliography shape, sorted by author surname — and **bypasses the numbered renumber pass**, which has nothing to line the body up with once the list carries no numbers. The reference backlink is a **bare** `[[<slug>]]` (not path-prefixed `[[sources/<slug>]]`) in either family, so the synthesis→source edge registers in cogni-wiki's link graph.
...
- `<WIKI_ROOT>/wiki/overview.md` — refreshed with a `## Recent syntheses` bullet for this synthesis (Step 10.5 sub-step 3, `overview_update.py recent-bullet`). This is now overview.md's **only** role — the curated-root layout moved the `MACHINE-OWNED:OVERVIEW-NARRATIVE` narrative to the `wiki/index.md` intro (spliced on `--apply-portal`, see the `wiki/index.md` row above).
...
- `<WIKI_ROOT>/wiki/index.md` — on `--apply-portal`, each grown `## <theme>` section gains/refreshes an engine-owned `MACHINE-OWNED:PORTAL-LEADIN` lead-in span (carried into the curated MAP above the section's count-link line by sub-step 3.5.1), and the `MACHINE-OWNED:OVERVIEW-NARRATIVE` intro block is spliced (Step 10.5 sub-step 3.5). Human (non-sentineled) lead-ins are never touched.
...
- `<project_path>/.metadata/contradiction-finalize.json` — Step 10.6 synthesis-side consistency scoreboard (schema `0.1.0`), written by `contradiction-finalize-store.py record` from the project's latest `contradictor-v<N>.json`. Carries `consistency_rate {syntheses_total, syntheses_clean, pct}` (clean = zero unresolved high-severity contradiction) + the same `resolution_coverage {resolved, contradictions, pct}` the ingest-time store reports + a per-synthesis `syntheses[]` detail. Written whenever the contradictor returned `ok: true` (degrades to a zeroed artifact with `skipped_reason` on a missing/unreadable contradictor file); absent on the contradictor skip/`ok: false` paths. Read by `pipeline-summary.py project` and surfaced in the Step 11 consistency-rate line.
- `<project_path>/.metadata/structural-review-v<N>.json` — Step 10.7 structural-quality verdict (schema `0.1.1`): per-dimension `structural_scores`, `citation_density`, `word_count` (the advisory Word Count Gate), `source_diversity`, `issues[]`, `strengths[]`, `verdict`, `score`. Written when the reviewer returns `ok: true`; absent on skip paths (`--dry-run`, `--no-reviewer`) and on `ok: false` failure paths.
