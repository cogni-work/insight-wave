...
2. Dispatch via the `Task` tool (matches the upstream `knowledge-curate` / `knowledge-fetch` agent-dispatch convention):
   ...
   Resolve `THEME_LABEL` the **same way Step 4.2 resolves `--category`**: take this source's first-listed sub-question ref `sub_question_refs[0]` and look it up in the Step 0 `theme_label` map; pass the resolved label (omit / pass empty on a legacy plan with no `theme_label`). The ingester writes it into the page's `theme_label:` frontmatter — the authoritative, frontmatter-resident membership signal `sub_index.py` reads to group the source under its theme (so a curated root index no longer needs per-page bullets to carry membership). Passing it here keeps the page's own frontmatter membership consistent with the `## <theme_label>` heading Step 4.2 files its index bullet under.
