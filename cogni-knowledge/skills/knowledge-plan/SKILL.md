...
| `--citation-format` | No | All five render as requested; the split is a marker family, not a support level — `ieee`/`chicago` are numbered, `apa`/`mla`/`harvard` author-date (see `${CLAUDE_PLUGIN_ROOT}/references/citation-formats.md`). Resolved in Step 0.5: flag > binding > framing suggestion > `ieee`. `wikilink` aliases to `ieee`. |
...
**Inherit the base charter as grounding (schema 0.1.4).** Step 0 already read the binding. Before the *ground* move asks for additional user-supplied material, read the base charter — `binding.get("charter", {}).get("domain"/"audience"/"scope", "")` — and, when `charter.domain` is non-empty, **inject it as inherited grounding context** into the framing: the base's domain/audience/scope anchor the sharpening turn's audience/thesis/scope options (a per-question framing under a charter that says "compliance officers, EU, 2024-2027" defaults its audience/scope to the charter rather than asking cold). State it in one line: *"Framing under this base's charter: <domain> · for <audience> · scope <scope>."* The user can still override every option. This is the base-level → per-question handoff documented in `${CLAUDE_PLUGIN_ROOT}/references/charter-framing.md`. **Fail-soft:** a missing/empty charter (a pre-0.1.4 base, where the `.get` chain returns `""`) → today's behaviour exactly, no charter line, ask for grounding from scratch.
...
- **`prose_density`**: `--prose-density` > `research_defaults.prose_density` > framing > `executive`. Validate against `{standard, executive}`; an invalid value falls back to `executive` with a one-line warning. (Resolve via `_knowledge_lib.normalize_prose_density` if you shell out; otherwise apply the same rule inline.)
...
- **`citation_format`**: `--citation-format` > `research_defaults.citation_format` > framing > `ieee`. `wikilink` aliases to `ieee` (`_knowledge_lib.normalize_citation_format`); unknown → `ieee`. All five render as requested: `ieee`/`chicago` as numbered superscripts, `apa`/`mla`/`harvard` as author-date markers with an alphabetical, un-numbered reference list.
- **`target_words`**: `--target-words` > `research_defaults.target_words` > framing > `2000`. Positive int (`_knowledge_lib.normalize_target_words`); non-positive/unparseable → `2000`.
...
```json
{
  ...
  "target_words": <resolved positive int, e.g. 2000>,
...
- **Binding has `research_defaults` but no writer-quality knobs** (pre-0.1.2 base, schema 0.1.1). Step 0.5's per-knob `.get(knob, DEFAULT)` falls straight through to the hard defaults (`executive` / `objective` / `ieee` / `2000`) — no error. The base keeps its persisted `market`/`output_language`.
