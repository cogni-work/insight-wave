...
| `--pause-before <phase>` | No | **Cost gate.** Halt before the named phase for an `AskUserQuestion` confirm; abort is a clean exit 0 that re-invocation resumes from. Validated against `{plan, curate, fetch, ingest, distill, compose, verify, finalize}`. **Default (unflagged) = one gating-policy menu at the first cost gate** — run unattended through finalize / pause again before the heavy `ingest` fan-out / abort — so the operator places the gate by *answering*, not by recalling a flag. **Explicit `--pause-before <phase>` = bare proceed/abort at that phase** (the scripted override, menu skipped); `none` / `--no-pause` disables the pause entirely. A phase already complete on a resume entry (in §0.5's `skip_set`) is never paused, and a resume **past** the default gate moves the single default gate forward to the next unpaid heavy-spend gate — `curate`→`ingest` (§0.5 step 4). Branch mechanics: §2. |
| `--no-pause` | No | Alias for `--pause-before none` — disable the default pre-`curate` pause (and the policy menu) and run fully unattended (pair with `--until finalize` for an explicit "run everything" invocation). |
...
2. **Resolve the cost-gating flags** against the canonical phase set `PHASES = {plan, curate, fetch, ingest, distill, compose, verify, finalize}` (the `_PHASE_ORDER` of §0.5 minus `none`):
   - `pause_before` = the `--pause-before` value if supplied, else `curate` (the safe-by-default gate). `--pause-before none` or `--no-pause` clears it (`pause_before = None`). An explicit `--pause-before <phase>` **overrides** the `curate` default — only the named phase is gated. On a **resume** entry this default may be advanced off an already-completed phase so the gate survives the skip — see §0.5 step 4's re-point rule.
   - `pause_is_default` = `True` iff **neither** `--pause-before` **nor** `--no-pause` was supplied (a bare run riding the `curate` default); `False` the moment either flag is set. This single bit is what makes the default `curate` gate render the §2 interactive **gating-policy menu** while every explicit-flag / autonomous invocation keeps the bare proceed/abort (or no gate) — so the scripted contract (`--no-pause` = fully unattended) stays byte-identical.
...
4. **Re-point the default gate off an already-paid phase.** When `pause_is_default` is `True` AND the resolved `pause_before` (§0 step 2) is in `skip_set` — its cost was already paid on a prior run, so the §2 pause-before guard (which requires `phase == pause_before AND phase ∉ skip_set`) would never fire and the default cost gate would be silently lost on resume — advance `pause_before` to the **next gate-eligible heavy-spend phase not in `skip_set`**. The default gate only ever lands on the two heavy-spend phases the policy menu is built around — `GATE_PHASES = {curate, ingest}` (the dominant web-research and ingest-fan-out spends); the lighter intervening phases (`fetch`, a near-no-op top-up since bodies are already fetched during `curate`; `distill`/`compose`/`verify`) are never the default gate, so the re-point steps over them: `curate` skipped ⇒ `pause_before = "ingest"`. If **both** `GATE_PHASES` are in `skip_set` (`ingest` already ran too), clear it (`pause_before = None`) — the dominant spend is behind us, so the run finishes unattended. This is what makes the §2 policy-decision sub-branch live at `ingest` on a resume past `curate` — without the re-point that sub-branch is unreachable. An **explicit-flag** run (`pause_is_default == False`) is **never** re-pointed: the operator named the phase deliberately, and a named phase already in `skip_set` stays correctly un-prompted.
...
1. **Pause-before guard (pre-dispatch).** If the phase equals `pause_before` AND it is **not** in `skip_set` (a `skip_set` phase already ran — its cost was paid, so never re-prompt), prompt before dispatching. The prompt has **two shapes**, selected by `pause_is_default`:

   - **Interactive gating-policy menu** — when `pause_is_default` is `True` AND this is the **policy-decision point**: `phase == "curate"`, OR (`phase == "ingest"` AND `"curate" ∈ skip_set` — the resume-past-curate case where the curate gate never fired). The `ingest` arm is reachable **only because §0.5 step 4 re-pointed `pause_before` from `curate` to `ingest`** when `curate ∈ skip_set` (without that re-point the outer guard `phase == pause_before` never matches at `ingest` and this arm is dead code — keep the two sites consistent). `AskUserQuestion` (single-select) lets the operator place the gate by answering, no flag required:
     - **"Run unattended through finalize (no more prompts)"** → set `pause_before = None` (clears every later gate), dispatch this phase, and continue with no further prompts (the answered equivalent of `--no-pause`).
     - **"Pause again before the heavy ingest fan-out (~50 agents)"** → set `pause_before = "ingest"` (re-point the single gate), dispatch this phase; the `ingest` gate then fires later as the **plain proceed/abort** shape below (the menu does not re-show — it is gated on the policy-decision point, and `ingest` after this choice is not `curate`). **Omit this choice when the menu itself is firing at `ingest`** (the resume-past-curate case) — there it collapses to proceed / abort, since the gate is already at ingest.
     - **"Abort now"** → print `aborted before <phase> — resume with --project-path <project_path>` and **exit 0** (the on-disk artifacts are a valid checkpoint — the safe-by-default property: a bare run still confirms before `curate`'s first spend).
   - **Bare proceed/abort** — otherwise (an explicit `--pause-before <phase>` was set, or this is the `ingest` gate after the operator chose "pause again"): `AskUserQuestion` (single-select: **proceed** / **abort**). On **abort**, print `aborted before <phase> — resume with --project-path <project_path>` and **exit 0**. On **proceed**, dispatch normally.

   The default `pause_is_default` run pauses exactly once at the policy-decision point (then honours the chosen policy); an explicit-flag run pauses once at the named phase. A `--cobrowse` run is **not** double-prompted by this gate — the cobrowse opt-in lives inside `knowledge-fetch` (a later phase), so the cost gate is the only added prompt.
...
On the fresh-topic `--topic` path with no gating flags (`pause_is_default == True`), `until` is `None` (run through `finalize`) and the single `curate` gate renders the interactive gating-policy menu — one answer places the rest of the run (unattended, or a second pause before `ingest`). `--pause-before none` / `--no-pause` clears the gate (and the menu) so the run is fully unattended; an explicit `--pause-before <phase>` gives the bare proceed/abort at that phase.
...
# Cost gate per not-skipped phase: pause-before guard fires when phase == pause_before (∉ skip_set).
# Two prompt shapes: a default (pause_is_default) run renders the 3-option gating-policy menu at the
# policy-decision point (curate, or ingest on a resume past curate — §0.5 step 4 re-points the gate there);
# explicit-flag / --no-pause runs render bare proceed/abort (or no gate). Abort = clean exit 0.
# Until stop (print partial summary, exit 0) fires after phase == until.
