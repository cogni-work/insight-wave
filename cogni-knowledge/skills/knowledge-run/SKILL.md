...
| `--pause-before <phase>` | No | **Cost gate.** Halt before dispatching the named phase and ask the operator to confirm (`AskUserQuestion`, proceed/abort) — abort is a clean exit 0 that re-invocation resumes from. Validated against the canonical phase set `{plan, curate, fetch, ingest, distill, compose, verify, finalize}`. **Default is `curate`** (the first web-cost phase), so a bare `knowledge-run --topic "…"` confirms the web spend once, then runs unattended. **On a default (unflagged) run the first cost gate is a one-shot *gating-policy* question** — run unattended through finalize / pause again before the heavy `ingest` fan-out / abort — so the operator places the gate by *answering*, never by recalling a flag (§2). **Passing this flag explicitly restores the bare proceed/abort at the named phase and skips the policy menu** — the scripted override: gate a later cost-bearing phase (`curate`, `ingest`, `compose`) directly, or pass `none` (or `--no-pause`) to disable the pause entirely. A phase already complete on a resume entry (in §0.5's `skip_set`) is never paused — its cost was already paid. |
