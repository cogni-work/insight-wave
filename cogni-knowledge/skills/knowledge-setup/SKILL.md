...
| `--citation-format` | No | Default citation format persisted to `binding.json::research_defaults.citation_format` (`ieee`/`chicago` numbered; `apa`/`mla`/`harvard` author-date — all five render as requested). **Flag-or-default** — not prompted (safe default `ieee`). |
