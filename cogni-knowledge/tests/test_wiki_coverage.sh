...
OUT=$(run_score "$WIKI" "$WORK/plan.json")
...
# --- Case 4: threshold boundary -------------------------------------------
# A very high threshold should drop every page back to uncovered.
OUT=$(run_score "$WIKI" "$WORK/plan.json" --threshold 0.99)
...
# A very low threshold should let at least one page through (partial or covered).
OUT=$(run_score "$WIKI" "$WORK/plan.json" --threshold 0.01)
...
if python3 "$SCRIPT" score --wiki-root "$WIKI" --plan "$WORK/plan.json" --threshold 1.5 >/dev/null 2>&1; then
  ...
  green "PASS: out-of-range --threshold is rejected"
fi
