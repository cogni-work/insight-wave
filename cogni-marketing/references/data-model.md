...
- **cogni-publishing**: Long-form content can be piped to `text-to-narrative` for arc-driven transformation and a design brief; `copywriter` provides optional polish.
- **cogni-workspace**: Evidence claims in content inherit verification status from the TIPS claims registry via `cogni-workspace:claims`.
