...
3. Resolve the market's locale via the workspace merge utility — run `python3 "${WORKSPACE_PLUGIN_ROOT:-$(ls -td "$HOME"/.claude/plugins/cache/insight-wave/cogni-workspace/*/ | head -1)}/scripts/get-market-config.py" --plugin portfolio --market <region-code>` — it returns a flat config including `locale` (e.g., `dach` → `de_DE`).
