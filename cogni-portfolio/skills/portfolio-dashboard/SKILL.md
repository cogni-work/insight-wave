...
0. **Sticky Navigation** — Pill-style nav bar with section links, active state tracking via scroll detection, backdrop blur. Click any link to smooth-scroll to that section. Nav order follows the portfolio workflow: Overview → [Sources] → Products → [Provider units] → Markets → Matrix → Taxonomy → [Anchors] → Customers → [Context] → Solutions → [Competitive] → Packages → Margins → [Pipeline] → [Communicate] → Claims → Actions.
...
10a. **Competitive Landscape** — (shown when at least one `competitors/*.json` file contains a non-empty `competitors[]` array) Portfolio-level competitor view, deduplicated by competitor name across all propositions. Each competitor card shows name with a "N propositions" count badge, the first observed positioning and differentiation statements, clickable proposition slug chips that open the per-proposition drawer (`openProposition`), strength/weakness pills (deduped union across appearances), and source URL links. Sorted by appearance count (desc), then by name. Hidden when no competitor data exists.
