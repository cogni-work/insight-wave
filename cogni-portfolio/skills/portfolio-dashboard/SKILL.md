...
- **If picking new** (or no design-variables exist): use the `cogni-publishing:manage-themes` skill (Operation 11, Select Theme) to let the user select a theme. It returns `theme_path`, `theme_name`, and `theme_slug`.
...
0. **Sticky Navigation** — Pill-style nav bar with section links, active state tracking via scroll detection, backdrop blur. Click any link to smooth-scroll to that section. Nav order follows the portfolio workflow: Overview → [Sources] → Products → [Provider units] → Markets → Matrix → [Competitive] → [Taxonomy] → [Anchors] → Customers → [Context] → Solutions → Packages → Margins → [Pipeline] → [Communicate] → Claims → Actions. Competitive sits directly after Matrix because competitor data is per-proposition, i.e. per F×M cell — the section reads as competitive pressure on the matrix above.
...
3a. **Source Freshness** — (shown when `source-registry.json` exists with at least one source entry) Four stat cards summarizing total sources (docs + URLs), current (with % bar), stale/superseded (with hover-tooltip listing the affected `source_id`s), and unreachable. Footer line shows the registry's `updated` date, total entities tracked, and the most-cited source (the one with the longest `entities[]`). Hidden when the registry is absent or empty.
...
4a. **Provider units** — renders from either of two sources (scan-output takes precedence when present). Fully absent when neither source yields a provider unit.
   - **Source 1 — `scan-output` (diagnostic)**: shown when `research/.metadata/scan-output.json` v1.2.0+ exists with a non-empty `provider_units` array (portfolio was built via `portfolio-scan`). Section header reads "Provider units (scan diagnostic)". Diagnostic only — authoritative feature-to-unit mapping lives in each feature's `source_lineage`.
      - **Consolidation-mode badge** — rendered beside the header from `scan-output.json.consolidation_mode` (introduced in schema v1.3.0): green `consolidated` (mode=`consolidate`), yellow `shadow review` (mode=`shadow`), neutral `research only` (mode=`research-only`). When `consolidation_mode` is absent (pre-v1.3.0 scans) the mode defaults to `consolidate`. Footer shows scan date, `scan-output.json v1.2.0+`, and `mode: <consolidation_mode>` (field added in v1.3.0+).
      - **Shadow candidates block** — when the mode is `shadow` and `research/scan-candidates/{company_slug}/` contains candidate JSONs, a block renders below the unit cards with a total count plus a compact per-company slug list so reviewers can locate candidate files on disk before promoting them into `features/`.
   - **Source 2 — `features` (authoritative)**: fallback when `scan-output.json` is absent but individual features carry a `provider_unit: {code, name, country, tier}` block (e.g., portfolios built via `portfolio-web-researcher`). Section header reads "Provider units (from feature metadata)" — no mode badge is shown here because feature-derived data is always already-imported. Each card gains a `country · tier` subline not available from scan-output. Footer shows `derived from N features` instead of scan metadata. Aggregated on the fly from per-feature `provider_unit.code`.
...
6. **Feature x Market Matrix** — Interactive grid. Each cell is color-coded (green = proposition + solution, yellow = proposition only, red = missing). When a proposition has variants, display a variant count badge on the cell (e.g., "3v" pill). When a proposition has a `quality_assessment`, display a quality badge on the cell: green dot for `"pass"`, yellow dot for `"warn"`, red dot for `"fail"`. Cells with at least one analyzed competitor also show a small numeric corner badge (count of competitors for that F×M pair); the above-the-fold coverage heatmap mirrors the signal with a corner dot in compressed mode and the same numeric badge in inline mode. Cell tooltips append `· N competitor(s)` when present. Click a cell to expand IS/DOES/MEANS, pricing tiers (type-aware: project/subscription/partnership), unit economics, and competitor summary. The drill-down panel includes a "Quality Assessment" section (when present) showing the overall score and per-dimension pass/warn/fail for DOES and MEANS, with `assessed_at` date. Below that, a collapsible "Variants" section lists all variants with their angle label, DOES/MEANS summary, tips_ref, and quality score. Variants are sorted by variant_id. The section is collapsed by default and shows a count header (e.g., "Variants (3)"). Competitor data for each proposition also appears in this drill-down panel (not as a standalone section).
6a. **Competitive Landscape** — (shown when at least one `competitors/*.json` file contains a non-empty `competitors[]` array; renders directly after the F×M matrix because competitor data is per-proposition, i.e. per F×M cell). Portfolio-level competitor view, deduplicated by competitor name across all propositions. Each competitor card shows: name plus a footprint summary badge ("N cells · F features × M markets"), expandable per-cell positioning and differentiation `<details>` blocks (summary line previews the first variant; expanded list shows each `{feature-chip + market-chip + per-cell text}`), strength/weakness pills (union across appearances) with `title=` tooltips naming the proposition slugs that contributed each pill, clickable proposition slug chips that open the per-proposition drawer (`openProposition`), and source URL links. Sorted by appearance count (desc), then by name. Hidden when no competitor data exists.
...
9a. **Context Intelligence** — (shown when `context/context-index.json` exists with at least one entry) Institutional intelligence extracted from uploaded documents, grouped by category (Competitive, Market, Pricing, Customer, Technical, Strategic — order follows the canonical category list in `references/data-model.md`). Each entry card shows summary (truncated to ~240 chars), source filename, confidence badge (high/medium/low), and entity-link chips for any referenced products/features/markets. Category groups are collapsible; the first non-empty category is expanded by default. Footer shows entry count, distinct source-file count, and the index `updated` date. Hidden when no context index exists.
...
This dashboard is the **reference implementation** of the design-variables pattern documented at `cogni-publishing/references/design-variables-pattern.md`. Other plugins building themed HTML dashboards (cogni-trends trend-report, scoring-ui, catalog) should follow the same 3-stage flow: select a theme through manage-themes → LLM derives design-variables.json → generator consumes JSON.
