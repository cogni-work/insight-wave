...
If `commercial_model_status` is present and `any_shared` is true, add a commercial-structure line before recommending solutions: a shared/fixed commercial structure was detected for one or more products — via a shared `revenue_model` (∈ {subscription, hybrid, partnership}), a fixed/catalog `commercial_model`, an off-enum `revenue_model` disposition, or a low propositions:distinct-commercial-models ratio — so the recommended next action is to **consolidate** — a shared reference cut (`shared_solution`) or a small package ladder (`packages`/Paketleiter) — rather than generating one tiered solution per proposition. The consolidation `next_action` emitted by `project-status.sh` already carries this wording; surface it instead of an "N solutions per proposition" suggestion.
