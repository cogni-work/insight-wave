...
3. For each group with ≥1 candidate, derive the category-grained feature:
   ...
   - **Description:** pick the candidate with the highest category-keyword overlap (tokenize `taxonomy_mapping.category_name`, strip the dedupe agent's stop-word set: `services`, `platform`, `solution`, `software`, `tools`, `management`; score each candidate by distinct keyword matches). Ties break by longest within the 20–35 word IS-layer budget from Step 7.1 — longest wins **inside** the budget, not outside it. Record the unselected candidate descriptions in `source_lineage` so per-stack nuance survives aggregation. This mirrors Pass A from Step 7.1, so the same stop-word set governs both ends of the pipeline.
