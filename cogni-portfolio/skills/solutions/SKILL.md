...
5. **Bootstrap delivery blueprints and shared solution eligibility.** Before discussing blueprint adaptation, ensure every product in the batch actually has a delivery blueprint. Products without blueprints get solutions designed from scratch with no structural consistency — different phases, pricing ratios, and assumptions across markets for the same product. This step fixes that by proposing blueprints proactively.
   ...
   **Commercial-structure consolidation gate — check `commercial_model_shared` first.** Read `commercial_model_status.matrix` from `project-status.sh` (one entry per product×market). When `commercial_model_shared` is `true` — a shared/fixed commercial structure, detected from any of four signals: a shared `revenue_model` (∈ {subscription, hybrid, partnership}), a fixed/catalog `commercial_model` (∈ {catalog, subscription, usage, partnership}), an off-enum `revenue_model` with a documented shared disposition (e.g. `transaction-fee`, `product_and_license`), or a low propositions:distinct-commercial-models ratio — **recommend consolidation**: a single shared reference cut via `shared_solution`, or a small package ladder (`packages`). Do this *before* the blueprint check below; the gate is **not** conditional on a `delivery_blueprint` already existing — a product with a decided shared model but an existing blueprint must still consolidate rather than tier one solution per proposition. If the user accepts, set `shared_solution: true` on the product so step 7 handles it as a shared solution and **skip the 1:1 per-proposition fan-out** for it. If the user declines consolidation, that product carries no `shared_solution` flag and falls back to the standard per-proposition fan-out in step 8 — it is never left without solutions. A product with a `project`-style revenue model and no other shared signal is not gated here and proceeds to step 8 as usual. (The user-facing recommendation text is emitted by `project-status.sh` in the project's output language.)
...
8. **For products not consolidated into a shared solution** (no `shared_solution: true` — i.e. a `project`-style revenue model, or a shared/fixed model whose step-5 consolidation the user declined): launch one `solution-planner` per proposition, in parallel. Products consolidated at the step 5 commercial-structure gate (now carrying `shared_solution: true`) are handled by step 7 and are not fanned out here. Include the `blueprint_guidance` object (or null) in each agent's task prompt.
