...
`tests/test-design-render.sh` grades the render with `drnd-NN-<discriminator>` ids under the same allocate-once rule; its recorded recipe mutates the adapter's `text()` body and must fail `drnd-10-frozen-copy`. Its expected strings come from the fixture inputs through the suite's own html.parser reader, never from a render. `drnd-34` and `drnd-35` need the pinned runtime: without it they print `SKIP:` naming the absent runtime and never `PASS:`. `drnd-36` compiles the render path with the oldest Python 3.9–3.11 interpreter on the host and prints `SKIP:` when there is none, as on CI's 3.12+ runner. The render fixtures live in `tests/fixtures/render/`: `composition-direct-de-edge-v2.json` is frozen `compose` output under the same regenerate-never-type rule; `themes/cogni-work/` is a fixed copy of the test theme, so captured geometry never moves with the bundled theme; and `plan-narrative-captured-v2.json` is a captured render of the narrative fixture with that theme — regenerate it only by rendering, and only when the layout rules change on purpose.
