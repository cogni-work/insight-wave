...
- **The deck overviews embed host font subsets.** `overview/boardroom-pptx.pdf` and `overview/editorial-pptx.pdf` are LibreOffice PDF exports, and each embeds a subset of the face LibreOffice substituted on the recording host: `ArialMT` in the boardroom overview and `TimesNewRomanPSMT` in the editorial one. No plugin script, theme, page or deck ships or embeds that face — the decks embed no font and the two proof brands ship none — so the two files are evidence of appearance only. Whether such subsets may stay in committed evidence, or the overviews should be rasterized like the page overviews, is a maintainer decision.
