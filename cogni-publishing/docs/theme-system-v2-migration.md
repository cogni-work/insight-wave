...
How to convert a tier-0 theme (single `theme.md`) into a tiered Theme System v2 theme (`manifest.json` + `tokens/` + optional `assets/`, `components/`, `templates/`). The walkthrough uses `cogni-publishing/themes/cogni-work/` as the worked example — every snippet maps to a file you can read on disk.
...
```
cogni-publishing/themes/cogni-work/tokens/
├── colors.json
├── typography.json
├── spacing.json
├── radii.json
...
```bash
python3 cogni-publishing/scripts/generate-tokens-css.py \
    --tokens-dir <user-themes>/<your-slug>/tokens --write
```
...
Components are HTML primitives downstream skills can copy-on-use. JSX is allowed but optional; HTML is the contract — a settled Theme System v2 design decision.
...
The full schema is `cogni-publishing/references/theme-manifest.schema.json` (JSON Schema draft-07). The reference doc is `cogni-publishing/references/theme-manifest.md`.

### Step 6. Phase 3 — Importing from a Claude Design bundle (alternative path)

Steps 1–5 are the **manual** authoring path: hand-populate each tier, write the manifest by hand, run the validator. For themes authored end-to-end in Claude Design (claude.ai/design), the Claude Design bundle importer collapses the whole sequence into a single import. The bundle is the upstream truth; the local theme directory is the materialised mirror.
...
4. Run the importer:
   ...
   python3 cogni-publishing/scripts/import-claude-design-bundle.py \
       --url <bundle-url> --target <user-themes>/<slug> [--allow-overwrite]
   ```
5. The importer fetches the bundle, verifies the gzip + sha256, untars, projects `colors_and_type.css` into the canonical token JSON files (keeping each supported `var()` alias as a role-to-role reference), regenerates `tokens.css` and, when an alias exists, `tokens.resolved.json` via `scripts/generate-tokens-css.py`, copies allowlisted component primitives, copies deck primitives and assets, materialises `theme.md` (auto-injecting the voice stub if the bundle omits it; success envelope reports `voice_section: "bundled"` or `"auto-injected-stub"`), regenerates `manifest.json`, runs `validate-theme-manifest.py`, and writes a `.claude-design-source` sidecar last with the bundle URL + sha256 + timestamp.
6. Run `bash cogni-publishing/scripts/verify-theme-backcompat.sh` to confirm the broader integration contract still holds (Phase A discover, Phase B consumer references, Phase D voice header).

The full mapping table — which bundle paths become which theme paths, the CSS-variable → JSON-token projection, the component allowlist, the strict-abort conditions — lives at [`cogni-publishing/references/claude-design-bundle-mapping.md`](../references/claude-design-bundle-mapping.md). Read it before extending or constraining the importer.
...
| A semantic CSS variable is missing from `tokens.css` | Its value was not a bare `var(--name)` (a fallback, `calc()` or `color-mix()`), or its chain ended on a declaration that is not a token | Read `aliases_dropped` and `declarations_dropped` in the importer envelope — each names the variable and why; rewrite it in Claude Design as a bare alias to a token and re-import |
| `alias-cycle` or `unresolved-reference` from the importer | A bundle alias loops, or names a variable the bundle's `:root` does not declare | Nothing was written. Fix the named variable in Claude Design and re-import |
...
```bash
python3 cogni-publishing/scripts/validate-theme-manifest.py \
    <user-themes>/<your-slug>
```
...
```bash
rm <user-themes>/<your-slug>/manifest.json
rm -rf <user-themes>/<your-slug>/{tokens,assets,components,templates}
```
...
- **Manifest schema reference:** `cogni-publishing/references/theme-manifest.md`
- **JSON Schema:** `cogni-publishing/references/theme-manifest.schema.json`
- **Manage Themes skill (Operation 7 — Author a Deep Theme System):** `cogni-publishing/skills/manage-themes/SKILL.md`
- **Reference implementation (cogni-work pilot):** `cogni-publishing/themes/cogni-work/`
- **Component loader contract:** `cogni-publishing/references/theme-component-loader.md` (its first consumer, cogni-workspace's HTML slide renderer, retired with the plugin's render chain)
