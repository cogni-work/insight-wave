---
contract_id: publishing-arc-registry
version: "2.0"
---
...
5. Add a mapping row and an element block to `${CLAUDE_PLUGIN_ROOT}/references/arc-taxonomy.md` — short names are the pre-colon segments of the contract's headings.
...
7. Run `bash cogni-publishing/tests/test-text-to-narrative-brief.sh` — its identity and path cases enumerate the arc files at run time, so the new arc is checked with no test edit.
