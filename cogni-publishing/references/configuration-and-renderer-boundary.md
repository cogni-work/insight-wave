...
The configured `renderer` pin above describes an external renderer consuming the retained `target-resolved-plan@1` chain. Platform rendering selects a host presentation skill through the design-render skill, or invokes an explicitly supplied host bridge. The separate, operator-provisioned browser runtime only measures pages; validation and verification install nothing. See [design-render.md](design-render.md).
