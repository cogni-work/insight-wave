...
Full worked examples, one per target, are the green fixtures at `cogni-publishing/tests/fixtures/design-brief/` — `slides-en.md`, `document-en.md`, `infographic-en.md`, `web-en.md` and `slides-de.md` — every one derived from a narrative fixture under `tests/fixtures/narrative-output/` and graded green by the checker.
...
`design` defaults, from the presentation-intent layer this skill shares with `${CLAUDE_PLUGIN_ROOT}/references/presentation-intent.md`: `register: quiet-executive`, `dark_slides: []`, `speaker_notes: full-script`, `imagery: none`, `variations: 1`. A caller override is recorded verbatim.
