...
`design-render` now treats the host presentation skill as the preferred PPTX renderer. Resolution is prose-driven—Codex selects bundled `presentations:Presentations` even when `document-skills:pptx` is installed; an Anthropic host selects its installed `anthropic-skills:pptx` or `document-skills:pptx`; other hosts select an installed skill whose description claims PPTX creation. There is no renderer registry. The exact unavailable envelope is `{"success": false, "error": "platform_renderer_unavailable"}`; the stdlib renderer remains a temporary fallback until its retirement change lands.
...
Platform provenance keeps `render-provenance@1` additive. Existing stdlib records may omit `kind` and `reproducible` and retain every legacy runtime, font, layout-face, plan and manifest requirement. Platform PPTX writes no `pptx-manifest.json`; package-derived editability in `design-verify` inventories the deck directly. The platform schema arm requires:

- Artifact identity, design-system name/version and content fingerprint; `renderer` with `kind: platform`, fully qualified `name`, actual `host`, `target` and either `version` or `marketplace_commit`; `reproducible: false`.
- `inputs.brief`, `inputs.composition` and `inputs.theme`, plus applicable supporting inputs, as `{path, sha256}` file records. The theme record points to its token file; additional theme/assets may be recorded as further inputs. `outputs.artifact` uses the same shape; `target_plan` is optional on this route.
- `live_proof` and `run.live` as matching booleans, plus `run.id`, matching host and skill identity, and `run.evidence` as a digest-bound JSON object documenting execution. That object must independently record `host`, `skill` and boolean `live`, all matching the outer run metadata; `--out-dir` reads and cross-checks them. A deterministic `test-platform-stub` may be admitted only with both live flags false; this establishes artifact admission, never skill execution.
- `attempts` in consecutive order from 1, each carrying findings, `preserve.differences`, before/after content fingerprints and nonempty ordered unit ids. Retain failed attempts faithfully. The admitted final attempt must have empty findings and preserve differences; the frozen input fingerprint and unit order remain unchanged throughout.
- `review` with its path/digest, empty `open_findings`, and explicit true coverage flags for `all_units`, `all_slides` and `overview`. The referenced review contains the artifact and unit locators required by the visual QA rubric; verification consumes that record independently.
- `applicability.theme`, `.fonts` and `.runtime`, each with `{applicable, reason, evidence}`. Evidence is an array of file records and must be nonempty when applicable; an inapplicable dimension still needs a concrete reason.

`check-provenance --out-dir` audits every input, output, review, host and applicability evidence file against its digest, validates the frozen brief/composition pair, and compares ordered unit ids to the composition. Without `--out-dir`, it checks the record's shape and internal consistency only. Paths resolve from the supplied output directory. Provenance documents execution; it cannot substitute for an actual host invocation or the independent visual and artifact checks.
...
1. for the html target, a family the theme ships resolves to its faces, and the page embeds each of them;
...
**Faces a theme ships.** A theme declares them in `assets/fonts/faces.json` as `{"faces": [...]}`. Each face carries:
- `family`: a plain name of letters, digits, spaces, `_` and `-` that is not a generic keyword.
- `weight`: an integer from 1 to 1000. It defaults to 400 when absent, so a declaration without one is the family's regular face.
- `file`: a path relative to the theme directory.
- `format`: `truetype` or `opentype`.
- `advance_em`: a positive number.
- `licence`: the path of the face's licence text, also inside the theme.

A face may also carry `advance_em_derivation`, saying how the metric was read.

A family ships one face per weight. The bundled `cogni-work` theme, for example, declares DM Sans at 400 and 700.

`resolve_theme` validates every declaration before anything is written. Each of the following fails as `invalid-theme` with check `theme-font`:
- a path that is absolute, contains `..`, or resolves through a symlink to anything outside the theme;
- a file that does not exist;
- bytes that do not start with the format's signature (`00 01 00 00` or `true` for truetype, `OTTO` for opentype);
- a weight that is a bool, a non-integer or out of range;
- a weight its family already declares;
- a file whose bytes an earlier declaration already names;
- a missing or non-positive `advance_em`.

The fidelity check knows an embedded face by its bytes, so one file declared under two weights would let a page label either weight with it. A theme is untrusted input and its faces' bytes are copied into the artifact, so containment is part of the contract rather than a convenience.

`advance_em` is declared data, because rendering is stdlib only and never parses a font. Derive it from the shipped file as the mean advance width of the glyphs U+0020 to U+007E map to, divided by `unitsPerEm` and rounded to three decimals. This reads the cmap, `hmtx`, `hhea` and `head` tables that TrueType and CFF OpenType files alike carry. The suite re-derives each shipped face's value that way.

A shipped family resolves only for a target that embeds it:
- **html.** The target embeds every face of the copy family, ordered by weight. Each face gets one `@font-face` whose `src` is a single `url(data:<type>;base64,…)` of that file's bytes, with the matching `format()` hint (`font/ttf` with `truetype`, `font/otf` with `opentype`). Each rule carries a `font-weight` descriptor equal to the face's declared weight, and all of them sit before the token block and outside the component rules. The descriptor is what keeps weights apart. Without it, every rule of the family would claim weight `normal`, and the last rule would set body copy. With it, the masthead and slot headings, which the browser sets bold, take the shipped bold face instead of a synthesized one. A weight the family does not ship is matched by CSS font matching; for example, a Semibold 600 heading is set in the 700 face.
- **pptx.** The target embeds no font, so a shipped family is skipped like any unshipped one, and the stack falls through to its generic family, recorded as a substitution.

The theme's tokens digest does not cover font bytes, so a shipped family's record also carries `file_sha256`, the copy face's digest, and `faces`, the `file`, `weight` and `file_sha256` of every face the page embeds.

Provenance records, per token, `requested_stack`, `requested_family`, `resolved_face`, `substituted` (true whenever the resolved face is not the first requested family), `skipped`, `fallback_chain` and `source` — `theme` for a family the theme ships, `bundled` or `generic` — plus `file_sha256` and `faces` for a `theme` source. A deck names a typeface rather than a family keyword. So the pptx target writes a resolved generic family as that family's documented `pptx_typeface`, a typeface Office ships, in the theme's font scheme and on every run. It embeds no font and records the typeface beside the resolution in its manifest.
...
- A **chart label** wraps at its label column, 40 % of the slot width, less an 8 px gap before the marks. It is estimated from the data item's `label` alone. Every point of one chart shares one row height, the tallest label's `max(lines × line height, 28 px) + spacing-3`, so a one-line label's row is as tall as a wrapped label's and a target that spreads its categories evenly over the series box keeps each mark on its label's row.
...
| `edge:<from>:<to>`, `rule:<key>`, `chart:<unit>` | a connector, a decorative accent rule and the chart frame; none carries text, and the chart frame carries its variant's `purpose` from the pattern library as its `descr` text alternative, never copy |
...
**The chart.** A sourced chart is one native bar chart in its own chart part, backed by an embedded workbook. Its categories are the brief's labels, its numeric cache the brief's literal numbers in order, and its series name the unit; the workbook carries the unit in `B1`, the labels in column A and the literals in column B, one row per point. The value axis pins its zero bound whenever every value lies on one side of zero — `c:min` 0 for positive values, `c:max` 0 for negative ones — because an application left to scale the axis itself may start it above zero when the values cluster, and bar lengths would stop being proportional to the values; values on both sides already span zero. The chart's graphic frame carries a `descr` text alternative, the variant's `purpose` from the pattern library, so an application's accessibility checker finds one; the data table's text stays the point labels and values on the slide. The point's label and value are also native text on its row, so the frozen strings stay text on the slide. Each point's row is the one the plan measured for the chart (§target-resolved-plan@2) — every row as tall as the tallest label's — and the rows stack from the top of the series box to its bottom. The chart frame is as tall as the series box and its plot area fills the frame, so the bar chart's evenly spread category bands, each the box height divided by the point count, are exactly those rows, and each bar is centred on its label's row. The label is one text frame at the box's left edge, exactly the chart label width wide with no inset, so the frame wraps the label where the plan did; the planned split is never written as line breaks or extra paragraphs. The value shares the row on one line, and the chart frame starts at the label column's right edge.
...
A render passes its own fidelity checks before it writes anything, and `design-verify` then checks the delivered output again, independently, against the frozen inputs. It adds per-family content preservation, object-level editability, declared accessibility, the critical visual classes, a persisted visual review, and a repair loop with a finite budget. See [`design-verify.md`](design-verify.md). The render path imports nothing from the verifier.
...
## Handover report examples

A deck has one slide per unit, preceded by a cover slide when the brief has a document title or subtitle. For example, with the bundled `cogni-work` theme: "Rendered 3 units to out/ as an editable deck: deck.pptx with 4 slides, 18 native objects and no fallback, plus target-plan.json, pptx-manifest.json and provenance.json. The theme ships DM Sans for pages, but a deck embeds no font, so the deck is set in Arial, the documented typeface for system-ui, and the manifest records the substitution. design-verify passes with no open finding."

   For a page with the same theme: "Rendered 3 units to out/ as a standalone page: index.html, target-plan.json and provenance.json. Copy is set in DM Sans, Regular and Bold, which the theme ships and the page embeds. design-verify passes with no open finding."

   With a theme that ships no face for its copy font: "Rendered 3 units to out/ as a standalone page: index.html, target-plan.json and provenance.json. The theme asks for Inter and ships no face for it, so the page is set in the system-ui chain — system-ui, then sans-serif — and provenance records the substitution. design-verify passes with no open finding."

## Finding response guide
...
- **`fit-overflow`** (pptx) — a unit needs more than one 1280 × 720 slide, a chart value would wrap, or a chart's rows or a system's nodes outgrow their box; `data.reference` names the unit. A deck never shrinks type, splits, merges, truncates or reorders a unit to fit. Shorten or split the content upstream, choose a pattern variant that carries less, or render the html target, whose frames may grow.
...
- **`invalid-theme`** — the theme's slug does not match the pinned design system, it has no `tokens/`, a required token role is missing, (pptx) a colour token is not hex, or (check `theme-font`) a shipped-face declaration in `assets/fonts/faces.json` is unusable; `design-render.md` §Fonts lists every rejection, from an unsafe or generic family name or a weight its family already declares to a file whose bytes an earlier declaration already names. `data.reference` names it. Select a matching theme or fix the token through `manage-themes`; for check `theme-font`, correct the declaration against `design-render.md` §Fonts.
- **`font-unresolved`** — no member of the theme's font stack is shipped by the theme (html only), bundled or a generic family. Add a generic family (`system-ui`, `sans-serif`, `serif`, `monospace`) to the end of the stack through `manage-themes`; never render with an unrecorded substitute.
