...
- Clause length, sentence-rhythm variance and the Floskel list: `${CLAUDE_PLUGIN_ROOT}/skills/copywriter/references/german-style-principles.md` (Wolf Schneider's maxims).
- The opening sentence — length, main-clause form, surprise before scene: `${CLAUDE_PLUGIN_ROOT}/skills/copywriter/references/german-hook-principles.md`; the narrative's Executive TL;DR first sentence is where those rules apply.
- The German readability band and the diagnostics the script reports: `${CLAUDE_PLUGIN_ROOT}/tests/fixtures/copywriter/readability.yml`, run by `${CLAUDE_PLUGIN_ROOT}/skills/copywriter/scripts/readability.sh` in Pass 4.
