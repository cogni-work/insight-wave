...
Reusable loader for Theme System v2 tier-3 component primitives. Lives at `cogni-publishing/scripts/load-theme-component.py`. Stdlib Python, JSON output convention, copy-on-use semantics. Use this from any consuming skill that wants to render with theme-supplied component primitives without reinventing the path resolution.
...
```bash
python3 cogni-publishing/scripts/load-theme-component.py \
...
spec = spec_from_file_location("loader", "cogni-publishing/scripts/load-theme-component.py")
...
The loader returns paths, never bytes. The caller reads the file, interpolates slide data into placeholders, and emits the result inline in the rendered HTML. The component file is not deep-copied into the output — the rendered HTML is the artifact, the component file stays where it is. Copy-on-use is the settled default; an opt-in live-theme binding is reserved for a future iteration.
...
- **Not a CSS bundler.** Tokens are handled separately via `tiers.tokens` (see `cogni-publishing/scripts/generate-tokens-css.py`). The loader only resolves component paths.
- **Not a live theme system.** Copy-on-use means the rendered HTML is frozen at render time. Live-reload is reserved for a future iteration.
- **Not a validator.** The loader is tolerant on purpose — it returns miss for shapes a validator would reject. Run `cogni-publishing/scripts/validate-theme-manifest.py` if you need schema enforcement.

## Reference consumer

The first consumer was cogni-workspace's own HTML slide renderer (the tiered-theme pilot), whose `--theme-slug` flag wired the loader for tier-1 tokens. That renderer retired with the plugin's local render chain, so the loader currently has no in-plugin consumer: it stays as shared infrastructure for cogni-website and cogni-portfolio, and tier-3 deck primitive integration remains gated on `cogni-work` shipping a `tiers.components.deck` family.
