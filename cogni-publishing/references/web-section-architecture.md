...
**Arc mapping:** See `$CLAUDE_PLUGIN_ROOT/references/arc-taxonomy.md` for the complete arc_id → arc_type mapping table, arc element names with EN/DE translations, and the element-to-role assignment heuristic. The shared reference is the single source of truth for every publishing skill that resolves an arc; its frontmatter `consumers:` list names them.
...
**Mapping logic:** Labels are assigned **content-source-first, role-based as fallback**. During decomposition, track which narrative H2 chapter each section's content originates from. If the chapter matches an arc element name, use that element as the label. Otherwise, fall back to role-based mapping. See `$CLAUDE_PLUGIN_ROOT/references/arc-taxonomy.md` for the full heuristic including chapter detection pseudocode.
