#!/usr/bin/env python3
"""check-theme-drift.py — Detect drift between standard and user theme copies.

Compares the bundled themes that ship with cogni-publishing (``themes/`` beside
this script's plugin root — the "standard" side) and the optional user theme
location (the "workspace" side). For each slug that exists on both sides,
computes a shallow signature and classifies the drift. Slugs that exist on only
one side are not reported — that is the normal case, not drift.

The user location resolves as ``--user-themes`` > ``--workspace-root``/themes >
``$COGNI_WORKSPACE_ROOT/themes``. It is read in place and never created.
...
Output: the standard envelope ``{"success": bool, "data": {...}, "error": "..."}``.

When the user location is missing or stale (no ``COGNI_WORKSPACE_ROOT``, stale
...
# The plugin that owns the bundled themes is the one this script ships in. It
# is derived from the script's own location, never from $CLAUDE_PLUGIN_ROOT:
# a caller in another plugin (or a compatibility delegate) runs with that
# variable pointing at ITS plugin, which carries no themes/ directory.
DEFAULT_PLUGIN_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def resolve_user_themes_dir(user_themes="", workspace_root=""):
    """Return the configured user theme directory, or "" when none applies.

    Precedence: an explicit ``user_themes`` directory, else
    ``<workspace_root>/themes`` where ``workspace_root`` falls back to
    ``$COGNI_WORKSPACE_ROOT``. A stale or missing location yields "". Nothing is
    ever created.
    """
    if user_themes:
        return user_themes if os.path.isdir(user_themes) else ""
    workspace_root = workspace_root or os.environ.get("COGNI_WORKSPACE_ROOT", "")
    if is_stale_path(workspace_root):
        return ""
    candidate = os.path.join(workspace_root, "themes")
...
def main():
    parser = argparse.ArgumentParser(description="Detect drift between standard and user theme copies.")
    parser.add_argument("--plugin-root", default="",
                        help="Root holding the bundled themes/ (default: this script's plugin)")
    parser.add_argument("--user-themes", default="",
                        help="User theme directory; beats --workspace-root and COGNI_WORKSPACE_ROOT")
    ...
    standard_dir = os.path.join(plugin_root, "themes")
    workspace_dir = resolve_user_themes_dir(args.user_themes, args.workspace_root)
