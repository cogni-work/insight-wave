#!/usr/bin/env python3
"""Discover all available themes across the bundled and user theme locations.

Scans two locations:
  1. Standard themes: the themes/ directory bundled with cogni-publishing, found
     relative to this script (``--plugin-root`` overrides it). $CLAUDE_PLUGIN_ROOT
     is deliberately NOT read: a caller in another plugin, or a compatibility
     delegate, runs with it pointing at a plugin that ships no themes.
  2. User themes ("workspace" source): an optional location, read in place and
     never created. Precedence: ``--user-themes`` > ``--workspace-root``/themes >
     $COGNI_WORKSPACE_ROOT/themes > auto-discovery.

When no explicit user location is given and COGNI_WORKSPACE_ROOT is empty or
points to a non-existent path (e.g. stale Cowork session), auto-discovers
...
# The plugin whose themes/ ship as the "standard" set: the one this script
# lives in (<plugin>/scripts/discover-themes.py).
DEFAULT_PLUGIN_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def _load_tier_validator():
    """Dynamically load the sibling validate-theme-manifest.py.
    ...
    (callers fall back to tier-0 behaviour silently). Only the sibling is tried:
    an environment-supplied plugin root can name a different plugin entirely.
    """
    ...
    candidates = [os.path.join(here, "validate-theme-manifest.py")]
...
def main():
    ...
    parser.add_argument("--user-themes", default="",
                        help="User theme directory, read in place; beats every other user-location source")
    ...
    # 1. Standard themes (bundled with the plugin this script ships in)
    ...
    # 2. User themes ("workspace" source), read in place — never created here.
    # An explicit --user-themes wins outright and suppresses auto-discovery.
    discovered = False
    if args.user_themes:
        workspace_dir = args.user_themes
        if not os.path.isdir(workspace_dir):
            print(f"WARNING: user themes dir not found: {workspace_dir}", file=sys.stderr)
            workspace_dir = ""
    elif is_stale_path(workspace_root) and not args.no_discover:
        if workspace_root:
