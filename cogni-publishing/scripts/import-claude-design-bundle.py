...
into a Theme System v2 theme directory.

Stdlib-only. Reads the curated mapping at
``references/claude-design-bundle-mapping.md``; the rules below are the
machine-readable mirror of that document. Claude Design is an optional,
observed input: the bundle layout is what exported bundles have been seen to
contain, not a published API, and a local ``--bundle`` archive imports with no
...
→ voice-header pre-check → project tokens (CSS → JSON), keeping each supported
``var()`` alias as a role-to-role reference → compile the token graph in memory
(an alias cycle or unresolved reference aborts here, before anything is
written) → write tokens/*.json and their generated projections via
``generate-tokens-css.py`` → copy components / assets / deck primitives per the
allowlist → regenerate manifest.json → remove files a previous import managed
that this one no longer produces → validate via ``validate-theme-manifest.py``
→ write ``.claude-design-source`` sidecar.

Re-syncable upstream contract: re-running with the same URL is a no-op when
the freshly-fetched archive's sha256 matches the sidecar's recorded sha256. A
re-import with ``--allow-overwrite`` replaces only files the sidecar records as
importer-managed; any other file in the target keeps its bytes.
...
Output: the standard JSON envelope on stdout
    {"success": bool, "data": {...}, "error": "string"}
Exit code 0 on success, 1 on failure. A token-graph rejection's ``data`` is the
compiler's finding (``code``, ``token``, ``reference``) and nothing else.
"""
...
# the first match wins. A literal-valued variable that matches none of these
# patterns is not a token and is ignored (base element styles). An alias-valued
# variable that matches none of them belongs to the bundle's semantic layer and
# lands in the `semantic` stem, keyed by its own name without the leading `--`.
TOKEN_PROJECTION = [
...
# any of these cannot represent a literal primitive. A value that is exactly
# one `var(--name)` with no fallback is the exception: it is a supported alias
# and is kept (see retains_alias). Every other var() form, and every calc(),
# color-mix(), env() or attr() value, is dropped and reported.
LOSSY_VALUE_PREFIXES = ("var(", "calc(", "color-mix(", "env(", "attr(")

# The one supported alias form: a bare `var(--name)`, no fallback, nothing else.
ALIAS_VAR = re.compile(r"var\(\s*(--[A-Za-z0-9-]+)\s*\)")

SEMANTIC_STEM = "semantic"
...
def retains_alias(value: str) -> bool:
    return ALIAS_VAR.fullmatch(value) is not None


class AliasGraphError(Exception):
    """An alias the bundle declares but the token graph cannot hold."""
...
def _project_name(var_name: str):
    """Map a primitive-rule variable to (stem, key), or None when no rule applies.

    Earlier patterns win — typography prefixes (--fs/--lh/--ls) must be checked
    before the broader --font-* rule, and the table already orders them so.
    """
    for pattern, stem, key_fn in TOKEN_PROJECTION:
        m = pattern.match(var_name)
        if m:
            return stem, key_fn(m)
    return None


def project_tokens(css_text: str) -> dict:
    """Project :root declarations into the canonical token graph.

    Returns ``{"tokens": {stem: {key: value}}, "aliases_dropped": [...],
    "declarations_dropped": [...]}``. A literal lands under its primitive rule.
    A supported alias — a value that is exactly ``var(--name)`` — lands under its
    own primitive rule when it has one, otherwise in the ``semantic`` stem, and is
    stored as ``{<stem>.<key>}`` pointing at its IMMEDIATE target, so a chain keeps
    every intermediate role rather than collapsing onto the final colour.

    An alias whose chain reaches an undeclared variable, or loops, raises
    ``AliasGraphError``: the bundle states a reference the theme cannot honour,
    and importing around it would silently lose a role. An alias whose chain ends
    on a declaration that is not itself a token (a calc() value, or a literal
    matching no rule) cannot be stored and is dropped — reported, never silent.
    The same declaration repeated keeps its last value, matching the CSS cascade
    for :root.
    """
    declared = {}
    for var_name, value in parse_root_declarations(css_text):
        declared[var_name] = value

    tokens = {stem: {} for stem in CANONICAL_TOKEN_STEMS}
    home = {}            # var name -> (stem, key) for every declaration that becomes a token
    alias_target = {}    # alias var name -> the var name it references
    aliases_dropped, declarations_dropped = [], []

    for var_name, value in declared.items():
        if retains_alias(value):
            alias_target[var_name] = ALIAS_VAR.fullmatch(value).group(1)
            continue
        if not is_literal_value(value):
            if value.lower().startswith("var("):
                aliases_dropped.append({
                    "token": var_name, "reference": value,
                    "reason": "var() with a fallback or extra content is outside the supported alias form",
                })
            else:
                declarations_dropped.append({"token": var_name, "value": value})
            continue
        projected = _project_name(var_name)
        if projected:
            stem, key = projected
            tokens[stem][key] = value
            home[var_name] = projected

    # Walk each alias chain to its terminal once, in declaration order, so the
    # first reported failure is deterministic.
    for alias in alias_target:
        seen = [alias]
        current = alias_target[alias]
        while current in alias_target:
            if current in seen:
                chain = " -> ".join(seen + [current])
                raise AliasGraphError(
                    "alias-cycle", alias, current,
                    "bundle alias cycle: {}".format(chain),
                )
            seen.append(current)
            current = alias_target[current]
        if current not in declared:
            raise AliasGraphError(
                "unresolved-reference", alias, current,
                "bundle alias {} references {}, which the bundle's :root does not "
                "declare".format(alias, current),
            )

    # Assign homes to the aliases whose chain ends on a token, then store each
    # one as a reference to its immediate target. Iterate to a fixed point: an
    # alias's home is known only once its target's is.
    pending = dict(alias_target)
    while pending:
        progressed = False
        for alias, target in list(pending.items()):
            if target in home:
                stem_key = _project_name(alias) or (SEMANTIC_STEM, alias[2:])
                tokens[stem_key[0]][stem_key[1]] = "{{{}.{}}}".format(*home[target])
                home[alias] = stem_key
                del pending[alias]
                progressed = True
            elif target not in alias_target or target not in pending:
                aliases_dropped.append({
                    "token": alias, "reference": target,
                    "reason": "the referenced declaration is not a token (no projection rule, or a lossy value)",
                })
                del pending[alias]
                progressed = True
        if not progressed:  # unreachable: cycles were rejected above
            break

    return {
        "tokens": tokens,
        "aliases_dropped": aliases_dropped,
        "declarations_dropped": declarations_dropped,
    }
...
def write_projections(tokens_dir: Path, compiled) -> dict:
    """Write the generated projections of an already-compiled token graph.

    ``tokens.css`` is always written; ``tokens.resolved.json`` only when the
    theme carries an alias — the same rule ``generate-tokens-css.py --write``
    applies, so a flat theme keeps exactly the files it always had. Returns the
    byte count and the relative names written.
    """
    css = _TOKEN_GEN.render_css(compiled)
    (tokens_dir / "tokens.css").write_text(css, encoding="utf-8")
    written = ["tokens.css"]
    if compiled.aliases():
        (tokens_dir / _TOKEN_GEN.RESOLVED_FILENAME).write_text(
            _TOKEN_GEN.resolved_text(compiled), encoding="utf-8")
        written.append(_TOKEN_GEN.RESOLVED_FILENAME)
    return {"css_bytes": len(css), "files": written}
...
def write_sidecar(theme_dir: Path, url: str, sha256: str, bundle_root: str, managed_files: list) -> None:
    sidecar = {
        "url": url,
        "sha256": sha256,
...
def remove_stale_managed(target: Path, previous: list, current: list) -> list:
    """Delete files a previous import managed that this import no longer produces.

    Only paths the previous sidecar listed are candidates, so a file the user
    added to the theme directory is never touched. A sidecar written before the
    list existed carries none, and then nothing is removed. Each candidate must
    be a relative path resolving inside ``target``; anything else is ignored.
...
def run(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="claude-design-bundle-") as tmpdir:
        ...
        try:
            projection = project_tokens(css_text)
        except AliasGraphError as e:
            return err(str(e), **e.finding())
        ...
        # Compile the in-memory graph before any write, so a graph the compiler
        # refuses leaves the target exactly as it was.
        try:
            compiled = _TOKEN_GEN.compile_maps({s: projected[s] for s in populated_stems})
        except _TOKEN_GEN.TokenError as e:
            return err(str(e), **e.finding())
        ...
        if args.dry_run:
            return emit(
                ...
                data={
                    ...
                    "aliases_dropped": projection["aliases_dropped"],
                    "declarations_dropped": projection["declarations_dropped"],
        ...
        previous_managed = (existing or {}).get("managed_files") or []
        ...
        # tokens/*.json + their generated projections
        tokens_dir = target / "tokens"
        try:
            token_stems = write_tokens(tokens_dir, projected)
            projections = write_projections(tokens_dir, compiled)
        except (OSError, RuntimeError) as e:
            return err("token materialisation failed: {}".format(e))
        css_bytes = projections["css_bytes"]
        ...
        managed = (
            ["theme.md", "manifest.json"]
            + ["tokens/{}.json".format(stem) for stem in token_stems]
            + ["tokens/{}".format(name) for name in projections["files"]]
            + ["components/web/{}".format(name) for name in components_report["web"]]
            + ["components/deck/{}".format(name) for name in deck_files]
            + ["assets/{}".format(name) for name in asset_files]
        )
        try:
            removed = remove_stale_managed(target, previous_managed, managed)
        except OSError as e:
            return err("cannot remove stale managed files: {}".format(e))
        ...
        # Sidecar last.
        try:
            write_sidecar(target, source_url, sha256, bundle_root.name, managed)
        except OSError as e:
