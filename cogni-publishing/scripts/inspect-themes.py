...
For each theme that the picker would resolve (merged across the bundled
themes shipped with cogni-publishing and the optional user theme location,
user copies shadowing bundled ones), report:
...
Read-only, stdlib only. Reuses ``signature()``, ``scan_dir()``, the default
plugin root and the user-location precedence from the sibling
``check-theme-drift.py`` via dynamic import (the filename uses hyphens, so
direct ``import`` does not work — same pattern as
``validate-theme-manifest.py:_load_token_generator()``).
...
def run_validator(theme_dir):
    """Subprocess the sibling validate-theme-manifest.py; return (pass: bool, first_error: str)."""
    script = os.path.join(HERE, "validate-theme-manifest.py")
...
def build_row(slug, theme_dir, source, strict):
    ...
    if strict:
        if sig["tier"] == "tier-0":
            ...
            ok, err = run_validator(theme_dir)
...
def main():
    ...
    plugin_root = args.plugin_root or _DRIFT.DEFAULT_PLUGIN_ROOT
    standard_dir = os.path.join(plugin_root, "themes")
    workspace_dir = _DRIFT.resolve_user_themes_dir(args.user_themes, args.workspace_root)
    ...
    for slug, theme_dir, source in merged_themes(standard_dir, workspace_dir):
        row = build_row(slug, theme_dir, source, args.strict)
        if row is None:
            continue
        if row["tier"] == "tiered":
            tiered_count += 1
