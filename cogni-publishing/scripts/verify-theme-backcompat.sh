...
#
# This is the umbrella check the Theme System v2 contract gates on. Per-surface
# evals each verify their own slice; this script verifies the integration —
# fields added by one surface must still be readable by every consumer, and any
# tier-0 theme must keep producing legacy-shaped output.
#
# Usage:
#   bash cogni-publishing/scripts/verify-theme-backcompat.sh [--help] [-v|--verbose] [--regenerate-baseline]
#
...
# cogni-publishing/tests/test-theme-backcompat.sh that run-plugin-tests.py
...
#       --root . \
#       --file cogni-publishing/scripts/baselines/_template-tier0-output.json \
#       --expr 's#"source": "standard"#"source": "mutated"#' \
#       --test 'bash cogni-publishing/tests/test-theme-backcompat.sh' \
...
Failure-mode triage table
  ...
  - tbc10-tier0-baseline-match ("tier-0 baseline mismatch") or
    tbc06-tier0-fixture-discover ("tier-0 fixture discover failed"):
      The manifest reader in discover-themes.py and its tier-0 fallback.
      The contract is "no manifest.json => byte-identical legacy output";
      if discover added a field, this fires.

  - tbc13-cogni-work-tiers-tokens ("cogni-work missing tiers.tokens" or
    "tokens.css absent"):
      The tiered cogni-work reference theme or the manifest schema. Either
      the reference theme regressed or the schema stopped accepting its
      manifest.

  - tbc14-validator-accepts-cogni-work ("validate-theme-manifest.py rejects
    cogni-work"):
      The manifest schema, the token compiler, or manage-themes deep-authoring
      guidance — schema and authoring surfaces drifted.

  - tbc18-migration-guide-present ("migration guide reference missing"):
      The Theme System v2 migration guide under docs/ moved or was removed.
...
Only when an intentional schema change ships (reviewed and signed off):
    ...
    bash cogni-publishing/scripts/verify-theme-backcompat.sh --regenerate-baseline
...
if [[ ! -f "$VALIDATOR_SCRIPT" ]]; then
  fail "tbc03-validator-present validate-theme-manifest.py not at $VALIDATOR_SCRIPT. The validator moved or was renamed."
fi
...
case "$TIERS_PROBE" in
  ...
  MISSING_THEME) fail "tbc13-cogni-work-tiers-tokens cogni-work missing from discover" "discover-themes.py did not return cogni-work. The tiered cogni-work reference theme regressed." ;;
  MISSING_TIERS) fail "tbc13-cogni-work-tiers-tokens cogni-work missing tiers.tokens" "discover output for cogni-work has no 'tiers' key. The manifest fallback or the manifest schema regressed. Check $PLUGIN_ROOT/themes/cogni-work/manifest.json." ;;
  MISSING_TOKENS_KEY) fail "tbc13-cogni-work-tiers-tokens cogni-work tiers.tokens absent" "manifest.json declares no tokens tier. The manifest does not match the v2 schema." ;;
  TOKENS_DIR_ABSENT:*) fail "tbc13-cogni-work-tiers-tokens cogni-work tokens dir absent" "tiers.tokens resolves to a path that does not exist: ${TIERS_PROBE#TOKENS_DIR_ABSENT:}. The reference theme lost its tokens/ directory." ;;
  TOKENS_CSS_ABSENT:*) fail "tbc13-cogni-work-tiers-tokens cogni-work tokens.css absent" "tokens dir exists but tokens.css is missing: ${TIERS_PROBE#TOKENS_CSS_ABSENT:}. The generated tier-1 tokens.css projection is missing." ;;
...
if python3 "$VALIDATOR_SCRIPT" "$PLUGIN_ROOT/themes/cogni-work" >/dev/null 2>&1; then
  ...
  fail "tbc14-validator-accepts-cogni-work validate-theme-manifest rejects cogni-work" "Run \`python3 $VALIDATOR_SCRIPT $PLUGIN_ROOT/themes/cogni-work\` for the error. The manifest schema, the token compiler, or the manage-themes authoring guidance drifted."
fi
...
if [[ -f "$MANAGE_SKILL" ]] && grep -q "manifest\.json" "$MANAGE_SKILL"; then
  ...
  fail "tbc17-manage-themes-references-manifest manage-themes SKILL.md manifest reference missing" "$MANAGE_SKILL no longer mentions manifest.json. The deep-authoring documentation regressed."
fi
...
if [[ -f "$MIGRATION_GUIDE" ]]; then
  ...
  fail "tbc18-migration-guide-present migration guide reference missing" "$MIGRATION_GUIDE not found. The guide was removed or the path moved."
fi
...
VISUAL_CONSUMERS=(
  # Neither cogni-publishing nor cogni-workspace lists a consumer of its own:
  # the story-to-* brief producers retired in favour of text-to-narrative, which
  # hands a design brief to Claude Design and reads no theme, and the local
  # render chain that read one (render-html-slides, enrich-report) retired after
  # them.
  "cogni-portfolio:portfolio-dashboard"
...
for entry in "${VISUAL_CONSUMERS[@]}"; do
  ...
  # A publishing-only install has no sibling plugin directories at all, so an
  # absent PLUGIN is a layout fact and is skipped. A present plugin whose listed
  # SKILL.md is gone is still the hard failure below: that is precisely when the
  # check has to fire.
  if [[ ! -d "$REPO_ROOT/$plugin" ]]; then
    c_skip "$entry — consumer plugin not present beside $PLUGIN_ROOT (standalone install)"
    continue
  fi
  if [[ ! -f "$skill_md" ]]; then
