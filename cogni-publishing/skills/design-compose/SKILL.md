...
3. Group consecutive frozen records into units, up to the chosen pattern's `constraints.max_records`, and choose one accepted pattern and variant per unit — by slide `type` or section role, record kind and whether the records carry data. One exception: on a unit that declares metric intent whose bound text repeats one of the brief's key figures, leave both out and let `compose` route it (step 5). Routing declines in three cases — no matching figure, a single match in a unit of more than four bound texts, or a candidate whose limits the draft's bindings exceed — and a unit it declines stays patternless and is rejected as `unknown-pattern`, so give such a unit a pattern instead. Parallel direct sections can share one comparison unit. A narrative `visual_intent` is a hint the choice may decline, never a requirement. A brief that carries sources needs one `sources` unit: the narrative source-register slide takes it, and a direct brief ends on a `sources` unit with no bindings.
