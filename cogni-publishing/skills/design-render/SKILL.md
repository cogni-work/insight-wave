...
3. Read the envelope. On success, report the output paths, the number of units and the font resolution: when `substituted` is true, name the requested face, the face used and, for pptx, the Office typeface the deck is written in, so the substitution is never silent.
...
4. For a deck, the native chart's data opens with the application's Edit Data command, speaker notes sit in the notes pane, and every citation is a live link. Rendering a deck never needs the browser runtime.
...
- Every deck object is native and editable. A picture is allowed only as a fallback its unit's pattern declares for the pptx target, recorded in the manifest with its capability and reason; today no pattern declares one, so a deck carries none.
