...
- **`invalid-theme`** — the theme's slug does not match the pinned design system, it has no `tokens/`, a required token role is missing, (pptx) a colour token is not hex, or (check `theme-font`) a shipped-face declaration is unusable, for example an unreadable or empty `faces.json`, an unsafe or generic family name, a weight that is not an integer from 1 to 1000 or that its family already declares, one file declared twice, an unsupported format, a path outside the theme, a missing file or licence, a file that is not its declared font format, or no positive `advance_em`; `data.reference` names it. Select a matching theme or fix the token through `manage-themes`; for check `theme-font`, correct the declaration in the theme's `assets/fonts/faces.json` against `design-render.md` §Fonts.
