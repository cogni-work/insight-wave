...
A caller that already holds a theme path — or runs where no workspace exists at all — resolves it without discovery:

```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/select-theme.py" --theme-path /path/to/theme.md
```
...
To disable auto-discovery — in CI, or any hermetic run — pass `--no-discover`. To name the user location explicitly:

```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/discover-themes.py" --user-themes "/path/to/themes"
...
1. Fall back to manual Glob scanning with the `path` parameter set to each base directory:
   ...
   - Workspace themes: pattern `*/theme.md`, path the user location resolved above (typically `$COGNI_WORKSPACE_ROOT/themes`)
