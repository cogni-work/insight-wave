...
ARC_DIR="${ARC_CONTRACT_STORY_ARC_DIR:-$PLUGIN_DIR/references}"
...
# ---------------------------------------------------------------------------- V1
arcs="$(ls "$ARC_DIR"/arc-*.md 2>/dev/null | xargs -n1 basename 2>/dev/null | sed 's/^arc-//; s/\.md$//' | grep -Evx 'registry|taxonomy' | sort)"
if [ -z "$arcs" ]; then
