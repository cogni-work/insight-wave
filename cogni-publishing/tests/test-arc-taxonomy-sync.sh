...
#
# WHAT THIS PINS
#   The `arc_id` set in the mapping table of cogni-publishing/references/arc-taxonomy.md
#   must equal the set of arc directories under
#   cogni-publishing/references/ (one flat arc-{arc-id}.md per arc).
#
...
TAXONOMY="${ARC_TAXONOMY_PATH:-$PLUGIN_DIR/references/arc-taxonomy.md}"
ARC_DIR="${ARC_STORY_ARC_DIR:-$PLUGIN_DIR/references}"
...
ls "$ARC_DIR"/arc-*.md | xargs -n1 basename | sed 's/^arc-//; s/\.md$//' | grep -Evx 'registry|taxonomy' | sort > "$TMPROOT/dir_ids.txt"
...
h1_victim=$(for f in "$ARC_DIR"/arc-*.md; do
  [ "$(basename "$f")" = "arc-taxonomy.md" ] && continue
