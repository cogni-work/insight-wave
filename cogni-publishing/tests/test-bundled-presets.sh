...
# out of `cogni-publishing/themes/`. "Offerable" is four separate properties, and
...
#      --file cogni-publishing/themes/signal/theme.md \
#      --expr 's/\*\*Text\*\*: `#0F172A`/**Text**: `#FFFFFF`/' \
#      --test 'bash cogni-publishing/tests/test-bundled-presets.sh' \
...
#      --file cogni-publishing/themes/boardroom/theme.md \
...
#      --file cogni-publishing/tests/test-bundled-presets.sh \
...
# --------------------------------------------------------------------------
# Scoped to skills/, which is where a dispatch can actually happen — this plugin
# ships no agents/ — and NOT to all of cogni-publishing, because this file would
# then match itself: the retired token is this guard's own matching data. That
# is the same exemption cogni-workspace's `tests/test-relocated-skill-hygiene.sh`
# takes for its spec table — a directory-level spec would fail on the files where
# the literal is the needle rather than the offence.
...
# or a crashed git as "no token found". The scanned directory is asserted
# present first, and only a real exit 1 over a present directory is the green
# arm; anything above 1 is a failure to scan, never a clean scan.

if [ ! -d "$PLUGIN_ROOT/skills" ]; then
  fail "bp-06-no-external-preset-skill the skills directory is missing under cogni-publishing, so the theme-factory token scan could not run"
else
  git -C "$PLUGIN_ROOT/.." grep -qI 'theme-factory' -- cogni-publishing/skills 2>/dev/null
  rc=$?
  case "$rc" in
    0) fail "bp-06-no-external-preset-skill a theme-factory token survives under cogni-publishing skills" ;;
    1) pass "bp-06-no-external-preset-skill no theme-factory token under cogni-publishing skills" ;;
