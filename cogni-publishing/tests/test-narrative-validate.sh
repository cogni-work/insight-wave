...
#     --test 'bash cogni-publishing/tests/test-narrative-validate.sh' --case NV21
#   --file cogni-publishing/skills/text-to-narrative/scripts/validate-narrative.py \
#     --expr 's{gate\("T0", not opening,}{gate\("T0", True,}' \
#     --test 'bash cogni-publishing/tests/test-narrative-validate.sh' --case NV19
#
...
CONTRACT="$PLUGIN_DIR/references/arc-corporate-visions.md"
...
for pair in "strategic-choice-en.md:strategic-choice:NV14" "consulting-problem-solving-de.md:consulting-problem-solving:NV15"; do
  ...
  OUT="$(python3 "$VALIDATOR" --narrative "$PLUGIN_DIR/tests/fixtures/narrative-output/$f" --contract "$PLUGIN_DIR/references/arc-$arc.md" --json 2>&1)"
  RC=$?
  if [ "$RC" -eq 0 ] && printf '%s' "$OUT" | grep -q '"success": true'; then
