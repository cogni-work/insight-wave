...
- **cogni-narrative** — two integration paths. (1) **Post-research narrative**: user invokes `/narrative --source-path <report-output-dir>` after research completes to produce a ~1,675-word executive summary with the chosen arc. cogni-narrative auto-bridges `[Source: Publisher](URL)` citations into per-source files via its built-in citation bridge (Phase 0.5). (2) **Arc-driven research report**: cogni-research itself can produce a long-form (3K–8K word) report whose section structure follows a cogni-narrative arc, via `story_arc_id` in `project-config.json`. The arc registry at `references/story-arcs.json` is a duplicated-and-audited subset of `cogni-narrative/skills/narrative/references/story-arc/` — kept in sync via the `audit-arcs` skill (mirrors the established `cogni-copywriting/skills/audit-copywriter` pattern). The two paths are independent: a project can run with `story_arc_id: corporate-visions` and still be passed to `/narrative` afterwards for the short-form executive summary.
...
- **cogni-workspace** — `references/market-sources.json` is a slim **overlay** keyed against the canonical `cogni-workspace/references/supported-markets-registry.json`. The overlay carries only research-specific operational metadata: `vocabulary_hints`, `local_query_tips`, `region_qualifiers` (search-query format), `local_language`, and `authority_metadata{}` keyed by domain (`category`, `authority` tier, `search_pattern`). Shared market fields (codes, names, locales, currencies, languages, regional qualifiers, canonical authority-domain set) live only in the registry — read sites call `cogni-workspace/scripts/get-market-config.py --plugin research --market <code>` which joins overlay + registry into the legacy embedded shape. Drift on the shared market set is structurally impossible; orphan overlays are surfaced by `cogni-workspace:audit-region-sources`.
...
|-------|------|---------|-------------|
| `market` | string | *required* | Region code for search localization. Must be one of the keys in the canonical workspace registry — research sees 21 markets via the merge utility (`dach`, `de`, `at`, `fr`, `it`, `es`, `nl`, `pl`, `cz`, `sk`, `hu`, `ro`, `hr`, `gr`, `mk`, `uk`, `us`, `eu`, `mx`, `br`, `cn`). The 18 canonical user-selectable codes surfaced in the research-setup menu are `dach`, `de`, `fr`, `it`, `pl`, `nl`, `es`, `cz`, `sk`, `hu`, `hr`, `gr`, `mx`, `br`, `cn`, `us`, `uk`, `eu`. `eu` is a composite market that fans out per-country researchers. No `global` option — research-setup resolves ambiguity by asking the user. The script refuses to run without a valid code (the only compat exception is legacy `--language de` → `dach`). |
...
| `citation_format` | string | "apa" | Citation style (apa/mla/chicago/harvard/ieee/local-wikilink). `wikilink` is accepted as a deprecated alias for `ieee` and normalised on write (the v0.7.x–v0.8.2 anchor-based shape broke in Obsidian — v0.8.3 collapses it to URL-direct superscripts). See `references/citation-formats.md` |
...
| `allow_short` | bool | `false` | When `true`, the Phase 4.5 word-count gate logs any shortfall but skips the writer expansion re-dispatch, and the Phase 5 expansion-review loop is skipped. The reviewer's stepped completeness cap still applies (the verdict is still honest), and the Phase 6 summary still surfaces the deficit warning — this flag only disables the automatic expansion attempts. Use when a power user wants deep-mode tree structure but intends to hand-edit prose downstream. Under `prose_density: executive`, this flag is a **no-op** — there is no shortfall to allow against a ceiling. The flag is preserved for legacy semantics but does not change the executive Phase 4.5 flow |
| `target_words` | int | default-by-depth (basic 3000, detailed 5000, **deep 5000** (was 8000 before v0.7.7), outline 1000, resource 1500) | Writer word-count floor (under `prose_density: standard`) or ceiling (under `prose_density: executive`), independent of `report_type`. Set explicitly to override the default-by-depth. Resolved at project creation by `initialize-project.sh` and pinned into `project-config.json` for the project's lifetime. Phase 4 writer dispatch, Phase 4.5 writer gate, Phase 5 expansion loop, and Phase 6 promotion gate all resolve their reference value from this field. `initialize-project.sh --target-words <N>` takes a positive integer. In v0.7.7 (issue #35) the deep default was reduced from 8000 to 5000 to align with professional deep-research norms and the single-voice writer's ~5.6–6.1K single-call ceiling; set `target_words: 8000` for the old 8K-deep long-form behaviour. The Phase 6 promotion gate hard-stops only when `report_type == "deep"` AND `target_words >= 8000` AND `allow_short != true` AND `prose_density: standard` — at `target_words < 8000` (including the new 5K default), under `prose_density: executive`, or with `allow_short: true`, the gate is advisory |
| `prose_density` | string | `standard` (field omitted from config when default) | Composable orthogonal knob added in v0.8.0 — independent of depth (`report_type`) and length (`target_words`). `standard` (default, today's behaviour) treats `target_words` as a floor, applies the "Cite aggressively" 2–3-citations-per-paragraph cadence, and runs the Phase 4.5 expansion re-dispatch + Phase 5 word-deficit loop on shortfalls. `executive` (v0.8.0+) treats `target_words` as a ceiling, switches the writer to Pyramid Principle + BLUF structure with **one citation per claim**, and inverts the orchestration: the Phase 4.5 floor-trigger becomes a ceiling check (no re-dispatch unless suspected truncation, `< 50%` of ceiling) and the Phase 5 word-deficit expansion loop is suppressed because the reviewer emits `Word excess` instead of `Word deficit` — the loop's `startswith("Word deficit")` predicate cannot match. The reviewer's Word Count Gate inverts its stepped completeness cap to fire on **excess** under `executive`: `1.02 < ratio ≤ 1.10` caps at 0.75, `1.10 < ratio ≤ 1.25` at 0.60, `> 1.25` at 0.45 — mirror-symmetric around 1.0 to the standard deficit caps. Per-arc compatibility is declared in `references/story-arcs.json arcs[<arc>].compatible_densities` (missing field defaults to `["standard"]` so future arcs that can't survive compression stay opt-in); both v1 arcs declare `["standard", "executive"]`. Legacy projects without the field default to `standard` at every read site — no migration needed. Phase 1.5a is the single resolution site; Phase 4 writer/reviewer dispatch, Phase 4.5 gate, Phase 5 loop guard, and Phase 6 summary all consume the pinned value. The canonical use case is `report_type=deep, target_words=4000, prose_density=executive` — the broadest curated sources distilled into a concise, scannable deliverable. Orthogonal to `tone`: tone is rhetorical register (concise / persuasive / narrative / executive — register only), density is structural ceiling + Pyramid + citation cadence; they compose orthogonally |
| `story_arc_id` | string | `standard-research` (field omitted from config when default) | Selects the section structure of the report. `standard-research` (default, today's behaviour) generates sections dynamically from sub-questions per the report-type template. A named arc (e.g., `corporate-visions`) bypasses the report-type template and structures the report around the arc's fixed elements at fixed proportions of `target_words`. The registry of valid arcs and their per-arc constraints (`compatible_report_types`, `supported_languages`, `compatible_densities`, `min_target_words`, `max_target_words`) lives in `references/story-arcs.json` — kept aligned with cogni-narrative's upstream definitions via the `audit-arcs` skill. v1 ships two arcs: `standard-research` (universal) and `corporate-visions` (Why Change → Why Now → Why You → Why Pay; restricted to `report_type ∈ {detailed, deep}` and `output_language ∈ {en, de}`). Phase 4 writer dispatch and Phase 5 reviewer dispatch both consume this field via the Phase 1.5a plan object. The reviewer's Arc-Structural Gate runs only when `story_arc_id` is set to a non-default arc — for `standard-research`, the gate is skipped and the writer's existing per-report-type templates apply unchanged (full backward compatibility). Orthogonal to `report_type`, `target_words`, `prose_density`, `tone`, `output_language`, and `citation_format` |
...
- Web research uses WebSearch + WebFetch (no MCP search providers), with market-localized search (intent-based bilingual routing via the merged market config — local + English queries against per-market authorities), optional source URL pre-fetch, and domain filtering
