...
**Preserve markdown citation syntax — do not collapse to plain text, do not regress to double-bracket `[[N]]`.** This rule applies to **every** revision path — default-mode targeted rewrites, expansion-mode deepening, factual corrections, and structural improvements alike. Plain-texting is a regression regardless of which phase of Phase 2 triggered the rewrite. When rewriting any sentence that contains a citation, the revised sentence must keep the form intact, matching the project's configured `CITATION_FORMAT`: `([Author, Year](https://...))` for apa/mla/harvard; `([Author, Year](../02-sources/data/src-*.md))` for local-wikilink; `<sup>[N](url)</sup>` for chicago/ieee/wikilink-alias (or `<sup>[N]</sup>` for sources with no URL). Collapsing to plain text `(Publisher, Year)`, dropping the URL portion, or producing the legacy broken hybrid `[[N]](#ref-N)` counts as a quality failure **regardless of word count, citation density, or cross-reference count**. This is issue #48 — the revisor silently plain-texted 37/37 citations across two iterations while preserving density, because density alone is blind to link loss. Before returning the revised draft, grep it for any inline citation matching the format's link pattern — if the count dropped relative to the pre-revision draft, fix the regressed citations before writing. Also grep for `\[\[\d+\]\]` (double-bracket numeric) — any match is the v0.8.x Obsidian-breaking drift and must be converted to the format's correct shape before the revised draft is written. For `apa` / `mla` / `harvard` / `local-wikilink`, the pattern is `\(\[.*\]\(.*\)\)`; for `chicago` / `ieee` / `wikilink`-alias, it is `<sup>\[\d+\](\(.*\))?</sup>` (the URL portion is optional only for URL-less sources).
