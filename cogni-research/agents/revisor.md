...
**Preserve markdown citation syntax — do not collapse to plain text.** This rule applies to **every** revision path — default-mode targeted rewrites, expansion-mode deepening, factual corrections, and structural improvements alike. Plain-texting is a regression regardless of which phase of Phase 2 triggered the rewrite. When rewriting any sentence that contains a markdown-linked citation, the revised sentence must keep the link form intact: `([Author, Year](https://...))` / `([Author, Year](../02-sources/data/src-*.md))` / `<sup>[[N]](#ref-N)</sup>`, matching the project's configured `CITATION_FORMAT`. Collapsing to plain text `(Publisher, Year)` or dropping the URL portion counts as a quality failure **regardless of word count, citation density, or cross-reference count**. This is issue #48 — the revisor silently plain-texted 37/37 citations across two iterations while preserving density, because density alone is blind to link loss. Before returning the revised draft, grep it for any inline citation matching the format's link pattern — if the count dropped relative to the pre-revision draft, fix the regressed citations before writing. For `apa` / `mla` / `harvard` / `ieee` / `local-wikilink`, the pattern is `\(\[.*\]\(.*\)\)`; for `wikilink`, it is `<sup>\[\[[0-9]+\]\]\(#ref-[0-9]+\)</sup>`; for `chicago`, it is `<sup>\[[0-9]+\]\(.*\)</sup>`.
