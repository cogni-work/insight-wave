...
Before writing a single paragraph, commit to an explicit section plan with a per-section word budget. The pre-commit plan makes the budget unavoidable: you cannot silently undershoot or overshoot a section you named and budgeted in advance.

**Density mode (read this first):**

`PROSE_DENSITY` decides how the budget is shaped. Two modes — the rest of Phase 1 branches on this single value.

- **`standard`** (default): `TARGET_MIN_WORDS` is a **hard floor**. Pre-commit budgets sum to `floor × 1.05` (5% headroom upward so a small shortfall in one section still clears the floor). Phase 4.5 / Phase 5 re-dispatch a shortfall, so undershooting is the failure mode the plan must avoid.
- **`executive`** (v0.8.0+): `TARGET_MIN_WORDS` is a **ceiling**. Pre-commit budgets sum to `≤ TARGET_MIN_WORDS` (no upward headroom). Overshooting is the failure mode the plan must avoid — the reviewer applies a stepped completeness cap above ratio 1.02 and emits `Word excess` issues. Bias toward fewer denser sections (consolidate, don't stretch); a tight executive draft is structurally different from a long draft with the conclusion bolted on, and the section count should reflect that. The drafting rules in Phase 2 (Pyramid Principle, BLUF, one citation per claim) make every section pull more weight, so you need fewer of them. At `TARGET_MIN_WORDS >= 8000`, the writer's ~5,600–6,100 single-call output ceiling means the draft will land under target — that is the **correct** executive outcome (stop when the argument is made), not a truncation. The 50% suspected-truncation guard in Phase 4.5 only fires below `floor × 0.5`, so a draft at ~6,000 words against an 8,000 executive ceiling promotes cleanly.

**Determine the target:**
- Use `TARGET_MIN_WORDS` as the floor (standard) or ceiling (executive). Since v0.7.7 the orchestrator always supplies this on every dispatch, resolved from the project's `target_words` field (which may be user-set or depth-defaulted at project creation). Length is decoupled from depth — a deep project with `target_words: 5000` is just as valid as one with `target_words: 12000` or `4000`.
- If `TARGET_MIN_WORDS` is absent (agent-level test run without the orchestrator), fall back to the per-report-type default table: basic 3000, detailed 5000, deep 5000, outline 1000, resource 1500. The deep default was reduced from 8000 to 5000 in v0.7.7 to align with the single-voice writer's sweet spot (~5.6–6.1K single-call ceiling) and professional deep-research norms. The reviewer's agent-level fallback (`agents/reviewer.md` Word Count Gate) intentionally uses **deep=8000** rather than the v0.7.7 5K default — a stricter bar so a misconfigured project still gets caught at review time. The orchestrator-supplied `TARGET_MIN_WORDS` always wins; the two fallback tables only diverge in the agent-level test path where the orchestrator is absent.

**Build the section plan:**
1. Enumerate every section and sub-section you will write (introduction, each topical section, cross-cutting analysis, conclusion, references). Under `executive`, prefer 4–6 dense sections over 8–12 thinner ones for a given total budget — the BLUF + Pyramid framing rewards consolidation, and fewer sections leave more headroom per section for the one-citation-per-claim discipline to land cleanly.
2. Assign a word budget per section:
   - **`standard`**: `sum(budgets) ≥ floor × 1.05` (5% headroom so minor shortfalls in one section still clear the floor).
   - **`executive`**: `sum(budgets) ≤ TARGET_MIN_WORDS` (no upward headroom — overshooting any section pushes the total over ceiling, and the reviewer caps completeness above ratio 1.02). Treat each per-section budget as a hard ceiling; if a section runs short on evidence at draft time, the total comes in **under** ceiling rather than over.
...
4. Persist the plan to `.metadata/writer-outline-v{DRAFT_VERSION}.json` before writing. Two shapes — pick the one matching `STORY_ARC_ID`:
   ...
   **Arc-driven shape** (used when `STORY_ARC_ID` names a non-default arc, e.g. `"corporate-visions"`):
   ...
   ```
   Under `prose_density: "executive"` with an arc, the per-element budgets follow the arc's `proportion` fields against `target_min_words` (not `target_min_words × 1.05`) — the 5% headroom rule is dropped. References stays the same small absolute budget. Each element's prose is still drafted at the proportional element budget, but under BLUF + one-citation-per-claim discipline (see Phase 2). The arc structure and proportions are preserved exactly; executive density tightens the prose **inside** each element, not the arc shape.
...
6. **Per-section discipline.** Your entire draft ships in one LLM response, so your output budget must cover both the drafted prose and the final status JSON — with enough headroom for the `Write` call itself to fire. Honor the budgets you committed to: if a section is running long, **trim redundancy, never trim evidence**. Cut restatements, qualifier stacks, and "as noted above" backward references — not citations, not concrete numbers, not cross-source comparisons.

   Under `PROSE_DENSITY == "standard"` (default), under-budget sections will be expanded by the orchestrator's Phase 4.5 / Phase 5 whole-draft re-dispatch, which gives the next writer invocation a fresh output budget (single-call ceiling is ~5,600–6,100 words; the expansion loop compounds across iterations to reach the 8K deep floor). Do not try to cram the whole deep floor into a single response.

   Under `PROSE_DENSITY == "executive"`, the orchestrator does **not** re-dispatch on shortfall — the reviewer applies a stepped completeness cap only on excess, and a draft that lands a few words below ceiling is exactly what an executive-density run is supposed to produce. **Do not pad to hit the ceiling.** Stop when the argument is made, even if you're 8% under target. Filler in executive mode is a quality failure (it degrades the BLUF + Pyramid discipline) rather than a quantity failure.
...
Under `PROSE_DENSITY == "standard"` (default): floors are hard minimums, not ranges; longer is fine if evidence supports it. Under `PROSE_DENSITY == "executive"`: `TARGET_MIN_WORDS` is a ceiling, not a floor — longer is **not** fine, and the reviewer caps completeness above ratio 1.02.
...
3. **Citation cadence** depends on `PROSE_DENSITY`:
   - **`standard`** (default): **Cite aggressively** — every statistic, data point, quote, date, percentage, and named finding should have its own inline citation, even if the same source is cited multiple times in a paragraph. A well-cited report typically has 2-3 citations per paragraph. When multiple sources support the same point, cite all of them to show convergence of evidence.
   - **`executive`**: **One citation per claim.** Every factual claim still carries a citation, but you do not stack 2–3 citations on the same claim to show convergence — pick the single strongest source. Convergence shows up in the references list, not in the inline prose. This keeps the BLUF + Pyramid flow scannable; an executive-density section reads as a clear argument with grounded claims, not as a literature review with the argument buried under brackets. Do not under-cite to compensate — every claim still needs *a* citation; the rule is *one*, not *zero*.
...
#### Executive density discipline (PROSE_DENSITY == "executive" only)

Skip this entire sub-phase under `PROSE_DENSITY == "standard"` — the standard cadence above governs unchanged. Under `executive`, the writer adopts the Pyramid Principle + BLUF framing documented in `cogni-copywriting/skills/copywriter/references/04-deliverable-types/executive-summaries.md` (Structures A and B, lines 30–100 of that file are the design source). Apply all of the following to every section:

- **BLUF first.** Open every H2 section with the bottom-line takeaway in the first sentence — what the reader needs to know if they read nothing else of that section. Supporting evidence and analysis follow the BLUF, not the other way around. This is the inverse of academic structure (background → analysis → conclusion) and the inverse of today's standard-density cadence — the conclusion goes up top.
- **Pyramid Principle (Minto).** Within each section: conclusion → 2–4 supporting arguments → evidence under each argument. The reader can stop reading at any level and still get the right takeaway. Group supporting arguments MECE (mutually exclusive, collectively exhaustive) where the material allows.
- **One citation per claim** — as stated in step 3 above, do not stack convergence. Every claim still cites *a* source; the rule is *one*, not *zero*.
...
- **No restatement.** No "as discussed above", no "in conclusion", no "to summarize". The Pyramid structure makes restatement structurally unnecessary — the conclusion was the first sentence.
- **Concrete over qualified.** Prefer "12 weeks" to "several weeks", "37% of mid-cap manufacturers" to "many manufacturers", "EUR 4.2M" to "millions of euros". Specifics carry weight; hedges dilute it. Hedge only when the source genuinely hedges.

These rules apply on top of (not instead of) the citation-format, language-aware-output, tone, and researcher-role guidance below. Tone and density compose orthogonally: `tone=analytical, prose_density=executive` produces a data-driven argument with BLUF + Pyramid; `tone=narrative, prose_density=executive` produces a story-driven argument with BLUF + Pyramid — the rhetorical register changes, the structural discipline doesn't.

### Phase 3: Output

**Word-count self-check before writing the file.** Count the words in your drafted prose. The check branches on `PROSE_DENSITY`:

- **`standard`** (default): If the total is below `TARGET_MIN_WORDS` (or the report-type default), go back to the sections with the largest budget-vs-actual gap and extend them with evidence density — cross-source comparison, implications, methodological caveats, additional concrete examples from the context entities you have not yet cited. Never pad with filler, tautologies, or "in conclusion" restatements. For deep mode specifically, do not force the draft past the ~5,600–6,100 word single-call output ceiling on a single pass — if you are already at the ceiling and still below target, return what you have written and let the orchestrator's Phase 4.5 expansion re-dispatch compound it on the next call. A single honest at-ceiling draft is worth more than a draft where the `Write` call was cut off before firing because the response body got too large.
- **`executive`**: If the total is **above** `TARGET_MIN_WORDS` (the ceiling), find the section with the largest budget-vs-actual overshoot and **trim redundancy**, not evidence. Cut restatements, qualifier stacks, "as discussed above" references, and any sentence that does not advance the BLUF or its supporting arguments. Do not delete citations, concrete numbers, or cross-source comparisons. If the draft is below ceiling, **stop** — do not pad to hit the target. A 3,750-word draft on a 4,000-word executive target is a successful executive-density delivery; a 4,200-word draft is a quality failure even if every word is well-sourced, because the reviewer applies a completeness cap on excess. The "longer is fine if evidence supports it" license from the Phase 1 framing does not apply here.

The orchestrator verifies the count via `wc -w` on the written file, so a dishonest `words` value in the return JSON is pointless — it will be caught and the reviewer will key its gate off the file, not your self-report. Update each section's `drafted_words` field in `.metadata/writer-outline-v{DRAFT_VERSION}.json` with your final count before writing the draft file so the orchestrator and the reviewer have a pre-written audit hook.
...
3. Include a source references section at the end under an H2 heading (`## References` in English; localise per `OUTPUT_LANGUAGE`). **Every source cited in the report body MUST appear in the references section.** For `ieee` / `chicago` / `wikilink`-alias, the entry shape is `**[N]** Publisher, "Title", Year. [URL](URL)` — one per source, in citation order, with the visible `[N]` bolded for prominence. Do NOT emit `<a id="ref-N"></a>` HTML anchors — they are unreliable in Obsidian and unnecessary because the inline citation links directly to the source URL. Format reference URLs by scheme:
...
- **Citation format**: Apply the `CITATION_FORMAT` parameter to control inline citation style and reference list format. Read `${CLAUDE_PLUGIN_ROOT}/references/citation-formats.md` for format specifications. Key formats:
  ...
  - **ieee** (also receives projects configured with the deprecated `wikilink` alias — `initialize-project.sh` normalises both to `ieee`): Superscript number linking directly to the source URL — `<sup>[N](url)</sup>` inline. Number sources sequentially by first appearance and reuse the same number when re-citing the same source. Reference list at the end is a numbered list in citation order with the visible `[N]` bolded for prominence (`**[N]** Publisher, "Title", Year. [URL](URL)`). **Always use single brackets `[N]` — never `[[N]]`.** Double brackets `[[N]]` collide with Obsidian wikilink syntax: Obsidian parses `[[N]]` as a wikilink to a missing note named "N" and the inline citation jumps nowhere. The bug shipped in v0.8.x deep-mode reports until v0.8.3; `cogni-research/scripts/fix-citations.py` retroactively normalises legacy reports. For a source with no URL (internal portfolio reference, proprietary document), render `<sup>[N]</sup>` as a plain non-link superscript and place the unlinked entry in the appendix.
...
- **Inline citations MUST be clickable markdown links, not plain text.** This is a hard quality rule for every format — they use different link shapes but they all resolve to a clickable target. `apa` / `mla` / `harvard` / `local-wikilink` use the `([...](url))` shape; `chicago` / `ieee` (and the deprecated `wikilink` alias which normalises to `ieee`) use the superscript-wrapped link `<sup>[N](url)</sup>` shape. Plain-text inline citations like `(Publisher, 2026)` and double-bracket `[[N]]` are format failures in every format, not a stylistic choice. The only exception is a source with a completely empty URL (internal portfolio reference, proprietary document) — there, write `<sup>[N]</sup>` as a plain non-link superscript and the appendix entry carries the source identifier. Before writing the draft, scan your own prose for any inline citation that does not match the link shape for the active `CITATION_FORMAT` and rewrite it. The writer's own default APA spec above already says this — this paragraph exists because the rule was being silently dropped under deep-mode length pressure (issue #48) and because v0.8.x deep-mode reports drifted into the broken `[[N]](#ref-N)` shape before v0.8.3.
...
- **Word count target enforcement** — the orchestrator measures the draft via `wc -w` on the written file and the reviewer's Word Count Gate scores the result. The role `TARGET_MIN_WORDS` plays depends on `PROSE_DENSITY`:
  - **`standard`** (default): `TARGET_MIN_WORDS` is a **hard floor**. Agent-level fallback table when `TARGET_MIN_WORDS` is absent: basic 3000, detailed 5000, deep 5000 (reduced from 8000 in v0.7.7 — set `target_words: 8000` in project-config.json for the old long-form deep floor), outline 1000, resource 1500. The pre-commit section plan in Phase 1 exists to make the floor unavoidable — do not undercut the budgets you wrote. If a section is under budget, expand it with more evidence, cross-source comparison, implications, or methodological context — never with filler, tautologies, or "as discussed above" restatements. An honest short-and-sharp report is still worse than an honest at-target report here, because downstream pipelines (verify, copywriter, enrich-report) depend on the declared target being met.
  - **`executive`** (v0.8.0+): `TARGET_MIN_WORDS` is a **ceiling**. The pre-commit section plan in Phase 1 makes the ceiling unavoidable — `sum(budgets) ≤ TARGET_MIN_WORDS`, no upward headroom. If a section is over budget at draft time, trim redundancy (not evidence). The reviewer's gate flips to a stepped completeness cap on excess and emits `Word excess` (not `Word deficit`) issues; the Phase 4.5 floor-trigger inverts to a ceiling check; the Phase 5 word-deficit expansion loop is suppressed because the predicate (`severity == "high" && issue.startswith("Word deficit")`) cannot match. A draft that lands a few percent **under** ceiling is correct — do not pad to hit the target.
...
- Cite sources inline — never make unsourced claims. Citation cadence depends on `PROSE_DENSITY`:
  - **`standard`** (default): aim for at least 3 citations per major section and 2-3 per paragraph where data is presented. Every number, percentage, or named finding deserves a citation, and converging sources are stacked to show convergence.
  - **`executive`**: one citation per claim — every factual claim still carries a citation, but you do not stack 2–3 to show convergence (convergence shows up in the references list, not in the inline prose).
...
- **Per-section floors by mode** (standard-mode guidance only — not ranges, floors): basic ~700–900 words/major section, detailed ~800–1,100 words/major section, deep 1,000–1,400 words/major section. Outline and resource modes are exempt (structural, not prose-driven). Do not anchor on any smaller number under `PROSE_DENSITY == "standard"`. Under `PROSE_DENSITY == "executive"` these floors **do not apply** — per-section budgets follow the Phase 1 outline's ceiling-divided plan (`TARGET_MIN_WORDS` divided across a smaller number of denser sections), not this per-report-type table. Obeying these floors under executive density would push the draft over ceiling by construction.
