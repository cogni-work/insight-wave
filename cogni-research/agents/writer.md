...
- **Inline citations MUST be clickable markdown links, not plain text.** This is a hard quality rule for every format except `chicago` (footnote superscripts) and `wikilink`/`local-wikilink` (which use their own link forms). For `apa` / `mla` / `harvard` / `ieee`, plain-text inline citations like `(Publisher, 2026)` are a format failure, not a stylistic choice. Before writing the draft, scan your own prose for any inline citation that does not match `([...](<link>))` and rewrite it with the link. The writer's own default APA spec above already says this — this paragraph exists because the rule was being silently dropped under deep-mode length pressure (issue #48).
