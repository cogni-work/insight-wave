...
**Research Pipeline Metrics (Phase 5 Step 4.5.0):**

| Variable | English (en) | German (de) |
|----------|--------------|-------------|
| `HEADER_FULL_PIPELINE_METRICS` | Research Pipeline Metrics | Forschungspipeline-Metriken |
| `MSG_PIPELINE_INTRO` | This research synthesized evidence through a 12-stage entity pipeline: | Diese Forschung synthetisierte Evidenz durch eine 12-stufige Entitätspipeline: |
...
| `ENTITY_DOMAIN_CONCEPTS` | Domain Concepts | Domänenkonzepte |
| `ENTITY_MEGATRENDS` | Megatrends | Megatrends |
| `ENTITY_SOURCES` | Sources | Quellen |
| `ENTITY_PUBLISHERS` | Publishers | Publisher |
| `ENTITY_CITATIONS` | Citations | Zitate |
| `ENTITY_CLAIMS` | Claims | Belege |
| `ENTITY_TRENDS` | Trends | Trends |
| `ENTITY_DIMENSION_SYNTHESES` | Dimension Syntheses | Dimensionssynthesen |
...
### 06-megatrends (Enhanced TIPS Structure)

Additional headers for enhanced megatrend content with executive summary, horizon-specific solutions, key findings summaries, and cross-references.

| Variable | English (en) | German (de) |
| -------- | ------------ | ----------- |
| `HEADER_SUMMARY_AT_GLANCE` | Summary at a Glance | Zusammenfassung auf einen Blick |
| `LABEL_WHAT` | What | Was |
| `LABEL_WHY_IMPORTANT` | Why Important | Warum wichtig |
| `LABEL_ACTION_HORIZON` | Action Horizon | Handlungshorizont |
| `LABEL_EVIDENCE_STRENGTH` | Evidence Strength | Evidenzstärke |
| `LABEL_TOP_RECOMMENDATION` | Top Recommendation | Top-Empfehlung |
| `HEADER_IMMEDIATE_ACTIONS` | Immediate Actions | Sofortmaßnahmen |
| `HEADER_STRATEGIC_INITIATIVES` | Strategic Initiatives | Strategische Initiativen |
| `HEADER_PREPARATION_PHASE` | Preparation Phase | Vorbereitungsphase |
| `HEADER_IMPLEMENTATION_PHASE` | Implementation Phase | Implementierungsphase |
| `HEADER_SCALING_PHASE` | Scaling Phase | Skalierungsphase |
| `HEADER_MONITORING_ACTIVITIES` | Monitoring Activities | Beobachtungsaktivitäten |
| `HEADER_PILOTING_CRITERIA` | Piloting Criteria | Pilotierungskriterien |
| `HEADER_OPTION_SECURING` | Option Securing | Optionssicherung |
| `LABEL_SUCCESS_INDICATORS` | Success Indicators | Erfolgsindikatoren |
| `LABEL_RESOURCES` | Resources | Ressourcen |
| `LABEL_DEPENDENCIES` | Dependencies | Abhängigkeiten |
| `HEADER_RELATED_ENTITIES` | Related Entities | Verknüpfungen |
| `HEADER_RELATED_CONCEPTS` | Related Concepts | Verwandte Konzepte |
| `HEADER_RELATED_MEGATRENDS` | Related Megatrends | Verwandte Megatrends |
| `HEADER_RELATED_TRENDS` | Related Trends | Zugehörige Trends |
| `LABEL_TOP_FINDINGS` | Top Supporting Findings | Top-Erkenntnisse |
| `LABEL_QUALITY` | Quality | Qualität |
| `TH_DIMENSION_METRIC` | Dimension | Dimension |
| `TH_SCORE` | Score | Score |
| `TH_DESCRIPTION` | Description | Beschreibung |
| `LABEL_SOURCE_COUNT` | Source Count | Quellenzahl |
| `LABEL_RECENCY` | Recency | Aktualität |
| `LABEL_CONSISTENCY` | Consistency | Konsistenz |
| `VALUE_INDEPENDENT_SOURCES` | independent sources | unabhängige Quellen |
| `VALUE_SOURCES_RECENT` | of sources < 12 months old | der Quellen < 12 Monate alt |
| `VALUE_HIGH_AGREEMENT` | High agreement across sources | Hohe Übereinstimmung der Quellen |
| `VALUE_MEDIUM_AGREEMENT` | Medium agreement across sources | Mittlere Übereinstimmung der Quellen |
| `VALUE_LOW_AGREEMENT` | Low agreement across sources | Geringe Übereinstimmung der Quellen |
| `LABEL_SYNERGY` | Synergy | Synergie |
| `LABEL_TENSION` | Tension | Spannung |
| `LABEL_ENABLER` | Enabler | Enabler |
| `MSG_KEY_FINDINGS_INTRO` | The following findings provide the strongest evidence for this megatrend: | Die folgenden Erkenntnisse liefern die stärkste Evidenz für diesen Megatrend: |
