...
**synthesis_files must contain**: `report`, `evidence`
...
### Per-Agent Filesystem Validation

Added in v2.8.0 (Step 2.3.5b) and v2.9.0 (Step 3.5.1b) of phase-3-parallel-findings-creation.md

After EACH findings-creator agent completes, validate the self-reported count against filesystem state:

#### Validation Steps

1. **Self-Report Check**: Parse `{"ok":true,"f":N}` to extract claimed count
2. **Filesystem Check**: Count files matching `batch_ref` pattern for this question
3. **Cross-Validate**: Compare self-report vs filesystem
4. **Action on Mismatch**: Queue for retry if delta > 50%

#### Failure Types

| Type                    | Description                                    | Trigger                             |
| ----------------------- | ---------------------------------------------- | ----------------------------------- |
| `self_report_mismatch`  | Agent claims N findings but M exist (M < N/2)  | Immediate retry (Step 2.3.5b)       |
| `zero_findings`         | Agent claims success but 0 findings exist      | Reconciliation retry (Step 3.5.1b)  |
| `missing_batch`         | No query batch file exists for question        | Reconciliation retry (Step 3.5.1)   |

#### Why Both Checks?

- **Step 2.3.5b (Immediate)**: Catches failures at source, enables fast retry within batch
- **Step 3.5.1b (Reconciliation)**: Safety net for edge cases that pass immediate validation

See [phase-3-parallel-findings-creation.md](phase-workflows/phase-3-parallel-findings-creation.md) for implementation details.
