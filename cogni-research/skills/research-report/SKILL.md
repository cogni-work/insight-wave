...
- **Default: 1 structural review iteration.** After revision, or if the first review accepts **and its issues list contains no high-severity `Word deficit` entry**, proceed to Phase 5.5. This is the common case and costs no more than today. The `Word deficit` carve-out matters: the reviewer's Word Count Gate can emit a high-severity `Word deficit` issue alongside an `accept` verdict when the draft lands exactly at the accept threshold with a completeness cap applied (score e.g. `0.820`, ratio e.g. `0.970`). In that edge case the default-iteration short-circuit does **not** fire — the word-deficit expansion loop below does instead, because the Phase 6 promotion gate would otherwise block promotion in deep mode (and emit `⚠ Below target` warnings in other modes) on a draft Phase 5 just let through.
- **Word-deficit expansion loop**, triggered when **any iteration's** verdict JSON contains an entry in `issues[]` where `severity == "high"` AND `issue.startswith("Word deficit")` — regardless of the top-level `verdict` field. The reviewer's Word Count Gate (see `agents/reviewer.md`) can issue a `Word deficit:` high-severity issue alongside an `accept` verdict when the draft lands at the accept threshold with a completeness cap; the loop still fires in that case, because a documented orchestration contradiction — "accept verdict says ship, word-deficit issue says expand" — is exactly what this predicate resolves. When this issue is present, the revisor runs in expansion mode (see `agents/revisor.md` — the +20% cap is lifted for expansion). Without follow-up review passes, the expansion cannot be verified and the word-count gate would be a dead letter.
