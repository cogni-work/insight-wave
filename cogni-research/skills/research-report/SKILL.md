...
1. Copy final accepted draft to `output/report.md`
   ...
   - **Write deterministic Report-Metadaten footer.** Immediately after the copy, run `${CLAUDE_PLUGIN_ROOT}/scripts/write-report-metadata.sh --project-path {project_path} --target-file {project_path}/output/report.md`. The script parses `agents/revisor.md` YAML `model:` field, reads `.metadata/execution-log.json` `phases.phase_5_review.iteration_count`, formats today's date per `project-config.json` `output_language`, strips any free-form footer the writer/revisor may have emitted, and appends the canonical block. On non-zero exit or `{"success": false}` JSON, log the error to `.metadata/execution-log.json` `phases.phase_6_finalization.metadata_write_error` and continue — the report is still usable without the footer. This step replaces the hallucinated LLM-generated footer that caused issue #49 (revisor self-attributing as Haiku when it is actually Sonnet).
