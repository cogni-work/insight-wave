...
description: |
  ...
  tone, citation style, target market (13 supported: DACH, DE, FR, IT, PL, NL, ES, MX, BR,
  CN, US, UK, EU — each with per-market authority sources and intent-based bilingual search),
...
>   eu     EU composite — 10 EU-wide domains; fans out per-country (de, fr, it, pl, nl, es; mx, br, cn excluded as non-EU)
> ```
...
- **Market**: must resolve to one of the 13 canonical codes defined in `references/market-sources.json`: `dach`, `de`, `fr`, `it`, `pl`, `nl`, `es`, `mx`, `br`, `cn`, `us`, `uk`, `eu`. There is no "global" option — downstream researchers use these codes to pick authority-source profiles, and an unknown code silently falls back to the DACH `_default` profile, masking user intent.
...
- "go" / "defaults" / "start" -> accept detected + default values. **Exception**: if market was ambiguous (tier 3 in Step 1) and the user's "go" reply did not pick a market, do NOT proceed — re-render the menu with just the Market row and the note "I still need a market — pick one of: dach, de, fr, it, pl, nl, es, mx, br, cn, us, uk, eu". The script will reject a missing market anyway; catching it here keeps the conversation clean.
