...
**Plugin**: cogni-diamond (v0.1.0, 7 skills, 1 agent, 1 hook)
...
**cogni-diamond** is the orchestrator that ties the entire cogni-works ecosystem together.
...
**Getting started**: The `diamond-setup` skill launches the engagement. It asks for the
...
1. Run `diamond-setup` for a sample client engagement
...
3. Walk through the project directory structure created by `engagement-init.sh`:
   - `diamond-project.json` — engagement config, vision, phase state
...
4. Show the `diamond-project.json` file — vision class, phase states, plugin references
5. Note: if an engagement already exists, `diamond-resume` is used instead
...
4. Run `diamond-setup` and complete the engagement scaffolding
...
- cogni-diamond orchestrates — it manages state, not content
...
- `diamond-setup` is the entry point: client + context + outcome → vision class → project scaffold
...
1. Run `diamond-discover` and observe which plugins are dispatched
...
4. Run `diamond-define` — show assumption mapping and verification
...
1. Run `diamond-develop` — observe value-modeler and proposition dispatch
2. Show solution options generated in `develop/`
3. Walk through scenario planning or opportunity scoring for the engagement
4. Run `diamond-deliver` — show claims verification and business case assembly
...
Real engagements span days or weeks. cogni-diamond is built for multi-session work.

**diamond-resume** is your re-entry point. It reads `diamond-project.json` and shows:
...
1. Run `diamond-resume` on the engagement from earlier modules
...
Ask the user to:
1. Review their engagement's current status via `diamond-resume`
...
1. **Multiple choice**: What does diamond-resume show?
...
- `diamond-resume` is the re-entry point for multi-session engagements
...
**diamond-export** generates the final deliverable package by dispatching to:
...
**The full pipeline** — how cogni-diamond orchestrates the entire ecosystem:

```
diamond-setup (vision framing)
    │
...
Every plugin you learned in Courses 1-10 has a role in this pipeline. cogni-diamond
...
1. Run `diamond-export` and observe the deliverable generation
...
- `diamond-export` generates deliverables by dispatching to cogni-visual and document-skills
...
- Every cogni-works plugin has a role; cogni-diamond sequences their work
...
14. **cogni-diamond** — Double Diamond consulting orchestrator (Course 11)
...
Canvas (hypothesis) → Portfolio (messaging) → Research → Analyze → Write → Verify → Polish → Market → Sell → Present
                                              └─── cogni-diamond orchestrates all of this ───┘
```

Every step has a cogni-works plugin. cogni-diamond ties them together into structured
