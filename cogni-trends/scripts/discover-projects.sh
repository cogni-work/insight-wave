#!/bin/bash
# Discover cogni-trends projects in the workspace and project registry.
# Usage: discover-projects.sh [--json] [--root <dir>] [--register <path>] [--unregister <path>]
# Scans for tips-project.json and trend-scout-output.json under cogni-trends/ directories.
# Also checks the project registry (~/.claude/cogni-trends-projects.json) for projects
...
" "${PROJECT_DIRS[@]+"${PROJECT_DIRS[@]}"}"
  ...
  if [ ${#PROJECT_DIRS[@]} -eq 0 ]; then
    echo "No cogni-trends projects found in $SEARCH_ROOT"
  else
