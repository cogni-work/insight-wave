#!/usr/bin/env python3
"""Generate a self-contained HTML dashboard for a cogni-trends TIPS project.

Usage: python3 generate-dashboard.py <project-dir> [--design-variables <path.json>] [--theme <path-to-theme.md>]
Output: <project-dir>/output/trends-dashboard.html
...
def main():
    ...
    if not os.path.isfile(os.path.join(project_dir, "tips-project.json")):
        print(json.dumps({"error": f"Not a cogni-trends project (missing tips-project.json): {project_dir}"}))
        sys.exit(1)
    ...
    output_path = os.path.join(output_dir, "trends-dashboard.html")
