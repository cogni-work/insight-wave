...
agents/              Autonomous rendering agents (brief -> output)
  ...
  enrich-report.md     Orchestrates the enrich-report skill (report → themed HTML). Dispatches report-html-writer (Phase 4) and enriched-report-reviewer (Phase 5b).
  report-html-writer.md  Worker agent (opus) — writes the complete self-contained HTML file from source markdown, enrichment-plan.json, and design-variables.json. Produces Chart.js charts, inline SVG concept diagrams, sidebar navigation, and full prose preservation. Runs the Python post-processor for infographic injection and content validation. Dispatched by enrich-report Phase 4.
...
| Agents | 19 | story-to-slides, pptx, html-slides, slides-enrichment-artist (worker), story-to-web, web, story-to-storyboard, storyboard, story-to-infographic, render-infographic-sketchnote (opus), render-infographic-whiteboard (opus), render-infographic-pencil (opus), editorial-sketch (worker, one-color line-art for editorial sketches), enrich-report, report-html-writer (worker, opus — HTML assembly for enrich-report Phase 4), enriched-report-reviewer (worker, visual quality review via Browser MCP), concept-diagram (worker, Excalidraw fallback), concept-diagram-svg (worker, default inline SVG), brief-review-assessor |
