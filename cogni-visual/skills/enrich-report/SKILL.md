...
The agent uses Pencil MCP tools (`open_document`, `batch_design`, `export_nodes`) to render the brief as a pixel-precise editorial data page. The PNG is exported at 2x scale for retina clarity.
...
When `--infographic-image` points to the Pencil-rendered PNG, the script embeds it as a base64 image with a click-to-fullscreen lightbox. If the PNG doesn't exist (Pencil was unavailable), the script falls back to the HTML infographic from `--infographic-data`.
