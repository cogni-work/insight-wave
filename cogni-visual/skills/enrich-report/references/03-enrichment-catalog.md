...
**SVG pattern:** 4-column flow with color-coded boxes per TIPS dimension. See `svg-patterns.md` (in libraries/) for element recipes.
...
**SVG pattern:** Central-radiating layout with theme nodes and labeled connections. See `svg-patterns.md` (in libraries/).
...
**SVG pattern:** Linear flow with color-coded step boxes connected by arrows. See `svg-patterns.md` (in libraries/) "process-flow" pattern.
...
**SVG pattern:** Varies by concept type. See `svg-patterns.md` (in libraries/) for common concept patterns (layered stack, convergence arrows, phase progression, 2x2 matrix).
