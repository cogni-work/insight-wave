...
/* Full-bleed editorial spread: caps at 1080 px (the Pencil design's native
   width) and centers in the viewport via a negative-margin trick. The fragment
   inside is now responsive (proportional columns after _load_infographic_html_fragment
   rewrites the hard-coded pixel widths), so this also reads cleanly on narrower
   viewports. Magenta rule top + bottom + cream surface keep the editorial
   discipline of the Pencil design (rule-above, rule-below, surface fill). */
.infographic-breakout {{
  --bleed-width: min(1080px, calc(100vw - 32px));
  --bleed-overshoot: calc((var(--bleed-width) - 100%) / 2);
  max-width: none;
  width: var(--bleed-width);
  margin-left: calc(0px - var(--bleed-overshoot));
  margin-right: calc(0px - var(--bleed-overshoot));
  padding: 40px 32px;
...
/* Sidebar-aware constraint: keep the bleed clear of the 240 px sidebar. */
@media (min-width: 1025px) {{
  .infographic-breakout {{
    --bleed-width: min(1080px, calc(100vw - 240px - 48px));
  }}
}}
...
def _load_infographic_html_fragment(html_path, output_dir):
    ...
    Defensive behavior (v0.16.12+):
    - If `html_path` is empty or missing, auto-detect the fragment at
      `{source_root}/cogni-visual/infographic-fragment.html` where source_root
      is one directory above output_dir (typical project layout). This makes
      Tier 1 robust against orchestrator bugs that drop --infographic-html.
    - Strip hard-coded `flex: 0 0 NNNpx` inline column widths so the fragment's
      rows reflow within whatever container width the breakout provides
      (the fragment was authored for 1080 px and would otherwise collapse).
    - Base64-embed `<img src="./.sketches/...">` references so the report stays
      a single self-contained file when served from a sibling output/ folder.
    """
    # Defensive auto-detect: if path missing, try the canonical project layout
    if not html_path or not os.path.isfile(html_path):
        candidate = os.path.join(os.path.dirname(output_dir) or output_dir,
                                 'cogni-visual', 'infographic-fragment.html')
    ...
    # Make hard-coded fixed-width column overrides responsive without flipping
    # the design's intended ratios. The fragment authors columns with inline
    # style="flex: 0 0 NNNpx;" at the Pencil page's native ~1000 px row width;
    # in a narrower breakout container these widths cause the sibling `flex: 1`
    # columns to collapse (e.g. the "WHY THIS DATE" or "THE BUYER HAS CHANGED"
    # paragraphs shrink to one word per line). The fix:
    #   - Keep the pixel basis (so the column TRIES to honor the design intent),
    #   - Allow shrinking (flex-shrink: 1 instead of 0),
    #   - Cap at a percentage of the row width so siblings always get a fair
    #     share when the container is much narrower than 1000 px. The percentage
    #     is derived from the design's native ratio (e.g. 560/1000 = 56 %,
    #     216/1000 = 22 %). Clamped to [15, 70] to keep both extremes sane.
    # Result: in a 1000 px row, the column gets exactly its designed pixel width;
    # in a 700 px row, the column shrinks proportionally and the sibling keeps a
    # readable share.
    def _fixedpx_to_responsive(match):
        px = int(match.group(1))
        max_pct = max(15, min(70, round(px / 10)))  # 560 -> 56, 580 -> 58, 216 -> 22
        return f'flex: 0 1 {px}px; max-width: {max_pct}%;'
    fragment = re.sub(r'flex:\s*0\s+0\s+(\d+)px\s*;', _fixedpx_to_responsive, fragment)
    # Also rewrite the class-level sketch-col rule, which uses the same
    # `flex: 0 0 216px` pattern and otherwise stays unresponsive.
    fragment = re.sub(
        r'(\.infographic-pencil-fragment\s+\.sketch-col\s*\{[^}]*?)flex:\s*0\s+0\s+(\d+)px\s*;',
        lambda m: f'{m.group(1)}flex: 0 1 {m.group(2)}px; max-width: {max(15, min(70, round(int(m.group(2)) / 10)))}%;',
        fragment
    )

    # Base64-embed sketch images referenced as ./.sketches/* — they live alongside
    # the fragment at {fragment_dir}/.sketches/* but the report HTML is served
    # from a different folder, so relative paths break. Inlining keeps the report
    # self-contained (consistent with the PNG path's base64 embed behavior).
    import base64
    fragment_dir = os.path.dirname(os.path.abspath(html_path))
    def _embed_sketch(match):
        prefix, src, suffix = match.group(1), match.group(2), match.group(3)
        # Resolve relative to fragment_dir, supporting ./, ../, or no prefix.
        sketch_path = os.path.normpath(os.path.join(fragment_dir, src))
        if not os.path.isfile(sketch_path):
            return match.group(0)  # leave unchanged if not found
        ext = os.path.splitext(sketch_path)[1].lower().lstrip('.')
        mime = {'png': 'image/png', 'jpg': 'image/jpeg', 'jpeg': 'image/jpeg',
    ...
    fragment = re.sub(r'(<img[^>]*\bsrc=")(\.{0,2}/\.sketches/[^"]+)(")',
                      _embed_sketch, fragment)
