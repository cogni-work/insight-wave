...
**Load libraries:** `$CLAUDE_PLUGIN_ROOT/libraries/infographic-layouts.md`, `$CLAUDE_PLUGIN_ROOT/libraries/EXAMPLE_INFOGRAPHIC_BRIEF.md`, `$CLAUDE_PLUGIN_ROOT/libraries/cta-taxonomy.md`.
...
**Output:** Data inventory (numbers, assertions, sequences, comparisons, concepts) + governing thought + arc type.
...
**Read reference:** `$CLAUDE_PLUGIN_ROOT/skills/story-to-infographic/references/02-infographic-mapping.md`
...
**Style preset selection** (if `style_preset` is `auto`):
Match style to context. The reference file has heuristics based on source type, arc type,
and audience context — e.g., trend report → data-viz, workshop material → sketchnote,
board presentation → corporate.

**Read reference:** `$CLAUDE_PLUGIN_ROOT/skills/story-to-infographic/references/03-style-presets.md`

If interactive: present layout + style recommendation via AskUserQuestion with 2-3 options.
On empty response, auto-select top recommendation.
...
transformation_notes: |
  ...
  Theme: {theme}. Style: {style_preset}. Layout: {layout_type}.
...
| Library | Purpose |
|---------|---------|
| **infographic-layouts.md** | Layout type schemas, block type catalog with field schemas and word limits |
| **EXAMPLE_INFOGRAPHIC_BRIEF.md** | Complete output format reference (stat-heavy, data-viz) |
