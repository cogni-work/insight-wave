...
- **`fix_frontmatter_defaults` reconciles a mismatched `id:`, not just a missing one** (v0.0.51, intra-plugin). `lint_wiki.py`'s `frontmatter_defaults` fixer previously only backfilled an *absent* `id:` line; a present-but-wrong `id:` (e.g. a source page whose frontmatter `id:` is a *different* source's slug — `health.py`'s `id_mismatch` hard error, which blocks `wiki-lint` without `--ignore-health`) was silently skipped, so `--fix=frontmatter_defaults` could not repair it. The id branch is now backfill-or-correct: missing → prepend `id: <slug>` (unchanged); present but ≠ filename stem → rewrite the line in place to `id: <slug>` (unquoted, matching the backfill form and `parse_frontmatter`'s quote-stripping), recorded as `id: <old> → <slug>`; already-correct → no change. The filename stem stays authoritative (`_wikilib.build_slug_index`); the fixer keeps its fresh-read-from-disk posture inside `_wiki_lock` so it still composes with `fix_reverse_link_missing`. No new `--fix` class or flag surface. Test: new crossed-id case in `tests/test_lint_fix.sh` (dry-run plan / wet rewrite / idempotency + `health.py` 0 `id_mismatch`). Closes #415; the `cogni-knowledge:source-ingester` post-write self-check guard and the gitignored local-data cleanup are deferred follow-ups.
