...
skills/                         12 wiki skills
  ...
  wiki-ingest/                    Add sources → summary page → backlink audit → log
    scripts/
      ...
      wiki_queue.py               Persistent ingest queue dispatcher (T3.1, v0.0.35+) — five operations (--enqueue, --next, --complete, --retry, --status) over `<wiki-root>/.cogni-wiki/queue/{pending,running,done,failed}/`; locked atomic state-dir transitions via os.rename + _wiki_lock
    references/
  ...
  wiki-dashboard/                 Self-contained HTML overview (pages, tags, backlink graph) + two-pass graph view (v0.0.34+, #221)
    scripts/
      render_dashboard.py         Reads wiki/ → writes wiki-dashboard.html (stdlib only)
      build_graph.py              Pass 1 (deterministic [[wikilink]] edges) + Pass 2 LLM-inferred edges (driven from SKILL.md, cached per pair) + Label Propagation communities → writes self-contained wiki-graph.html with inline canvas force-directed renderer (v0.0.34, #221)
    references/
      graph-renderer.md           Graph-renderer contract + cache schema + Pass-2 workflow + future swap-in points
...
references/
  ...
  scheduled-drainer/              T3.2 scheduled-runner deployment templates (v0.0.39+)
    README.md                       Deployment-shape selector + recommendation rationale
    github-actions.md               Canonical recommended shape (wiki-daily-ingest.yml + future wiki-daily-discover.yml)
    cloud-routine.md                Alternative for remote-but-no-GH wikis
    local-cron.md                   Alternative for on-box / air-gapped / GitHub-banned wikis (cron + launchd + systemd-timer)
    loop.md                         Dev-only in-session shape (smoke test + iteration)
```
...
| Scripts | 19 | backlink_audit.py, wiki_index_update.py, batch_builder.py, config_bump.py, _wikilib.py, convert_to_md.py, rebuild_context_brief.py, health.py, lint_wiki.py, rebuild_open_questions.py, wiki_status.sh, render_dashboard.py, build_graph.py, refresh_planner.py, extract_page_claims.py, resweep_planner.py, migrate_layout.py, prefill_foundations.py, wiki_queue.py |
...
│   ├── questions/             type: question (research-question nodes; written by cogni-knowledge:knowledge-ingest)
...
└── .cogni-wiki/
    ...
    ├── graph-cache/           Per-pair Pass-2 LLM judgement cache (v0.0.34+, #221; one JSON file per evaluated page-pair)
    └── queue/                 Persistent ingest queue (v0.0.35+, T3.1; `pending/`, `running/`, `done/`, `failed/` dirs of per-job JSON files)
```
...
type: concept | entity | summary | decision | interview | meeting | learning | synthesis | note | source | question
tags: [tag1, tag2]
created: YYYY-MM-DD
updated: YYYY-MM-DD
...
The `question` type (introduced in v0.0.50, schema_version `0.0.7`) is reserved for research-question nodes — one per sub-question of a research run — written by `cogni-knowledge:knowledge-ingest`. The page body `[[links]]` the source findings that answer it (a machine-owned `## Findings` block), each answering source links back, so the question→finding relation joins the backlink graph and satisfies the `R1` forward→reverse contract. Same additive posture as `source`: cogni-wiki only recognises the type and routes the page to `wiki/questions/`; per-type semantics (`theme_label:`, `sub_question_id:`, `search_guidance:`, the `## Findings` body convention) are owned by the ingestor, not by cogni-wiki's schema.
...
- **Append-only log.** Every ingest, query, synthesis, lint, update, prefill, graph, queue, and (as of v0.0.27) health run writes a line to `wiki/log.md` with an ISO date prefix. Operation prefixes today: `ingest`, `query`, `synthesis` (v0.0.23+, filed-back query answers vs un-filed `query` reads), `lint`, `health` (v0.0.27+, free zero-LLM preflights vs tokenful `lint` runs), `update`, `setup`, `migrate` (v0.0.28+, one-shot layout migrations), `prefill` (v0.0.33+, foundation seeding from the plugin's `foundations/` library), `graph` (v0.0.34+, two-pass graph builds), `queue` (v0.0.35+, persistent-queue events: `enqueued`, `failed`, `retried`), and `compose` / `verify` / `finalize` (v0.0.45+, write-side phases of `cogni-knowledge`'s v0.1.0 inverted pipeline running against a bound wiki — cogni-knowledge operations, not cogni-wiki's, catalogued here like the `type: source` page type because an external consumer authors them against this wiki; `knowledge-ingest` reuses `ingest`, and `knowledge-plan`/`curate`/`fetch` write no log line). The full enum is in each wiki's `SCHEMA.md` "Log format" block. All prefixes are surfaced separately in `wiki-resume` and `wiki-dashboard`. The `queue` and `compose`/`verify`/`finalize` extensions are purely additive — no `schema_version` bump required because pre-extension readers count the new lines in their catch-all bucket without crashing.
...
| `.cogni-wiki/queue/{pending,running,done,failed}/<id>.json` | Atomic state-dir transitions (`os.rename` between sibling dirs) for queue jobs | `wiki_queue.py::cmd_{enqueue,next,complete,retry}` (v0.0.35) |
...
T3.1 (v0.0.35) shipped the persistent ingest queue. T3.2 (v0.0.39) ships
the thin scheduled-runner half — four deployment shapes that periodically
call `wiki-ingest --next` against the queue so ingest no longer requires
a human at the keyboard. State stays in T3.1 (queue files + `_wiki_lock`);
the runner owns nothing.

Reference templates live under `references/scheduled-drainer/` — one file
per shape plus a selector README:

| Shape | When to pick | Recommended? |
|------|-------------|--------------|
| **GitHub Actions** (`references/scheduled-drainer/github-actions.md`) | Wiki is hosted in a git repo on GitHub | **Yes** — canonical recommendation |
| **Cloud Routine** (`references/scheduled-drainer/cloud-routine.md`) | Wiki is remote but not on GitHub (corporate-banned, GitLab-only, etc.) | No — alternative for remote-no-GH |
| **Local cron / launchd / systemd-timer** (`references/scheduled-drainer/local-cron.md`) | On-box only, GitHub-banned, or air-gapped | No — alternative for on-box-only |
| **`/loop`** (`references/scheduled-drainer/loop.md`) | Dev / iteration / smoke-testing | **No** — explicitly not for production |

The recommendation rationale (lifted from issue #212's deferred-suggestion
comment): wiki-hosted-on-GitHub + GH Actions drainer is the path that
unlocks Claude Code on the web with zero extra state machinery — the
runner is just `git pull → wiki-ingest --next → commit → push`, and
multi-runner contention is solved by git itself (rebase-and-retry on
push). No new lock primitive on top of `_wiki_lock`. Portability claim
survives — the wiki is still plain markdown, it just happens to live in
a git remote.

For wikis that can't use GitHub, Cloud Routine and local cron are the
honest alternatives — they trade off some affordances (no commit log, no
multi-runner safety) but keep the queue itself unchanged, so a wiki
migrated from local cron to GitHub Actions doesn't need to reformat any
state.

**What a scheduled drainer is allowed to do:** exactly one thing per tick —
call `wiki-ingest --next`. The dispatcher returns one of `{action: "pick", …}`
(picked a job and ran it), `{action: "noop", reason: "queue_empty"}`,
`{action: "noop", reason: "running_busy"}`, or `{action: "noop", reason: "all_scheduled_future"}`.
All four exit 0. Scheduled drainers MUST NOT call `--enqueue`, `--complete`,
or `--retry`, MUST NOT touch the queue files directly, and MUST NOT page
operators on noops.

**Drainer-hint nudge** (v0.0.39+, T3.2). `wiki-resume` rule 5a fires when
the queue is genuinely *stalled* — `queue.pending > 0`, `queue.running == 0`,
`queue.oldest_pending_age_hours > queue.drainer_hint_threshold_hours`,
AND `queue.last_next_at` is null or older than the same threshold. Default
threshold is 24 hours, overridable per wiki via `drainer_hint_threshold_hours`
in `.cogni-wiki/config.json`. The threshold + recent-activity gate
prevents nagging a user who is actively manually draining (recent
`last_next_at` keeps rule 5a silent) or who just enqueued a batch
(oldest_pending_age_hours below threshold keeps it silent). When the
rule does fire, it points the user back at `references/scheduled-drainer/`
to install one of the production shapes.
...
- **scheduled-drainer references + drainer-hint refinement** (v0.0.39, intra-plugin, T3.2 from issue #212 / #232). Four reference templates land under `references/scheduled-drainer/` — `github-actions.md` (canonical recommended shape, wiki-daily-ingest.yml + future wiki-daily-discover.yml placeholder), `cloud-routine.md` (alternative when wiki is remote-but-no-GH), `local-cron.md` (alternative when wiki is on-box / air-gapped / GitHub-banned), and `loop.md` (dev-only in-session shape). A selector `README.md` quotes the parent-issue deferred-suggestion rationale and provides the decision table. No new code surface — the runners are external schedulers calling `claude -p --bare "/cogni-wiki:wiki-ingest --next"`, all queue state stays in T3.1. `wiki_status.sh` adds three drainer-hint fields to its `queue` sub-object (`oldest_pending_age_hours`, `last_next_at`, `last_next_age_hours`) plus the `drainer_hint_threshold_hours` config-driven threshold (default 24, override via `.cogni-wiki/config.json`). `wiki-resume` rule 5a is tightened: from the v0.0.35 unconditional `pending>0 AND running==0` trigger to the v0.0.39 `pending>0 AND running==0 AND oldest_pending_age_hours > threshold AND (last_next_at is null OR last_next_age_hours > threshold)` gate, so users who are actively draining manually aren't nagged. New `tests/test_scheduled_drainer.sh` ships a `/loop`-equivalent in-process drain — bootstrap fixture wiki, enqueue 3 jobs, alternate `--next` and `--complete --success` to confirm the queue drains and noop semantics hold. SCHEMA.md operation enum unchanged. Closes #232 (T3.2 from #212).
- **persistent ingest queue spine** (v0.0.35, intra-plugin, T3.1 from issue #212). New `wiki-ingest` Mode D adds four flags — `--enqueue <source>`, `--next`, `--queue-status`, `--queue-retry <id>` — driven by a single dispatcher script `${CLAUDE_PLUGIN_ROOT}/skills/wiki-ingest/scripts/wiki_queue.py`. Jobs live as JSON files under `<wiki-root>/.cogni-wiki/queue/{pending,running,done,failed}/`; `os.rename` between sibling dirs (POSIX-atomic on the shared `.cogni-wiki/` filesystem) wrapped in `_wiki_lock` enforces single-worker semantics so two `--next` calls from separate sessions cannot pick the same job. `--next` refuses to advance while any job sits in `running/`, preserving the Karpathy invariant ("source N+1 must see source N's just-written page") by construction — the queue decouples *when* the next ingest fires from *who* is at the keyboard, not *whether* it stays strictly sequential. Failed jobs surface via `--queue-status` (`running_started_at` ISO timestamp gives operators a "is it stuck?" hint without the v0.0.35 surface needing lease/PID-file machinery — auto-recovery is T3.2 drainer territory). `wiki-resume` adds two decision-tree rules at slots 3a (`queue.failed > 0`) and 5a (`queue.pending > 0 AND queue.running == 0`), placed by the same "what blocks reliable reads first" ranking the existing rules follow. SCHEMA's log-format enum gains `queue` (additive — pre-v0.0.35 readers count `queue` lines in their catch-all bucket; no `schema_version` bump needed). New `tests/test_queue.sh` ships 14 assertions covering pre-migration probe, empty-queue noop, distinct ids on same-second enqueue, atomic pending→running pick, single-worker `running_busy` refusal, success/failure complete paths, retry round-trip, priority ordering, future-`scheduled_at` skip, **concurrent `--next` from two subshells exercising the actual `_wiki_lock` contention contract**, and error paths (retry on non-failed, complete on non-running). Closes #212 Tier 3 item T3.1; T3.2 (scheduled drainer) is the natural next promotion candidate — see issue #212 §"Tier 3" + the deferred-suggestion comment for the Cloud Routine / GH Actions / `/loop` runner-shape recommendation.
- **two-pass graph layer** (v0.0.34, intra-plugin). New `wiki-dashboard --graph yes` flag drives `build_graph.py` to render `<wiki-root>/wiki-graph.html` — a self-contained interactive graph view. **Pass 1** is deterministic: every `[[wikilink]]` resolved against `_wikilib.build_slug_index` becomes an `EXTRACTED` edge (self-loops and dangling targets dropped). **Pass 2** is LLM-driven from the SKILL.md workflow (mirrors `wiki-lint`'s Step-4 pattern — scripts never call an LLM directly): `build_graph.py --mode enumerate-candidates` emits page pairs that share tags / type / body-token Jaccard ≥ 0.20-0.35 AND aren't already covered by Pass 1 AND aren't foundation×foundation pairs AND aren't fresh cache hits; the orchestrator evaluates each via the configured model (default `sonnet`); `build_graph.py --mode record-judgement` atomically writes one cache file per pair to `<wiki-root>/.cogni-wiki/graph-cache/<sha256(slug_a|slug_b)>.json`. Cache hit requires `model_id` + `page_a_hash` + `page_b_hash` all match — model rotation or page edits invalidate naturally. Communities computed via deterministic Label Propagation (sorted-slug seeded, lower-id-wins tie-break; ~60 LOC stdlib; faithful Louvain deferred). HTML uses a custom vanilla-canvas force-directed renderer (~300 LOC inline JS) instead of vis-network — preserves the dashboard's "open offline, share as a single file" property without the ~600 KB inline UMD penalty. **No new shared-state file added to the lock table** — graph-cache files are unique-by-construction per pair (one writer per `record-judgement` call); `atomic_write` provides crash-safety. Graph build appends one `## [YYYY-MM-DD] graph | nodes=…, extracted=…, inferred=…, communities=…` line to `wiki/log.md`. New `tests/test_graph.sh` ships 8 assertions: pre-migration probe, exact Pass-1 edge count, self-loop & dangling exclusion, self-contained HTML (zero external URLs), idempotency, cache hit on re-enumerate, stale-cache invalidation on page edit, community count ≥ 2. Closes #212 Tier 2 item #5 (#221); 7 of 7 Tier 2 items now landed.
- **foundations + wiki-prefill** (v0.0.33, intra-plugin). New plugin-side `foundations/` library of curated `type: concept` pages with `foundation: true` frontmatter (Porter's Five Forces, Jobs-to-be-Done, MECE, Pyramid Principle, OODA, SWOT, BCG Matrix, Value Chain, Lean Canvas, Wardley Mapping, Double Diamond — 11 starter entries; community can extend via PR). New `wiki-prefill` skill copies a tag-filtered subset (`consulting` / `product` / `strategy` / `all`) into `wiki/concepts/`, idempotently and under `_wiki_lock`; `{{PREFILL_DATE}}` placeholders substitute to today's ISO date so the staleness clock starts at prefill time and never thereafter. `entries_count` bump goes through the locked `config_bump.py --delta N` (no new shared-state file added — pages are unique-by-construction). `wiki-update` refuses to edit `foundation: true` pages without `--force`, `wiki-lint` skips `orphan_page` / `no_sources` / `stale_page` / `stale_draft` warnings on them and surfaces `stats.foundation_count`, and `wiki-ingest`'s Step 1 detects foundation slug-collisions and routes the user to "ingest as a related page (different slug)" rather than overwriting (analogous to the re-ingest warning, but stricter). `wiki-setup` Step 6 prompts the user to prefill the consulting subset by default; the prompt is suppressed deterministically via `wiki-setup --skip-prefill-prompt`, which `wiki-from-research`'s Step 2 dispatch always passes (cold-start from a research project is already a domain-specific seeding path; layering canonical foundations on top would clutter the user's wiki). Detection of foundation pages is owned by `_wikilib.is_foundation_page(fm)` so every consumer reads the same source of truth. New `tests/test_prefill.sh` exercises `--list`, `--dry-run`, wet apply with `entries_count` correctness, idempotent re-run, lint skip-foundations, and the pre-migration probe. Closes #212 Tier 2 item #4 (#224).
- **wiki-from-research opt-in for wiki-mode deposits + project-config path fix** (v0.0.40, intra-plugin; Phase 2 seam for cogni-knowledge #264). Two changes, one bullet:
  - **Added.** `wiki-from-research` gains two additive flags — `--allow-wiki-source` and `--cycle-guard-cleared` — that together lift the default Step 0(3) / Step 1d abort on `report_source ∈ {wiki, hybrid}` projects. Default behavior is unchanged for direct users; the flag pair exists for orchestrators (today: `cogni-knowledge:knowledge-report`) that have verified no circular-evidence chain via their own cycle-guard pass. This skill does NOT re-run the guard itself — cycle semantics are caller-specific (cogni-knowledge owns the `derived_from_research:` lineage stamp it walks; a future caller may walk a different signal). Both abort sites (Mode B Step 0(3) and Mode A re-check Step 1d) get the same conditional treatment, so the parameter table now documents the flags as Mode A & Mode B (not Mode B only as originally drafted).
  - **Fixed.** Step 0(3)'s project-config read path was `<project>/project-config.json`. cogni-research moved its config to `<project>/.metadata/project-config.json` in v0.7.x — the v0.0.39 path was stale, so before this PR the Mode B `report_source ∈ {wiki, hybrid}` abort either silently never fired (file-not-found tolerated) or was unusable on any v0.7.x+ project, depending on how the LLM interpreted the missing file. The exact failure mode in practice is unverified — we have no logs from real Mode B runs on wiki/hybrid projects (the abort was the documented refusal, so users would not have tried). Direct users with existing wiki/hybrid scripts that relied on the v0.0.39 read path should audit; the new path matches `cogni-research/scripts/initialize-project.sh:487`.
- **wiki-query gains `--wiki-root`** (v0.0.41, intra-plugin; Phase 3 seam for cogni-knowledge #264). Additive flag mirroring the same `--wiki-root` override already on `wiki-resume` / `wiki-lint` / `wiki-dashboard`. When passed, Step 1 skips the upward cwd walk and uses the supplied path directly (after verifying `<wiki-root>/.cogni-wiki/config.json` exists). Default behaviour is unchanged for direct users. The flag exists so orchestrators that resolve the wiki from a manifest — notably `cogni-knowledge:knowledge-query`, which reads `wiki_path` from `.cogni-knowledge/binding.json` — can dispatch wiki-query with a pinned target rather than depending on cwd-walking from the caller's working directory. Drops the prompt-prefix shim that knowledge-query carried in v0.0.8.
- **contract-level regression tests for cogni-knowledge seams** (v0.0.42, intra-plugin; Phase 2/3 debt cleanup for cogni-knowledge #264). Two stdlib-only test scripts land under `tests/`: `test_wiki_from_research_flags.sh` (asserts the `--allow-wiki-source` / `--cycle-guard-cleared` flag pair lifts the `report_source ∈ {wiki, hybrid}` abort at Step 0(3) and Step 1d, and that the project-config read path is `.metadata/project-config.json` — the post-v0.0.40 form, NOT the stale bare-filename form), and `test_wiki_query_wiki_root.sh` (asserts `--wiki-root` exists in the parameter table and Step 1 skips the upward cwd walk when the flag is set). These are **contract-level grep tests** — both skills are pure LLM with no scripts to execute, so the tests catch the most likely regression class (a path or flag silently disappears from the SKILL.md) without pretending to verify LLM behaviour. Style matches `test_lint_health_partition.sh` / `test_prefill.sh` (bash 3.2, exit non-zero on failure). `tests/README.md` gains a "Contract tests" section listing both files and the grep-only scope. No behaviour change.
- **three alpha-findings fixes for the cogni-knowledge orchestrator chain** (v0.0.43, intra-plugin; Phase 4 alpha findings F2–F5 for cogni-knowledge #264). Three script-level bugs that block the `cogni-knowledge:knowledge-research` + `knowledge-report` end-to-end chain on real v0.7.x+ cogni-research projects.
  - **Fixed (F2).** `skills/wiki-ingest/scripts/_wiki_research.py::locate_research_project` only probed `cogni-research-<slug>/`. cogni-research's `initialize-project.sh` since v0.7.x creates `<slug>/` (when `--slug` is set) or `<slug>-<date>/` (when the slug is derived) — neither prefix shape was findable. After the legacy probes, the resolver now tries `<slug>/` exact and `<slug>-*/` prefix matches under both `wiki_root.parent` and `wiki_root`, each gated on `.metadata/project-config.json` presence so a neighbour dir that happens to share a prefix is not accepted. Failure message lists both shape sets so the user can correct the layout.
...
- **`type: source` accepted in the page-type allowlist** (v0.0.44, intra-plugin, schema_version `0.0.6`; unblocks cogni-knowledge #264 / PR #269 milestone 6 `knowledge-ingest`). One-entry extension of `_wikilib.PAGE_TYPE_DIRS` (`"source": "sources"`); `VALID_TYPES` and `iter_pages` derive from it, so `health.py` accepts the type with no further code changes. Hard-fail boundary stays at `0.0.5`; new configs advertise `"0.0.6"`. cogni-wiki owns *recognition* of the type; per-type semantics (`pre_extracted_claims:`, body convention) stay in cogni-knowledge. Test: `tests/test_source_page_type.sh`. Closes #270.
- **`wiki_index_update.py` self-cleans the wiki-setup seed placeholder** (v0.0.46, intra-plugin; cross-plugin support for cogni-knowledge #264 Slice 16 / #306). `wiki-setup` seeds `wiki/index.md` with an empty `## Categories` section holding `_No pages yet. Run \`wiki-ingest\`…_`. Script-level callers (`cogni-knowledge:knowledge-ingest` / `knowledge-finalize` call `wiki_index_update.py` directly, never the wiki-ingest LLM skill) inherited that dead placeholder forever. New pure helper `strip_seed_placeholder(text)` runs inside `update_index()`'s existing `_wiki_lock` block before the atomic write on any real slug-mode insert/update: it drops the placeholder line and the now-empty `## Categories` heading. Idempotent (no-op once gone) and confined to the exact seed string so a user's real `## Categories` heading carrying content is untouched. No schema bump (index-text cleanup only). Test: `tests/test_index_placeholder_selfclean.sh`. Closes #306.
- **`lint_wiki.py` in-process `--fix` fixers read each page fresh, not the start-of-scan snapshot** (v0.0.46, intra-plugin; surfaced reviewing the cogni-knowledge #264 Slice 16 conformance gate). `fix_synthesis_no_wiki_source` and `fix_frontmatter_defaults` previously rewrote a page from the `all_pages[slug]` snapshot captured before any fix ran; in a single `--fix=all` invocation that **clobbered** a write an earlier in-process fixer made to the same page in the shared lock — most importantly `fix_reverse_link_missing`'s `## See also` reverse-link append onto a cited source that *also* needed an `id:`/`updated:` fix, which silently un-did de-orphaning. Both now resolve the path via `slug_index` and `read_text()` fresh before editing, so the fixers compose. New `tests/test_lint_fix.sh` case plants a cited source missing `id:` and asserts one `--fix=all` pass applies BOTH the reverse link and the `id:` backfill with 0 residual `orphan_page`.
- **`wiki_index_update.py --max-summary` word-boundary clamp backstop** (v0.0.47, intra-plugin; cross-plugin support for cogni-knowledge #324). The #311 German bake-in produced `wiki/index.md` one-liners cut **mid-word** ("…Sonderka" for "…Sonderkategorien von Daten"). The cut was LLM-side — cogni-knowledge's authoring contract said "≤180 chars" and the model honored it with a hard `[:180]` slice; `wiki_index_update.py` stored the fragment verbatim (it never truncated). New shared helper `_wikilib.clamp_summary(text, max_len)` (modeled on `batch_builder.py::_truncate_title`, plus an ellipsis) collapses to one line and clamps to `max_len` codepoints on a WORD boundary, appending `…` only when it actually cuts. Wired through an **opt-in** `--max-summary <chars>` arg applied right after the existing `summary.strip()` / non-empty check; default `None` → verbatim, so every other caller (incl. cogni-wiki's own semantic `wiki-ingest` SKILL.md) is byte-identical. It is a guardrail, not a target — the real fix is semantic authoring on the cogni-knowledge side (v0.1.12), which passes `--max-summary 240` as a defensive ceiling. No schema bump (index-text only); no new script (Scripts count stays 19). Test: `tests/test_index_summary_clamp.sh`. Closes #324.
- **`type: question` accepted in the page-type allowlist** (v0.0.50, intra-plugin, schema_version `0.0.7`; unblocks cogni-knowledge #407 — sub-questions as first-class wiki nodes). One-entry extension of `_wikilib.PAGE_TYPE_DIRS` (`"question": "questions"`); `VALID_TYPES`, `iter_pages`, and `resolve_page_path` derive from it, so `health.py` (imports `VALID_TYPES`) and `render_dashboard.py` (imports `PAGE_TYPE_DIRS`, `#9ca3af` fallback for unmapped types) accept the type with no further code changes — `render_dashboard.py` additionally gains a `TYPE_COLORS` entry (`#be123c`) as polish. Mirrors the `type: source` (#270, v0.0.44) land exactly. Hard-fail boundary stays at `0.0.5`; new configs advertise `"0.0.7"`. cogni-wiki owns *recognition* of the type; per-type semantics (`theme_label:`/`sub_question_id:`/`search_guidance:` frontmatter, the `## Findings` body, the sq↔finding `R1` backlink pair) stay in cogni-knowledge (`knowledge-ingest` Step 4.5). Test: `tests/test_question_page_type.sh`. Closes #407 (Part A).
- **`fix_frontmatter_defaults` reconciles a mismatched `id:`, not just a missing one** (v0.0.51, intra-plugin). `lint_wiki.py`'s `frontmatter_defaults` fixer previously only backfilled an *absent* `id:` line; a present-but-wrong `id:` (e.g. a source page whose frontmatter `id:` is a *different* source's slug — `health.py`'s `id_mismatch` hard error, which blocks `wiki-lint` without `--ignore-health`) was silently skipped, so `--fix=frontmatter_defaults` could not repair it. The id branch is now backfill-or-correct: missing → prepend `id: <slug>` (unchanged); present but the raw value ≠ filename stem → rewrite the line in place to the canonical unquoted `id: <slug>`, recorded as `id: <old> → <slug>`; already-canonical → no change. The comparison is against the *raw* captured value (quotes intact), matching `health.py`'s `id_mismatch` key — `_wikilib.parse_frontmatter` keeps surrounding quotes, so a quoted-but-stem-correct `id: "<slug>"` is also an `id_mismatch` and is normalised to the unquoted form (`ID_LINE_RE` gained a value capture group, symmetric with `UPDATED_LINE_RE`). The filename stem stays authoritative (`_wikilib.build_slug_index`); the fixer keeps its fresh-read-from-disk posture inside `_wiki_lock` so it still composes with `fix_reverse_link_missing`. No new `--fix` class or flag surface. Test: new crossed-id and quoted-id cases in `tests/test_lint_fix.sh` (dry-run plan / wet rewrite / idempotency + `health.py` 0 `id_mismatch`). Closes #415; the `cogni-knowledge:source-ingester` post-write self-check guard and the gitignored local-data cleanup are deferred follow-ups.
