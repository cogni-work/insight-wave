...
- **Append-only log.** Every ingest, query, synthesis, lint, update, prefill, graph, queue, and (as of v0.0.27) health run writes a line to `wiki/log.md` with an ISO date prefix. Operation prefixes today: `ingest`, `query`, `synthesis` (v0.0.23+, filed-back query answers vs un-filed `query` reads), `lint`, `health` (v0.0.27+, free zero-LLM preflights vs tokenful `lint` runs), `update`, `setup`, `migrate` (v0.0.28+, one-shot layout migrations), `prefill` (v0.0.33+, foundation seeding from the plugin's `foundations/` library), `graph` (v0.0.34+, two-pass graph builds), `queue` (v0.0.35+, persistent-queue events: `enqueued`, `failed`, `retried`). The full enum is in each wiki's `SCHEMA.md` "Log format" block. All prefixes are surfaced separately in `wiki-resume` and `wiki-dashboard`. The `queue` extension is purely additive — no `schema_version` bump required because pre-v0.0.35 readers count `queue` lines in their catch-all bucket without crashing.
