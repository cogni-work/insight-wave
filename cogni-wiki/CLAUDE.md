...
- **wiki-from-research opt-in for wiki-mode deposits** (v0.0.40, intra-plugin; Phase 2 seam for cogni-knowledge #264). `wiki-from-research` gains two additive flags — `--allow-wiki-source` and `--cycle-guard-cleared` — that together lift the default Step 0(3) / Step 1d abort on `report_source ∈ {wiki, hybrid}` projects. Default behavior is unchanged for direct users; the flag pair exists for orchestrators (today: `cogni-knowledge:knowledge-report`) that have verified no circular-evidence chain via their own cycle-guard pass. This skill does NOT re-run the guard itself — cycle semantics are caller-specific (cogni-knowledge owns the `derived_from_research:` lineage stamp it walks; a future caller may walk a different signal). Step 0(3) also gets its project-config path corrected from `<project>/project-config.json` to the canonical `<project>/.metadata/project-config.json` to match `cogni-research/scripts/initialize-project.sh:487`. The change is additive — pre-v0.0.40 callers see identical behavior because both new flags default to absent and the conditional refusal collapses back to the unconditional refusal. Closes the cogni-knowledge round-trip seam.
