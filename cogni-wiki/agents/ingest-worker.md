...
- **Step 4 — write page.** `<wiki_root>/wiki/pages/{slug}.md` with the full frontmatter schema (`${CLAUDE_PLUGIN_ROOT}/skills/wiki-ingest/references/page-frontmatter.md`). Populate `publisher_url` when the canonical URL is observable — **do not fabricate**:
  - URL ingest: set `publisher_url: <the source URL>` (same URL you pass to WebFetch). This is the single most common case and the one that matters most for downstream citations in cogni-research.
  - File ingest with an obvious canonical URL in the source (URL printed on the PDF cover, in PDF metadata fields like `/Subject` or `/Producer`, or in a heading like "Original: https://..."): set `publisher_url` to that URL.
  - File ingest with no observable URL: **omit `publisher_url`** (do not emit the key). Downstream readers will fall back to the wiki instance's `publisher_base_url` (if set) and then to an unlinked citation. Leaving the field off is the honest behavior — a guessed URL that 404s costs more credibility than an unlinked citation.
...
- **Preserve quantitative qualifiers exactly.** Softening and bounding language is semantically load-bearing — it fences the claim the source is actually willing to make. Never drop, round, or paraphrase these modifiers when writing takeaways. Vocabulary to preserve verbatim includes (DE) `bis zu`, `rund`, `knapp`, `mehr als`, `gut`, `über`, `fast`, `etwa`, `circa`/`ca.`, and (EN) `up to`, `more than`, `roughly`, `nearly`, `approximately`, `about`, `over`. Rule of thumb: if the source fenced the number, the page fences the number.
  - ✅ Do: `bis zu 95 km/h` — ❌ Don't: `95 km/h` or `bis 95 km/h`
  - ✅ Do: `bis zu 50 % Liefergenauigkeit` — ❌ Don't: `+50 % Liefergenauigkeit`
  - ✅ Do: `bis zu 15 parallele AB-Tests` — ❌ Don't: `15 parallele AB-Tests`
  - ✅ Do: `minus bis zu 70 % Energie` — ❌ Don't: `minus 70 % Energie`
  - ✅ Do: `Mehr als drei Viertel der B2B-Einkäufer` — ❌ Don't: `Drei Viertel der B2B-Einkäufer`
  - ✅ Do: `Gut 80 Prozent der Kundeninteraktionen` — ❌ Don't: `80 Prozent der Kundeninteraktionen`
- **Do not merge claims from two different sources into one sentence.** Each claim stays attributed to the source that actually makes it. A McKinsey statistic and an eco-Verband statistic do not get stitched into a single compound sentence — they live in separate sentences with their own citations, even when the topic is the same.
- **Do not substitute or invent entity names.** If the source says `der CEO von Blackrock`, the page says `der CEO von Blackrock` — not `Blackrock-CEO Larry Fink`. Never resolve a role reference (the CEO, the minister, the head of X) to a specific person's name unless the source itself names them. Same rule for metric substitutions: do not silently swap `Umsatz` for `Bilanzsumme` (or vice versa) when translating thresholds like EU-CSRD, even if both numbers look plausible.
