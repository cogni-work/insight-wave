...
| `--fix=<class>` | No | Apply the deterministic auto-fix for one or more lint classes (repeatable; `--fix=all` enables every safe class). Supported: `reverse_link_missing`, `synthesis_no_wiki_source`, `entries_count_drift`, `frontmatter_defaults`, `alphabetisation`, `raw_citation_depth`, `portal_heading_dedup`. See §"Auto-fix mode" below. |
...
| `raw_citation_depth` | Repairs depth-wrong `../raw/<file>` source citations (left over from the pre-schema-0.0.5 flat layout, when pages did not yet live two levels deep) by rewriting them to `../../raw/<file>`. Touches both the frontmatter `sources:` field (all three YAML shapes) and body `## Sources` links; prose links elsewhere are never touched. Rewrites only when `<wiki-root>/raw/<file>` exists — skips otherwise rather than guessing. Subdirectory tails are preserved. Inverts `health.py`'s `missing_source` check so an upgraded wiki can clear the new errors in one pass and run lint again. Idempotent: an already-`../../raw/` citation is left untouched. | per-type page body via `_wikilib.atomic_write` |
| `portal_heading_dedup` | Reconciles a `wiki/index.md` portal that already carries a `## <theme>` heading split across two competing sections on disk — the repair complement to the on-write single-instance prevention. Folds each later duplicate's `- [[slug]]` bullets into the first occurrence (merged + alphabetised, skipping a slug the survivor already carries), preserves the first section's curated lead-in verbatim, and drops the duplicate heading shell. Routes through `wiki_index_update.py --collapse-only`, reusing the same pure function the inserter runs on every write — no separate dedup logic. Idempotent: a clean (single-instance) index is a no-op. | `wiki/index.md` via `wiki_index_update.py` |

**Locking.** The four in-process page-body fixers (`reverse_link_missing`, `synthesis_no_wiki_source`, `frontmatter_defaults`, `raw_citation_depth`) run inside one `_wiki_lock(wiki_root)` block so they serialise against concurrent `wiki-ingest` runs. The three scripted fixers (`entries_count_drift`, `alphabetisation`, `portal_heading_dedup`) acquire their own locks via the underlying scripts, after the in-process lock is released — no recursion, no deadlock.
