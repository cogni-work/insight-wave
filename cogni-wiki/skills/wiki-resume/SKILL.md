...
- `queue` (v0.0.35+) — a sub-object with `available`, `pending`, `running`, `done`, `failed`, `oldest_pending_id`, `running_started_at`, plus four drainer-hint fields added in v0.0.39 (T3.2): `oldest_pending_age_hours`, `last_next_at`, `last_next_age_hours`, `drainer_hint_threshold_hours`. `available: false` when `<wiki-root>/.cogni-wiki/queue/` is absent (every wiki that has never run `wiki-ingest --enqueue` falls into that branch). The `done` field is an **all-time** count from a raw directory listing (matches the rest of `wiki_status.sh`'s file-count style); for the 30-day filtered `done_recent` figure, dispatch `${CLAUDE_PLUGIN_ROOT}/skills/wiki-ingest/scripts/wiki_queue.py --status` directly. `running_started_at` is the `started_at` ISO timestamp of the oldest job currently in `running/` — operators read it as a "is the running job stuck?" hint without the v0.0.35 surface needing lease/PID-file machinery. `oldest_pending_age_hours` (v0.0.39) is derived from the leading Unix-second prefix of `oldest_pending_id` and tells rule 5a how long the queue has been stalled. `last_next_at` (v0.0.39) is the newest `finished_at` across `done/` ∪ `failed/` (bounded scan: 200 newest files per bucket; falls back to file mtime when older jobs lack the `finished_at` stamp) and tells rule 5a whether a drainer is recently active. `last_next_age_hours` is derived from `last_next_at`. `drainer_hint_threshold_hours` is the config-driven threshold (default 24, override via the `drainer_hint_threshold_hours` key in `.cogni-wiki/config.json`); rule 5a fires only when both `oldest_pending_age_hours` AND `last_next_age_hours` exceed it (or `last_next_at` is null). Surfaced separately by Step 3 and used by Step 4's queue rules 3a / 5a.
...
5a. **`queue.available AND queue.pending > 0 AND queue.running == 0 AND queue.oldest_pending_age_hours > queue.drainer_hint_threshold_hours AND (queue.last_next_at is null OR queue.last_next_age_hours > queue.drainer_hint_threshold_hours)`** → "{queue.pending} sources have been queued for {oldest_pending_age_hours:.0f}h with no recent --next activity (last drain: {last_next_at or 'never'}). Set up a scheduled drainer — see `${CLAUDE_PLUGIN_ROOT}/references/scheduled-drainer/` (GitHub Actions recommended for git-hosted wikis; Cloud Routine for remote-but-no-GH; local cron/launchd for on-box-only). Or drain one now with `/cogni-wiki:wiki-ingest --next`." (v0.0.39+, T3.2; tightened from v0.0.35's unconditional pending>0 trigger so users who are actively manually draining aren't nagged.)
   - When pending > 0 but the threshold gate doesn't fire (oldest pending is fresh, OR a recent `--next` has completed within the threshold window), the queue counts already show in Step 3's Inventory line — no separate rule fires, the user sees the queue state at a glance and decides themselves.
...
Rules 1–3 are new in v0.0.27 — they fire on the freshly-collected health snapshot so structural problems surface before any other recommendation. The deterministic split lets resume make a confident statement about what is broken without burning lint tokens. Rules 3a and 5a are new in v0.0.35 (T3.1) — they're placed by the same "what blocks reliable reads first" ranking the existing rules follow. `queue.failed` (3a) comes after `claim_drift_count` because failed-queue jobs are an operator-attention signal, not a structural-correctness one — they don't outrank `entries_count_drift`. `queue.pending` (5a) sits next to `orphan_raw_count` because both are "you have work queued up" prompts. Rule 5a was tightened in v0.0.39 (T3.2) — it now also requires the oldest pending job to exceed `drainer_hint_threshold_hours` (default 24, configurable per wiki via `.cogni-wiki/config.json`) AND requires no recent `--next` activity within the same window. The combination prevents nagging a user who is actively manually draining (recent `last_next_at` keeps the rule silent) or who just enqueued a batch in the last hour (oldest_pending_age_hours below threshold keeps the rule silent). The hint fires only when the queue is genuinely *stalled* — at which point pointing the user at `references/scheduled-drainer/` to install a scheduled runner is the right next step. Rule 5's concrete `--discover` command is deliberate: the older prose-only recommendation ("run wiki-ingest on them") left Claude (and by extension the user) to figure out how to enumerate the orphans, which in practice meant asking the user to type them out. Rule 6 (new in v0.0.30) surfaces the persistent open-questions backlog only when lint is fresh — a stale lint would mean stale gaps. Rule 9 surfaces the "the wiki is a vault, but you haven't asked it anything" anti-pattern that file-back synthesis pages were built to fix.
...
- `${CLAUDE_PLUGIN_ROOT}/references/scheduled-drainer/` — scheduled-drainer deployment shapes (T3.2, v0.0.39+); rule 5a points the user here when the queue is genuinely stalled
