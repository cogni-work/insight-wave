...
**Citation rule**: every new factual claim the update adds must link to a source. Either an inline `[../raw/file.pdf]` reference or a `[[wikilink]]` to another page (which in turn must cite a source). If the user cannot produce a source, the update stops. No exceptions.
