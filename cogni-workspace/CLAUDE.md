...
- `tests/test-mcp-declaration-hygiene.sh` keeps that true: no `cogni-*/.mcp.json` exists, the registry still maps `mcp_excalidraw` to the desktop key `excalidraw` (the key MCP tool names derive from, so renaming it silently breaks every `mcp__excalidraw__*` matcher), and cogni-portfolio's PreToolUse matcher — the only one left once this plugin's render chain retired — survives. It carries a per-arm liveness floor so a glob that stops matching cannot make every case pass vacuously
...
**Decision (maintainer ruling, 2026-09-13): the doku wiki is retired.** This plugin bundled a vendor-curated reference wiki about the ecosystem — plugin, skill, agent, concept, architecture and workflow pages — shipped twice: a repo-root `wiki/` tree as the editing surface and `cogni-workspace/wiki/` as the vendored copy synced into it by a release script. It went because nothing read it: its one programmatic consumer, the `ask` skill, had already retired, leaving the pages reachable only by a prose instruction to open the index; every page duplicated what the plugin READMEs, the `CLAUDE.md` files and `docs/` already state; and keeping two trees in agreement had grown a sync script, a 600-line reconciliation record and four guard suites with no reader on the other side.

- **What went with it:** both trees; `scripts/release-bundle-wiki.sh` and its suite; the three wiki suites (`test-wiki-tree-parity.sh`, `test-wiki-bare-name-roster.sh`, `test-wiki-namespace-sync.sh`); the `PAGE_PARITY` arm of `tests/test-layering-claim-reconciled.sh` (its cases L4, L5, L7 and L10 — the surviving ids are deliberately not renumbered); arm D1 of `tests/test-mcp-declaration-hygiene.sh`; and `references/wiki-tree-reconciliation.md`, the two-tree decisions record. The `wiki` keyword and the wiki clause left the plugin description in both manifests
- **What has no successor:** the command cheatsheet and the plugin-selection guide the wiki carried from the cogni-help retirement. Workflow walkthroughs live on as `docs/workflows/*.md`. All page content stays recoverable from git history
- **What this means for future edits:** a roster change no longer has a wiki surface to sweep, and a retirement no longer needs a wiki-side decision recorded. The `**/.cogni-wiki/.lock` ignore rule stays — it serves cogni-knowledge's user-created knowledge bases, which are a different system sharing only a page-layout convention
...
- `text-to-narrative` transforms structured input into an executive narrative using a story arc selected from the registry and adds Phase 7, which cuts the finished narrative into one `design-brief.md` for Claude Design (slides, document, infographic or web). It succeeded the `narrative` skill and is now the **only** home of every narrative asset, bundled **flat** because a directory nested under `references/` is a skill-spec finding (the retired skill's nested tree carried 18 baseline entries, deleted with it). `references/arc-registry.md` chooses (one declarative block per arc plus the detection algorithm and the shortlist confirmation); each arc is **one contract file**, `references/arc-{arc}.md`, carrying `contract: 2` frontmatter and a fixed heading set — Intent, Selection, Headings, Composition, Elements, Validation, See Also; `references/techniques-overview.md` strengthens; `references/language-{shared,en,de}.md` express, loaded at Pass 3 only; `references/validation.md` is the single home of every universal gate, its deterministic half run by `scripts/validate-narrative.py`; `references/density-ceilings.md` is the one home of every text-length ceiling the brief applies, one table per target, read at run time by `scripts/check-design-brief.py`; `references/design-brief-template.md` owns the brief grammar, with the green fixtures under `tests/fixtures/design-brief/` as its worked examples. The suites: `tests/test-arc-contract-shape.sh` keeps every arc on the one-file shape and fails when a nested layer grows back; `tests/test-arc-taxonomy-sync.sh` case H1 derives the render side's short element names from the contracts' headings; `tests/test-narrative-validate.sh` proves each deterministic gate goes red on a narrative that breaks precisely its rule and green on three conforming fixtures; `tests/test-narrative-bridge-citations.sh` pins the citation bridge on both input shapes; and `tests/test-text-to-narrative-brief.sh` grades the brief. `references/density-ceilings.md` is now the sole home of the slide ceilings (their former home, `scripts/check-brief.py`, retired with the render chain) and still states the web and infographic ceilings beside the `libraries/` files they were carried from. **What retired with `narrative` and has no successor:** the `--format` derivative mode (executive brief, talking points, one-pager), the `narrative-writer` and `narrative-adapter` agents, and the `/narrative` and `/narrative-adapt` commands. The derivative-mode drop is deliberate, and generated narratives have no condensation successor: every `text-to-narrative` output carries `arc_id`, while `copywriter` rejects `--scope=compress` in arc mode. For non-arc documents only, `/copywrite <file> --scope=compress` remains the lossless word-count reduction path without recreating the retired format templates. The derivative-mode trigger phrases remain retired in `references/retired-trigger-phrases.tsv`; only the four generation phrases were re-claimed by the successor. `commands/text-to-narrative.md` registers `/text-to-narrative`
- `copywriter` polishes documents with 7 messaging frameworks (BLUF, Pyramid, SCQA, STAR, PSB, FAB, Inverted Pyramid), arc-aware preservation, and EN/DE-pivot translation across seven languages, and its Step 4 reads the result back through parallel stakeholder personas. Agent: `copywriter`
- `commands/copywrite.md` registers `/copywrite`, the one copywriting slash command; `--scope=review` is the standalone stakeholder read

**Decision (maintainer ruling, 2026-09-13): `copy-reader` is retired and its review absorbed into `copywriter`.** The seam was already one-way — copywriter's Step 4 did nothing but dispatch copy-reader, and the copywriter agent's JSON contract already carried `stakeholder_reviews[]` and `synthesis{}` — so the material moved one level down rather than being re-homed. The seven persona profiles and `synthesis-protocol.md` now sit flat under `skills/copywriter/references/`, and `references/stakeholder-review.md` carries the procedure (parallel fresh-context persona agents, synthesis, one improvement pass) so the SKILL.md stays under the skill-spec length ceiling. The `reader` agent and the `/review-doc` command went with the skill; `/copywrite <file> --scope=review` (with `--personas=` and `--no-improve`) is the standalone read, and cogni-portfolio's `portfolio-communicate` next-step prose points there. One behaviour changed on purpose: the retired skill's report asserted post-improvement persona scores that no persona had re-measured — the one `major` finding in `docs/architecture/loop-health-map.md` — and the absorbed step reports **pre-edit scores only**, stating what was applied and never what it achieved. The ten trigger phrases are ledgered as retired in `references/retired-trigger-phrases.tsv`, none re-claimed: the copywriter description had 24 characters of headroom under the 1024 cap. `tests/test-relocated-skill-hygiene.sh` dropped the two `TREE_SPECS` rows for the deleted tree and agent; the surviving copywriter rows still pin the `cogni-copywriting:` token out of the adopted tree.
...
**Decision (maintainer ruling, 2026-09-07): the local render chain is retired.** This plugin renders nothing inside Claude Code any more. `text-to-narrative` is the one visual path: it hands a single `design-brief.md` to Claude Design, which renders and themes it. The chain that used to render a hand-authored or caller-supplied brief locally — adopted from cogni-visual across the absorption stages — had no in-repo producer once the `story-to-*` brief producers and `narrative-publish` retired, and none was being added; a chain whose only input is a brief someone writes by hand against a template is a maintenance surface with no pipeline behind it, so it went rather than being kept on a named producer.

- **What went:** the `render-html-slides` and `enrich-report` skills; the five slash commands (`/enrich-report`, `/render-html-slides`, `/render-infographic`, `/render-infographic-editorial`, `/render-infographic-handdrawn`); fifteen agents — the per-format renderers `render-infographic-pencil`, `render-infographic-sketchnote`, `render-infographic-whiteboard`, `storyboard`, `web`, `pptx`, `html-slides`, the orchestrator `enrich-report`, the workers `concept-diagram`, `concept-diagram-svg`, `editorial-sketch`, `report-html-writer`, `enriched-report-reviewer`, `slides-enrichment-artist`, and the `brief-review-assessor` scorer; the scripts `check-brief.py`, `parse-brief.py`, `brief-render-qa.py`, `cartographic-outline.py` and `rasterize-sketch.py`, with the vendored Natural Earth data under `references/cartographic-data/` that only the cartographic sketch read (its root `NOTICE` entry went with it); `references/agent-tool-declarations.md` and `scripts/breadcrumb-allowlist.txt`, whose every entry named a deleted agent; 29 of the 35 `libraries/` files — the worked example briefs, the brief contract and validation checklists, the layout vocabularies, the CTA taxonomy and the Excalidraw / SVG pattern libraries; the suites `test-check-brief.sh`, `test-parse-brief.sh`, `test-brief-render-qa.sh`, `test-slide-grammar.sh`, `test-brief-layout-sync.sh`, `test-brief-density-sync.sh` and `test-agent-interactive-flag.sh` (whose discovered population emptied) with their fixtures; and the `PreToolUse` Excalidraw canvas hook, `hooks/ensure-excalidraw-canvas.sh`, since no agent here grants an `mcp__excalidraw__*` tool any more — cogni-portfolio keeps its byte-identical copy, and the canvas-lock suite moved there with it. `hooks.json` therefore declares `SessionStart` only. The registry's `required_by` no longer lists this plugin for `mcp_excalidraw` or `pencil`, and the three prose mirrors the MCP hygiene suite pins follow it
- **What stayed, and why:** six `libraries/` files are runtime dependencies of surviving skills, not renderer material, and a wholesale `libraries/` deletion would have broken `text-to-narrative` at run time. `arc-taxonomy.md` is read by `text-to-narrative`'s arc registry, `tests/test-arc-taxonomy-sync.sh`, cogni-website and cogni-portfolio; `presentation-intent.md` by the design-brief template and cogni-consult's publish routing; `infographic-style-presets.md`, `infographic-block-copywriting.md`, `web-section-architecture.md` and `web-section-copywriting.md` are the homes `references/density-ceilings.md` cites for the infographic and web ceilings, and the cogni-website page templates read the last two. Re-measure with `git grep -ohE 'libraries/[A-Za-z0-9._-]+' -- cogni-workspace/skills/text-to-narrative/` before touching that tree again; the six were fixed by measurement, not by name
- **Trigger phrases:** the 21 phrases the two retired skills advertised are ledgered in `references/retired-trigger-phrases.tsv` as retired, none re-claimed — a rendering phrase must not route to `text-to-narrative`, whose input and output contracts differ
- **Earlier retirements this closes out:** the three `story-to-*` brief producers and their four driver agents, and `narrative-publish`, had already retired in favour of `text-to-narrative`; their trigger phrases are ledgered. Brief-based rendering is history, not a live capability
