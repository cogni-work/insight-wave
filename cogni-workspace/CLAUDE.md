...
- `tests/test-mcp-declaration-hygiene.sh` keeps that true: no `cogni-*/.mcp.json` exists, the registry still maps `mcp_excalidraw` to the desktop key `excalidraw` (the key MCP tool names derive from, so renaming it silently breaks every `mcp__excalidraw__*` matcher), cogni-portfolio's PreToolUse matcher — the only one left once this plugin's render chain retired — survives, and the two copies of `concept-mcp-server-map.md` stay byte-identical. It carries a per-arm liveness floor so a glob that stops matching cannot make every case pass vacuously
...
- **Earlier retirements this closes out:** the three `story-to-*` brief producers and their four driver agents, and `narrative-publish`, had already retired in favour of `text-to-narrative`; their trigger phrases are ledgered. The brief-based rendering the wiki's concept pages describe is history, not a live capability
