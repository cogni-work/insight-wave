...
| **Phase analyst** | The agent that assesses phase readiness and recommends methods from the 10-method library |
...
The phase-analyst agent recommends methods from the 10-method library for this phase. You select which to apply.
...
| Method recommendations feel wrong for my domain | The phase-analyst recommends from a 10-method library that may not cover all domains | You can select any method manually; the recommendations are suggestions, not requirements |
...
- **New consulting methods** — the 10-method library in `references/methods/` covers standard approaches; new methods follow the YAML frontmatter format in the existing files
